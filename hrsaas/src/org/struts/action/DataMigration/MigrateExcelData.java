...
public class MigrateExcelData {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(MigrateExcelData.class);
	...
	public static void downloadTemplateWithData(HttpServletRequest request,
		...
		try {
			...
			String templateFilePath = dataPath + "/DataMigration/Templates/"
					+ templateName;

			String downloadFileName = "PeoplePower_Employee_" + applicationName
					+ ".xls";
			String downloadFilePath = dataPath + "/DataMigration/" + client
					+ "/Templates/" + downloadFileName;

			File clientFile = new File(downloadFilePath);
			new File(dataPath + "/DataMigration/" + client + "/Templates")
					.mkdirs();
			...
			ReportGenerator rg = new ReportGenerator("Xls", "Sample", workbook,
					sheet);
			...
			rg.tableBodyAsText(columnNames, data, cellWidth, cellAlign,
					colsAsDouble);
		...
		try {
			...
			for (int i = 0; i < lastColNo; i++) {
				...
				if (cellForColName != null) {
					...
					String cellValueForColInfo = cellForColInfo
							.getStringCellValue().trim();

					if (!(cellValueForColName.equals("")
							|| cellValueForColName.equals("Integrity Status") || cellValueForColName
							.equals("Comments"))) {
			...
			if (columnNameMap != null && columnNameMap.size() > 0) {
				...
				for (Iterator<Integer> k = columnNameMap.keySet().iterator(); k
						.hasNext();) {
					columnInfo[l][0] = k.next();
					columnInfo[l][1] = columnNameMap.get(Integer
							.parseInt(String.valueOf(columnInfo[l][0])));
					columnInfo[l][2] = columnInfoMap.get(Integer
							.parseInt(String.valueOf(columnInfo[l][0])));
					l++;
				}
			}
	...
	private static String[][] getColumnNames(String dataPath,
		...
		try {
			...
			if (listOfColumns != null && listOfColumns.length > 0) {
				...
				for (int i = 0; i < listOfColumns.length; i++) {
					...
					String columnName = properties
							.getProperty(column + ".name");
					String columnInfo = properties
							.getProperty(column + ".info");
				...
				if (colNames != null && colNames.size() > 0) {
					...
					for (Iterator<Integer> keyIt = colNames.keySet().iterator(); keyIt
							.hasNext();) {
						int key = keyIt.next();
	...
	public static HashMap<Integer, Boolean> isColumnsMandatory() {
		HashMap<Integer, Boolean> columnInformation = new HashMap<Integer, Boolean>(
				0);
		try {
	...
	public static boolean isFileToBeUploaded() {
		...
		try {
			...
			rg.tableBodyAsText(colNames, uploadData, cellWidth, cellAlign,
					colsAsDouble);
	...
	public static void openTemplate(HttpServletRequest request,
			HttpServletResponse response, String templateName, String filePath) {
		try {
	...
	private static Properties readColumnsInformation(String dataPath) {
		...
		try {
			FileInputStream fileInputStream = new FileInputStream(dataPath
					+ "/DataMigration/Columns_Information.properties");
	...
	private static Object[][] uploadDateData(ArrayList<String> dataFromExcel,
			int colNo) {
		...
		try {
			...
			for (int rowNo = 0; rowNo < dataFromExcel.size(); rowNo++) {
				if (!(dataFromExcel.get(rowNo) == null
						|| dataFromExcel.get(rowNo).equals(null)
						|| dataFromExcel.get(rowNo).equals("null") || dataFromExcel
						.get(rowNo).equals(""))) {
				...
				try {
					...
					writeStatusData(rowNo, "Fail", "'" + excelValue + "' in '"
							+ columnName + "' is not in 'mm/dd/yyyy' format.");
	...
	public static Object[][] uploadExcelData(int colNo,
			Object[][] dataFromMaster, int dataType, boolean isMandatory) {
		System.out
				.println("isMandatory value i am getting is =============================="
						+ isMandatory);
		...
		try {
			...
			if (lastRowNo > 0) {
				for (int rowNo = START_ROW_NO; rowNo <= lastRowNo + 1; rowNo++) {
							...
							if (cell != null) {
								if (dataType == DATE_TYPE) {
									try {
										...
										cal.setTime(d);
										excelValue = (cal.get(Calendar.MONTH) + 1)
												+ "/"
												+ cal
									...
									if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC
										...
										|| dataType == NUMBER_DOUBLE_TYPE_NEGATIVE) {
									if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC
											|| cell.getCellType() == HSSFCell.CELL_TYPE_FORMULA) {
										excelValue = String.valueOf(cell
												.getNumericCellValue());
									} else {
										excelValue = cell.getStringCellValue()
												.trim();
					...
					System.out.println("dataFromExcel.get(counter)--"
							+ dataFromExcel.get(counter) + "-");
					if (isMandatory && dataFromExcel.get(counter).equals("")) {
						String columnName = colNames[0][colNo];
						writeStatusData(counter, "Fail", columnName
								+ " should not be blank.");
			...
			if (isDataValid && dataFromExcel != null
					&& dataFromExcel.size() > 0) {
				switch (dataType) {
				case STRING_TYPE:
				...
				case MASTER_TYPE:
					excelData = uploadMasterData(dataFromExcel, dataFromMaster,
							colNo);
					break;

				case STRING_TYPE_WITHOUT_SPL_CHARS:
					excelData = uploadStringDataWithoutSplChars(dataFromExcel,
							colNo);
					break;
				...
				case MASTER_DATE_TYPE:
					excelData = uploadMasterDateData(dataFromExcel,
							dataFromMaster, colNo);
					break;
	...
	/** This method checks for duplicate values in the sheet
	 * @param dataFromExcel
	 * @param colNo
		...
		try {
			...
			for (int rowNo = 0; rowNo < dataFromExcel.size(); rowNo++) {
				...
				if (isDataDuplicate) {
					...
					String columnName = colNames[0][colNo];
					writeStatusData(rowNo, "Fail", "'" + excelValue
							+ "' in '" + columnName
							+ "' is duplicate.");
					fileToBeUploaded = false;
				}
	...
	private static Object[][] uploadMasterData(ArrayList<String> dataFromExcel,
			Object[][] dataFromMaster, int colNo) {
			...
			for (int rowNo = 0; rowNo < dataFromExcel.size(); rowNo++) {
				...
				if (dataFromMaster != null && dataFromMaster.length > 0) {
					for (int j = 0; j < dataFromMaster.length; j++) {
						String idFromMaster = String
								.valueOf(dataFromMaster[j][0]);
						String valueFromMaster = String.valueOf(
								dataFromMaster[j][1]).trim();

						if (excelValue.equalsIgnoreCase(valueFromMaster)
								|| excelValue.equals(" ")) {
							if (excelValue.equals(" ")) {
								masterData[rowNo][0] = "";
							} else {
				...
				if (!isDataExists) {
					...
					String columnName = colNames[0][colNo];
					if(excelValue.equals(" ")){
					writeStatusData(rowNo, "Fail", "'Values not available in "
							+ columnName + "' Master.");
					}
					else{
						writeStatusData(rowNo, "Fail", "'" + excelValue
								+ "' in '" + columnName
								+ "' not available in master.");
					}
					fileToBeUploaded = false;
				}
			}
	...
	private static Object[][] uploadMasterDateData(
			ArrayList<String> dataFromExcel, Object[][] dataFromMaster,
			int colNo) {
				...
				if (dataFromMaster != null && dataFromMaster.length > 0) {
					for (int j = 0; j < dataFromMaster.length; j++) {
						...
						try {
							...
							String dateFromExcel = (cal.get(Calendar.MONTH) + 1)
									+ "/"
									+ cal.get(Calendar.DAY_OF_MONTH)
									+ "/" + cal.get(Calendar.YEAR);

							cal.setTime(sdf.parse(valueFromMaster));
							String dateFromMaster = (cal.get(Calendar.MONTH) + 1)
									+ "/"
									+ cal.get(Calendar.DAY_OF_MONTH)
									+ "/" + cal.get(Calendar.YEAR);

							if (dateFromExcel.equals(dateFromMaster)
									|| excelValue.equals(" ")) {
								if (excelValue.equals(" ")) {
	...
	private static Object[][] uploadNumberData(ArrayList<String> dataFromExcel,
			int colNo) {
		...
		try {
			...
			for (int rowNo = 0; rowNo < dataFromExcel.size(); rowNo++) {
				...
				if (matcher.matches() || excelValue.equals(" ")) {
					...
					String columnName = colNames[0][colNo];
					writeStatusData(rowNo, "Fail", "'" + excelValue + "' in '"
							+ columnName + "' is not in proper format.");
	...
	private static Object[][] uploadNumberNegativeData(
			ArrayList<String> dataFromExcel, int colNo) {
		Object[][] numberData = null;
		try {
			numberData = new Object[dataFromExcel.size()][1];
			String excelValue = " ";
	...
	private static Object[][] uploadStringData(ArrayList<String> dataFromExcel,
			int colNo) {
	...
	private static Object[][] uploadStringDataWithoutSplChars(
			ArrayList<String> dataFromExcel, int colNo) {
		Object[][] stringData = null;
		try {
			stringData = new Object[dataFromExcel.size()][1];
	...
	private static Object[][] uploadYearData(ArrayList<String> dataFromExcel,
			int colNo) {
	...
	private static void writeStatusData(int rowNo, String status, String comment) {
		try {
			...
			if (!(uploadData[rowNo][lastColNo] == null
					|| uploadData[rowNo][lastColNo].equals(null)
					|| uploadData[rowNo][lastColNo].equals("") || uploadData[rowNo][lastColNo] == "")) {
				oldStatus = String.valueOf(uploadData[rowNo][lastColNo]);
			}

			if (!(uploadData[rowNo][lastColNo + 1] == null
					|| uploadData[rowNo][lastColNo + 1].equals(null)
