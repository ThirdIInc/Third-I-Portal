 ...
 * 
 */
public class MonthlyArrearsAction extends ParaActionSupport {

	private MonthlyArrears monthlyArrears;
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(MonthlyArrearsAction.class);

	public void prepare_local() throws Exception {
		monthlyArrears = new MonthlyArrears();
		monthlyArrears.setMenuCode(603);
	}

	public Object getModel() {
		return this.monthlyArrears;

	}

	public MonthlyArrears getMonthlyArrears() {
		return monthlyArrears;
	}

	public void setMonthlyArrears(MonthlyArrears monthlyArrears) {
	...
	public String input() throws Exception {
		MonthlyArrearsModel model = new MonthlyArrearsModel();
	...
	public String callForEdit() throws Exception{
		...
		MonthlyArrearsModel model = new MonthlyArrearsModel();
		model.initiate(context, session);
		Object[] netAmt=model.getNetAmount(monthlyArrears.getArrCode(),monthlyArrears.getArrRefYear());
		monthlyArrears.setTotalEmpCount(String.valueOf(netAmt[0]));
		monthlyArrears.setTotalNetAmount(String.valueOf(netAmt[1]));
		model.terminate();
	...
	public String processArrears() 
	{
		try {
			...
			if(empId != null && empId.length > 0)
			{
				...
				rows = new Object[empId.length][c.length+d.length+12];
			...
			if(monthlyArrears.getPayinSalFlag().equals("true")){
			...
			{  commented by mangesh*/ 
			 if(!model.chkJoiningDate(monthlyArrears))
			{
				finalData = rows;
	...
	public String processRecovery() 
	{
		try {
			...
			if(model.chkSalFinalize(monthlyArrears))
			{
				finalData = rows;
				addActionMessage("Salary for the month "+Utility.month((Integer.parseInt(monthlyArrears.getArrRefMonth())))
						+"-"+monthlyArrears.getArrRefYear()+" has been finalized. So monthly recovery cannot be added or modified");
				request.setAttribute("rows", null);
				model.terminate();
				getNavigationPanel(2);
				return SUCCESS;
			...
			/*else if(!model.chkSalProcess(monthlyArrears) && !model.chkJoiningDate(monthlyArrears))
			{  commented by mangesh*/ 
			else if(!model.chkJoiningDate(monthlyArrears))
			{
				finalData = rows;
				addActionMessage("Salary for "+monthlyArrears.getEmpName()+" for the month "+Utility.month((Integer.parseInt(monthlyArrears.getArrMonth())))
						+"-"+monthlyArrears.getArrYear()+" has not been processed. So monthly recovery cannot be added");
				model.setHeaders(monthlyArrears, request);
				request.setAttribute("rows", rows);
				reset_Emp();
				model.terminate();
			...
			{
				// if recovery already processed
				if(!model.chkArrProcessed(monthlyArrears))
				{
					// processing for selected employee - UPDATED BY REEBA
					//resultData = model.processArrears(request, monthlyArrears,path, roundParam,monthlyArrears.getDivEsicZone());
					resultData = model.processRecovery(request, monthlyArrears,path, roundParam);
					Calendar cal = Calendar.getInstance();
					cal.setTime(Utility.getDate(01 + "-" + monthlyArrears.getArrMonth() + "-" + monthlyArrears.getArrYear()));
					double daysOfMonth = Double.parseDouble(String.valueOf(cal.getActualMaximum(java.util.Calendar.DAY_OF_MONTH)));					if(!monthlyArrears.getSalDays().equals(""))
						salDaysValue = Double.parseDouble(monthlyArrears.getSalDays());
					
					if(salDaysValue!=0){
						logger.info("Org Arr Days "+monthlyArrears.getOrgArrDays());
						if(Double.parseDouble((String.valueOf(monthlyArrears.getOrgArrDays()))) > (salDaysValue)){
							logger.info("In Arr Days > daysOfMonth - salDaysValue" );
							addActionMessage("Salary received  for "+salDaysValue+" days. \nSo recovery days should not exceed "+salDaysValue+" days");
							request.setAttribute("rows", rows);
							model.terminate();
							getNavigationPanel(2);
							return SUCCESS;
		...
		try {
			...
			else if(emp_id != null && emp_id.length > 0)
			{
				...
				int saveType = model.save(rows, d, c, emp_id, month, year,
						total_credit, total_debit, sal_days, monthlyArrears,netPay,forMonth,forYear,eligDays,arrType);
				if (saveType == 1)
		...
		try {
			...
			try {
				if (!monthlyArrears.getEmpViewId().equals("")) {
					empFilterQuery += " AND HRMS_EMP_OFFC.EMP_ID ="
							+ monthlyArrears.getEmpViewId();
				}
			} catch (Exception e) {
	...
	public String f9Search()
	{
		try {
			String query = " SELECT TO_CHAR(TO_DATE(ARREARS_REF_MONTH,'MM'),'MONTH') , "
						  ...
						  +" HRMS_EMP_TYPE.TYPE_NAME,HRMS_PAYBILL.PAYBILL_GROUP,ARREARS_REF_MONTH, ARREARS_CODE,NVL(ARREARS_PAY_TYPE,'ADD'),DECODE(ARREARS_PAY_IN_SAL,'N','false','true'),DECODE(ARREARS_DEDUCT_TAX,'Y','true','false') FROM HRMS_ARREARS_LEDGER "
	...
	/*		try {
		...
		/*	//UPDATED BY REEBA
			...
			//UPDATED BY REEBA
			String[] fieldNames = {"monthRef","arrRefYear","divName","brnName","deptName","typeName","payBillName","arrRefMonth","arrCode","monthRef","arrearPayType","payinSalFlag","deductTaxFlag"};
			//eArrMonth
			int[] columnIndex = {0,1,2,3,4,5,6,7,8,7,9,10,11};
	 ...
	 * @return
	 */
	public String setFilters(MonthlyArrears monthlyArrears, String sqlQuery)  
	{
		try
		{
	...
	**/
	...
	public void recalculate()
	{
		try {
			...
			String recalEmp[][] = new String[recalChkEmp.length][5];
	...
	public String removeRecord()
	{
		try {
			...
			Object[][] rows = new Object[empId.length - removeEmp.length][c.length+d.length+12];
			Object[][] credit_debit = new Object[empId.length][d.length + c.length];
			for (int i = 0; i < empId.length; i++) {
				/**
				 * FOR GETTING CREDIT AND DEBIT FROM JSP
