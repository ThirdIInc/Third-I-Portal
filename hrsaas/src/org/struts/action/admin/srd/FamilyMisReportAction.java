 ...
 *
 */
public class FamilyMisReportAction extends ParaActionSupport {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
	.getLogger(FamilyMisReportAction.class);

	FamilyMisReport familyreport;
	public void prepare_local() throws Exception {
		// TODO Auto-generated method stub
		familyreport=new FamilyMisReport();
		familyreport.setMenuCode(742);
	}
	...
	public FamilyMisReport getFamilyreport() {
		return familyreport;
	}

	public void setFamilyreport(FamilyMisReport familyreport) {
		this.familyreport = familyreport;
	}

	
	
	public String report() {
		FamilyMisReportModel  model = new FamilyMisReportModel ();
		model.initiate(context, session);
		String result = model.getReport(familyreport, response);
		model.terminate();
		clear();
		return null;
	}
	...
	public String familyReport() throws Exception {
		FamilyMisReportModel model = new FamilyMisReportModel();
		model.initiate(context, session);
		String reportPath="";
		model.getFamilyMISReport(familyreport, request, response, reportPath);
		model.terminate();
		return null;
	}
	...
	public final String mailReport()
	{
		try
		{
		  final FamilyMisReportModel model = new FamilyMisReportModel();
		  model.initiate(context, session);
	...
	public String clear() {
			...
			familyreport.setEmpid("");
			familyreport.setEmpName("");
			
			familyreport.setDesgCode("");
			familyreport.setDesgName("");
			familyreport.setCenterId("");
			familyreport.setCenterName("");
			
			
			familyreport.setToken("");
			familyreport.setStatus("");
			familyreport.setDeptName("");
			familyreport.setDeptCode("");
			
			familyreport.setDivCode("");
			familyreport.setDivsion("");
			familyreport.setReportType("");
			return "success";
		}
	...
	public String f9MultiDiv() {
		try {
			...
			if (this.familyreport.getUserProfileDivision()!= null
					&& this.familyreport.getUserProfileDivision().length() > 0)
				query += " WHERE DIV_ID IN (" + this.familyreport.getUserProfileDivision()
						+ ")";
			...
			logger.error("Error in FamilyReportAction.f9MultiDiv() method Action : "
							+ e.getMessage());
			return "";
		}
	}
	...
	public String f9MultiDept() {
		try {
			...
			logger
					.error("Error in FamilyReport.f9MultiDept() method Action : "
							+ e.getMessage());
			return "";
		}
	}
	...
	public String f9MultiRank() {
		try {
			...
			logger
					.error("Error in FamilyReport.f9MultiRank() method Action : "
							+ e.getMessage());
			return "";
		}
	}
	...
	public String f9div() throws Exception {
		...
		if(familyreport.getUserProfileDivision() !=null && familyreport.getUserProfileDivision().length()>0)
			query+= " WHERE DIV_ID IN ("+familyreport.getUserProfileDivision() +")"; 
			query+= " ORDER BY  DIV_ID ";
