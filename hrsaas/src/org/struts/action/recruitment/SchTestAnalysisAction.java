...
import org.paradyne.model.recruitment.SchTestAnalysisModel;
import org.struts.lib.ParaActionSupport;

public class SchTestAnalysisAction extends ParaActionSupport {

	SchTestAnalysis bean;
		@Override
		public void prepare_local() throws Exception {
			// TODO Auto-generated method stub
			bean=new SchTestAnalysis();
			bean.setMenuCode(924);
		...
		public SchTestAnalysis getBean() {
			return bean;
		}

		public void setBean(SchTestAnalysis bean) {
			this.bean = bean;
		}
		...
		public String report() {  
			SchTestAnalysisModel model=new SchTestAnalysisModel();
			model.initiate(context, session);
			String[] colnames = {getMessage("serial.no"),getMessage("cand.name"),getMessage("emailIDLabel"),getMessage("ContactLabel"),getMessage("Interview.Round"),getMessage("interviewDate"),
				getMessage("interviewTimeLabel"),getMessage("interviewVenueLabel"),getMessage("userIdChkLabel"),getMessage("passwordChkLabel"),getMessage("rec.name"),getMessage("schStatusChkLabel"),
				getMessage("reqs.code"),getMessage("reqDateLabel"),getMessage("BranchLabel"),getMessage("DepartmentLabel"),
				getMessage("dividionLabel"),getMessage("position"),getMessage("hiringMangerLabel"),getMessage("AppStatusLabel"),getMessage("reqStatusLabel"),getMessage("interviewRoundTypeLabel")};
		...
		public String f9Interviewer()
		{
			try
			{
			  ...
			  String query = " SELECT EMP_TOKEN ,EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME , EMP_ID "
				  			+" FROM HRMS_EMP_OFFC ORDER BY  EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME	";
		...
		{  
			String query = "SELECT NVL(CAND_FIRST_NAME||' '||CAND_MID_NAME||' '||CAND_LAST_NAME,''),CAND_CODE FROM HRMS_REC_CAND_DATABANK ORDER BY UPPER(CAND_FIRST_NAME) ";
				...
				String [] headers = {getMessage("candidateLabel")};
