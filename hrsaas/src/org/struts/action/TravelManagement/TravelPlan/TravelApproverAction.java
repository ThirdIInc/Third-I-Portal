...
import org.paradyne.model.TravelManagement.TravelPlan.TravelApproverModel;
...
public class TravelApproverAction extends ParaActionSupport{
	...
	public void prepare_withLoginProfileDetails() throws Exception {
		traApp.setMenuCode(809);
		TravelApproverModel model = new TravelApproverModel();
		model.initiate(context, session);
		boolean result=	model.callRequistionStatus(traApp);
		if(result){ 
			//model.callStatus(traApp,"P",request);
		}else
		{
			addActionMessage("Insufficient priviliges to approve the Travel Requisition."); 
		}
	...
	public String saveForSingleApprover()
	{
		TravelApproverModel model = new TravelApproverModel();
	    model.initiate(context, session);
	    int level =model.getLevelNo(traApp); 
	    System.out.println("traApp.getApproverComments()=========== "+traApp.getApproverComments());
	    model.forward(traApp, "A", traApp.getTrAppId(),traApp.getApproverComments(),traApp.getEmpId());
		Object[][]empFlow=generateEmpFlow(traApp.getEmpId(), "TYD", (level+1));
		boolean result= model.changeApplStatus(traApp, empFlow,traApp.getTrAppId(),traApp.getEmpId());
		if(result){
	...
	public String saveForSingleReject()
	{
		TravelApproverModel model = new TravelApproverModel();
	    model.initiate(context, session);
	    model.forward(traApp, "R", traApp.getTrAppId(),traApp.getApproverComments(),traApp.getEmpId());  
