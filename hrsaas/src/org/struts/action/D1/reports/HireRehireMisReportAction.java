...
import org.paradyne.model.D1.reports.HireRehireMisReportModel;
...
public class HireRehireMisReportAction extends ParaActionSupport {
	HireRehireMisReport misreport;
	
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
	.getLogger(org.struts.lib.ParaActionSupport.class);
	
	public void prepare_local() throws Exception {
		misreport = new HireRehireMisReport();
		misreport.setMenuCode(2059);
	}
	
	public HireRehireMisReport getModel() {
		return misreport;
	}

	public HireRehireMisReport getMisreport() {
		return misreport;
	}



	public void setMisreport(HireRehireMisReport misreport) {
	...
	public String f9division() throws Exception {
		...
		String submitToMethod = "HireRehireMisReport_chk.action";
	...
	public String f9employee() throws Exception {
		...
		String submitToMethod = "HireRehireMisReport_chk1.action";
	...
	public String saveReport() throws Exception {
		HireRehireMisReportModel model = new HireRehireMisReportModel();
	...
	public String reset() throws Exception {
		...
		misreport.setTrackingNumber("");
		misreport.setEmpFirstName("");
		misreport.setActionReason("");
		misreport.setFirstNameFlag("");
		misreport.setMiddleNameFlag("");
		misreport.setLastNameFlag("");
		misreport.setSocialSecurityNumberFlag("");
		misreport.setCityFlag("");
		misreport.setStateFlag("");
		misreport.setPhoneNumberFlag("");
		misreport.setSexFlag("");
		misreport.setMartialStatusFlag("");
		misreport.setEducationFlag("");
		misreport.setBirthDateFlag("");
		misreport.setReferalSourceFlag("");
		misreport.setHireDateFlag("");
		misreport.setPhysicalWorkLocationFlag("");
		misreport.setActionReasonFlag("");
		misreport.setGradeFlag("");
		misreport.setPayGroupFlag("");
		misreport.setAnnualSalaryFlag("");
		misreport.setExecutiveEmployeeFlag("");
		misreport.setTrackingNumberFlag("");
	...
	public String searchSavedReport() throws Exception {
		...
		String query = " SELECT REPORT_CRITERIA, NVL(REPORT_TITLE,' '), REPORT_ID FROM  HRMS_MISREPORT_HDR "
				+ " WHERE REPORT_TYPE = 'HireRehireMIS'  ORDER BY  REPORT_ID ";
		 ...
		 */
		String submitToMethod = "HireRehireMisReport_setDetails.action";
	...
	public String viewOnScreen() {
		...
		try{
			...
			model.initiate(context, session);
			//PASSING LABEL NAMES
			String labelNames[] = {getMessage("first.name"), getMessage("middle.name"),
					getMessage("last.name"), getMessage("social.security.number"),
					getMessage("city"), getMessage("state"),
					getMessage("phone.number"), getMessage("sex"),
					getMessage("martial.status"),getMessage("education"), 
					getMessage("birth.date"),getMessage("referal.source"),
					getMessage("hire.date"),getMessage("physical.work.location"),
					getMessage("action.reason"),getMessage("grade"),
					getMessage("pay.group"),getMessage("annual.salary"),
					getMessage("executive.employee"),getMessage("tracking.number"),getMessage("social.insurance.number"),
					getMessage("division"),getMessage("department"),
					getMessage("branch"),getMessage("designation"),
					getMessage("employee.type"),getMessage("application.date"),
					getMessage("application.status")};
			//CALL TO MODEL
	 ...
	 * @throws Exception
	 */
	public String f9empFirstName() throws Exception {
		String query = " SELECT DISTINCT TO_CHAR(EMP_FIRST_NAME) FROM  HRMS_D1_NEW_HIRE_REHIRE  "
				+ " ORDER BY  EMP_FIRST_NAME ";
		String[] headers = { getMessage("employee.first.name") };

		String[] headerWidth = { "100" };

		String[] fieldNames = { "misreport.empFirstName" };
		int[] columnIndex = { 0 };
		String submitFlag = "false";
		String submitToMethod = "HireRehireMisReport_chk3.action";
	...
	public String f9trackingNumber() throws Exception {
		String query = " SELECT TRACKING_NUMBER FROM  HRMS_D1_NEW_HIRE_REHIRE  "
				+ " ORDER BY  TRACKING_NUMBER ";
		String[] headers = { getMessage("tracking.number") };

		String[] headerWidth = { "100" };

		String[] fieldNames = { "misreport.trackingNumber" };
		int[] columnIndex = { 0 };
		String submitFlag = "false";
		String submitToMethod = "HireRehireMisReport_chk3.action";
