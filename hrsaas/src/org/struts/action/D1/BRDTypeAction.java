...
import org.paradyne.model.D1.BRDTypeModel;
 ...
 *
 */
public class BRDTypeAction extends ParaActionSupport {
	
	BRDType bean = null;
	
	public void prepare_local() throws Exception {
		bean = new BRDType();
		bean.setMenuCode(5036);
	}

	/**
	 * @return the bean
	 */
	public BRDType getBean() {
		return this.bean;
	}
	 ...
	 */
	public void setBean(BRDType bean) {
		this.bean = bean;
	}
	 ...
	 */
	public String input() {
		BRDTypeModel model = new BRDTypeModel();
	...
	public String callPage()  {
		final BRDTypeModel model = new BRDTypeModel();
		model.initiate(context, session);
		this.input();
		this.getNavigationPanel(1);
		model.terminate();
	...
	public String f9action() {
		final String query = "SELECT TYPE_NAME, TYPE_ID  FROM HRMS_D1_BRD_TYPE ";
		final String[] headers = {this.getMessage("typeLabel")};
		final String[] headerWidth = {"15"};
		final String[] fieldNames = {"typeName", "typeCode"};
		final int[] columnIndex = {0, 1};
		final String submitFlag = "true";
		final String submitToMethod = "BRDType_setRecord.action";
	...
	public String saveData() throws Exception {
		try {
			...
			model.initiate(context, session);
			boolean result;
			if ("".equals(this.bean.getTypeCode())) {
