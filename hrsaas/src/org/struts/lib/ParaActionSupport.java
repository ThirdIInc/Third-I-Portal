...
public abstract class ParaActionSupport extends ActionSupport implements ModelDriven, Preparable, ServletRequestAware, ServletResponseAware, ServletContextAware {
	...
	public Object[][] generateDeptFlow(String deptCode, String type, int order) {
		...
		Object result[][] = reporting.generateDeptFlow(deptCode, type, order);
		reporting.terminate();
		return result;
	}
	...
	public void getPanel(String menuCode, String modeCode, boolean insertFlag, boolean updateFlag, boolean deleteFlag, 
		...
		try {
			...
			System.out.println("----------------|navigation list|---------------"+navList);
		...
		if(bean == null) {
			try {
				...
				//logger.error("Exception in setting response text:" + e);
			}
		}	
	}
	...
	/**
	 * Get records for employee who is logged in
	 */
	/**
	 * @param bean
	 * @return String as part of query that returns records who is logged in
	 */
	...
	public String getprofileQuery(BeanBase bean) {
		...
		if (bean.isGeneralFlag()) {
			...
			/*if (bean.getUserProfilePaybill().length() > 0) {
				if ( bean.getUserProfileEmpType().length() > 0 || bean.getUserProfileCenters().length() > 0) {
					query += " AND ";
				}else{
					query += " WHERE ";
				}
				query += " HRMS_EMP_OFFC.EMP_PAYBILL IN ("
					+ bean.getUserProfilePaybill() + ")";
			}*/
	...
	public void prepare() throws Exception {
		 ...
		 */
		try {
			LoginBean bean = (LoginBean)request.getSession().getAttribute("loginBeanData");
			if(bean == null) {
				...
				response.sendRedirect("http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/pages/common/loginSessionPage.jsp");
				...
				//Updated by Prajakta B(4 june 2013) for multiple profile codes
				applicationBean.setUserProfileId(bean.getProfileCode());
				applicationBean.setMultipleProfileCode(bean.getMultipleProfileCode());
				//ends Updated by Prajakta B(4 june 2013) for multiple profile codes
				applicationBean.setUserAccessProfileId(bean.getAccessProfileCode());
				applicationBean.setScreenWidth(bean.getScreenWidth());
				...
				applicationBean.setUserProfileCenters(bean.getUserProfileCenters()); //center
				applicationBean.setUserProfileEmpType(bean.getUserProfileEmpType()); //emptype
				applicationBean.setUserProfilePaybill(bean.getUserProfilePaybill()); //emptype
				applicationBean.setUserProfileDivision(bean.getUserProfileDivision()); //paybill 
				 ...
				 */
				try {
					boolean[] rights = userModel.getAccessRights(String.valueOf(applicationBean.getMenuCode()), bean.getMultipleProfileCode());
					...
					/*	applicationBean.setUserProfileCenters(userModel.getProfileCenters(bean.getProfileCode())); //center
						...
						applicationBean.setUserProfilePaybill(userModel.getProfilePaybill(bean.getProfileCode())); //emptype
...
*/					
		...
		//Calculate Serever response time
		try {
			long timeEnd = System.currentTimeMillis();
			double responseTime = (timeEnd - timeStart) / 1000.0;
	...
	public void setF9Window(String query, String[] headers,
		 ...
		 setF9Window(query, headers,
					...
					submitFlag, submitToMethod,false ,null,false,null);
	}
	...
	public void setF9Window(String query, String[] headers,
			String[] headerWidth, String[] fieldNames, int[] columnIndex,
			String submitFlag, String submitToMethod,String otherDB) {
		 setF9Window(query, headers,
					 headerWidth, fieldNames, columnIndex,
					submitFlag, submitToMethod,false ,null,false,otherDB);
	}
	...
	public void setF9Window(String[][] data , String[] headers,
			String[] headerWidth, String[] fieldNames, int[] columnIndex,
			String submitFlag, String submitToMethod) {
		 setF9Window(null, headers,
					 headerWidth, fieldNames, columnIndex,
					submitFlag, submitToMethod,false ,data,false,null);
	}
	public void setF9WindowSearch(String query, String[] headers,
			String[] headerWidth, String[] fieldNames, int[] columnIndex,
			String submitFlag, String submitToMethod) {
		 setF9Window(query, headers,
					 headerWidth, fieldNames, columnIndex,
					submitFlag, submitToMethod,false ,null,true,null);
	}
	/**
	 ...
	 */
	public void setF9Window(String query, String[] headers, String[] headerWidth, String[] fieldNames, int[] columnIndex, String submitFlag, String submitToMethod, boolean isLms, String[][] data,boolean f9search,String otherDB) {
		int RECORDS_TOTAL = 0;
		int TOTAL_RECORDS = 0;
		int PAGE_NO = 0;
		int RECORDS_TO = 0;
		...
		if(applicationBean.getF9query().equals("")) {
			if (data!=null) {
				...
				obj = model.getData(query,isLms,otherDB);
			}
		} else{
			obj = model.getData(f9query,isLms,otherDB);
		} // end of else statement
	...
	/**
	 * @param query -: Specifies sql query to execute
	 * @param headers -: Specifies column heading
	 * @param textAreaId -: Specifies textArea Id
	 * @param hiddenId -: Specifies Id of hidden field
	 * @param submitFlag -: Specifies the boolean flag to call another method which returns 'success'
	 * @param submitToMethod -: Specifies the path of the method
	 */
	public void setMultiSelectF9(String query, String header,String textAreaId,String hiddenId,String submitFlag,String submitToMethod ) { 
		F9Model model = new F9Model();
		model.initiate(context, session);
		String[][] obj = null;
		obj = model.getData(query, false,null);
		
		request.setAttribute("multiSelectData",obj);
		request.setAttribute("header", header);
		request.setAttribute("textAreaField", textAreaId);
	...
	/**
	 * This function is used to set the report
	 * @param query
	 * @param RequiRecords
	 * @param headers
	 * @param headerWidth
	 * @param fieldNames
	 * @param fieldNamesForRequi
	 * @param columnIndex
	 * @param columnIndexForRequi
	 * @param title
	 * @param request
	 */
	public void setReport(Object[][] cand, String query, Object[][] RequiRecords, String[] headers, String[] headerWidth, String[] fieldNames, String[] fieldNamesForRequi, int[] columnIndex, int[] columnIndexForRequi, String title, String subtitle, HttpServletRequest request) {
		/* Begin This code is for displaying candidate status records along with pagination */
		int RECORDS_TOTAL = 0;
		int TOTAL_RECORDS = 0;
		int PAGE_NO = 0;
		int RECORDS_TO = 0;
		int RECORDS_INTERVAL = 20;
		...
		// if (applicationBean.getF9query().equals("")) {
		// applicationBean.setF9query(query);
		// obj = model.getData(query);
		Object[][] obj = cand;
		// } // end of if statement
		// else {
		// obj = model.getData(f9query);
		// } // end of else statement

		RECORDS_TOTAL = obj.length;
		TOTAL_RECORDS = obj.length;
		int pages = Math.round(RECORDS_TOTAL / 20);// get total number of pages
		double row = (double)obj.length / 20.0;
		java.math.BigDecimal value1 = new java.math.BigDecimal(row);
		...
		if(obj.length == 0) {
			// addActionMessage("NO CANDIDATE DATA FOUND");
		} // end of if statement

		String[] displayData = new String[RECORDS_INTERVAL];
		Object[][] f9DataObject = new Object[RECORDS_INTERVAL][columnIndex.length];
		...
		request.setAttribute("Req", (String)request.getParameter("reqCode"));
		request.setAttribute("candidateCode", (String)request.getParameter("candidateCode"));
		request.setAttribute("rankId", (String)request.getParameter("rankId"));
		request.setAttribute("frmDate", (String)request.getParameter("frmDate"));
		request.setAttribute("toDate", (String)request.getParameter("toDate"));
		request.setAttribute("type", (String)request.getParameter("type"));
		request.setAttribute("subtitle", subtitle);
		/* End This code is for displaying candidate status records along with pagination */

		/* Begin This code is for displaying Requisition records along with pagination */
		int RECORDS_TOTAL1 = 0;
		int TOTAL_RECORDS1 = 0;
		int PAGE_NO1 = 0;
		int RECORDS_TO1 = 0;
		int RECORDS_INTERVAL1 = 0;

		BeanBase applicationBean1 = (BeanBase)getModel();
		RECORDS_TO1 = applicationBean1.getRecordCount1();// Get total number of records fetched from executing a query

		int PageNo1 = 1;

		try {
			if(applicationBean1.getPageNo1() != null) {// Check page is selected or not
				PageNo1 = Integer.parseInt(applicationBean1.getPageNo1());// sets page no. to selected page number
			} // end of if statement
			else {
				RECORDS_TO1 += RECORDS_INTERVAL1;
			} // end of else statement
		} catch(Exception e) {
			//logger.error(e);
		} // end of try-catch block
		applicationBean1.setRecordCount1(RECORDS_TO1);

		/**
		 * Executes a query and get the records
		 */
		CandidateStatusModel model1 = new CandidateStatusModel();
		model1.initiate(context, session);

		RECORDS_TOTAL1 = RequiRecords.length;
		TOTAL_RECORDS1 = RequiRecords.length;
		int pages1 = Math.round(RECORDS_TOTAL1 / 1);// get total number of pages
		double row1 = (double)RequiRecords.length / 1.0;
		java.math.BigDecimal value2 = new java.math.BigDecimal(row1);

		int rowCount1 = Integer.parseInt(value2.setScale(0, java.math.BigDecimal.ROUND_UP).toString());
		model1.terminate();

		if(RequiRecords.length == 0) {
			// addActionMessage("NO DATA FOUND");
		} // end of if statement

		String[] displayData1 = new String[RECORDS_INTERVAL1];
		String[][] f9DataObject1 = new String[RequiRecords.length][columnIndexForRequi.length];
		String[][] data = new String[RECORDS_INTERVAL1][RequiRecords.length];

		int z1 = 0;
		for(int x = 0; x < RequiRecords.length; x++) {
			try {
				if(x <= RECORDS_TO1 && x >= RECORDS_TO1 - RECORDS_INTERVAL1) {
					String valueRequi = "";
					for(int y = 0; y < RequiRecords[x].length; y++) {
						valueRequi += RequiRecords[x][columnIndexForRequi[y]] + ",";
						f9DataObject1[z1][y] = (String)RequiRecords[x][columnIndexForRequi[y]];
					}

					String actValue1 = valueRequi.substring(0, valueRequi.length());
		...
		// Append fields names
		String fldName1 = "";
		for(int i = 0; i < fieldNamesForRequi.length; i++) {
			fldName1 += fieldNamesForRequi[i] + ",";
		} // end of for loop
		fldName1 = fldName1.substring(0, fldName1.length() - 1);

		String[] hedersForRequi = new String[fldName1.length()];

		for(int k = 0; k < fieldNamesForRequi.length; k++) {
			hedersForRequi[k] = fieldNamesForRequi[k];
		}
		// Add column headings into a MAP
		java.util.Map<String, String> headersMAP1 = new HashMap<String, String>();
		for(int i = 0; i < fieldNamesForRequi.length; i++) {
			headersMAP1.put("" + i, hedersForRequi[i]);
		} // end of for loop

		/**
		 * Sets the necessary request objects
		 */
		request.setAttribute("headerMAP1", headersMAP1);
		request.setAttribute("headerNames1", hedersForRequi);
		request.setAttribute("fieldNames1", fieldNamesForRequi);
		request.setAttribute("f9Data1", GloActionSupport.scanData(f9DataObject1));
		request.setAttribute("displayData1", displayData1);
		request.setAttribute("pages1", rowCount1);
		request.setAttribute("PageNo1", PageNo1);
		request.setAttribute("TOTAL_RECORDS1", TOTAL_RECORDS1);

		/* End This code is for displaying Requisition records along with pagination */
