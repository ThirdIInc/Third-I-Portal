 ...
 * 
 */
public abstract class CustomerActionSupport extends ActionSupport implements
		ModelDriven, Preparable, ServletRequestAware, ServletResponseAware,
		ServletContextAware {

	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(org.struts.lib.CustomerActionSupport.class);
	...
	public void prepare()  
	{
		try {
			...
			prepare_local();
			CustomerReportLoginBean bean = null;
			bean = (CustomerReportLoginBean) request.getSession().getAttribute(
					"loginBeanData");
			if(bean == null) {
			...
			applicationBean.setCustomerUserEmpId(bean.getCustomerCode());
	...
	/*public void prepare_OLD() throws Exception {
		...
		try {
			...
			CandidateLoginBean bean = null;
			try {
				bean = (CandidateLoginBean) request.getSession().getAttribute(
						"loginBeanData");
			} catch (Exception e) {
			...
			if (bean == null) {
				...
				BeanBase applicationBean = (BeanBase) getModel(); // INSTANTIATE BEAN
				//	applicationBean.setUserLoginCode(bean.getLoginCode());
				
				System.out.println("bean.getCandidateCode()                       "+bean.getCandidateCode());
				 	applicationBean.setCandidateUserEmpId(bean.getCandidateCode());
	...
	public String getprofileQuery(BeanBase bean) {
		...
		try {
			if (bean.isGeneralFlag()) {
				...
				/*
				 * if (bean.getUserProfilePaybill().length() > 0) { if (
				 * bean.getUserProfileEmpType().length() > 0 ||
				 * bean.getUserProfileCenters().length() > 0) { query += " AND ";
				 * }else{ query += " WHERE "; } query += " HRMS_EMP_OFFC.EMP_PAYBILL
				 * IN (" + bean.getUserProfilePaybill() + ")"; }
				 */
	...
	public String getprofilePaybillQuery(BeanBase bean) {
		...
		if (bean.isGeneralFlag()) {
			...
			if (bean.getUserProfilePaybill().length() > 0) {
				query += "WHERE HRMS_EMP_OFFC.EMP_PAYBILL IN ("
	...
	public void setF9Window(String query, String[] headers, String[] headerWidth, String[] fieldNames, int[] columnIndex, String submitFlag, String submitToMethod, boolean isLms) {
		...
		if(applicationBean.getF9query().equals("")) {
			...
			obj = model.getData(query,isLms);
		} else {
			obj = model.getData(f9query,isLms);
		} // end of else statement
