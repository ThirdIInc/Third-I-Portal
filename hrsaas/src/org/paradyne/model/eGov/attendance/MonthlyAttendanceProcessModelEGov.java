 ...
 *
 */
public class MonthlyAttendanceProcessModelEGov extends ModelBase {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(MonthlyAttendanceProcessModelEGov.class);
	 ...
	 */
	private Object[][] attendanceSettings = null, uploadAttendance = null,uploadRecAttendance = null, holidaysFromDailyAttendance = null, holidaysFromMaster = null;
				...
				for (int i = 0; i < attendanceDetailsOnDaysBasis.length; i++) {	
					...
					if(uploadRecAttendanceMap !=null && uploadRecAttendanceMap.size()>0){
						...
						if(leaveRecoveryObj !=null && leaveRecoveryObj.length>0){
							 ...
							 */
							if(leavePolicyData!=null && leavePolicyData.length>0){
								for (int d = 0; d < leavePolicyData.length; d++) {
									//System.out.println("String.valueOf(leavePolicyData[d][0]) :"+String.valueOf(leavePolicyData[d][0]));
									if(String.valueOf(leavePolicyData[d][0]).equals(String.valueOf(attendanceDetailsOnDaysBasis[i][1]))){
										if(String.valueOf(leavePolicyData[d][1]).equals("P")){
											paidFlag=true;
										}	
										if(String.valueOf(leavePolicyData[d][1]).equals("U")){
											unPaidFlag=true;
	...
	public void loadAttendanceDetails(String concatEmployeeIds, LeavePolicyData leavePolicyData, boolean branchHoliDayFlag, 
		...
		try {
			...
			uploadRecAttendance=loadUploadRecAttendanceData(year, month);
			uploadRecAttendanceMap=loadUploadRecAttendanceDataMap(year, month);
	 ...
	 * @throws Exception
	 */
	private Object[][] loadUploadRecAttendanceData(String year,String month) throws Exception{
		String query="SELECT ATTN_DTL_EMP_ID,ATTN_DTL_LEAVE_ID,ATTN_DTL_LEAVE_VALUE,ATTN_DTL_LEAVE_REC_VALUE FROM HRMS_UPLOAD_ATTN_DTL_"+year+" " 
					+"	WHERE ATTN_DTL_MONTH="+month+" AND  ATTN_DTL_YEAR="+year+"  AND (ATTN_DTL_LEAVE_VALUE>0 OR ATTN_DTL_LEAVE_REC_VALUE>0 )";
	 ...
	 * @throws Exception
	 */
	private HashMap<String, Object[][]> loadUploadRecAttendanceDataMap(String year,String month) throws Exception{
		...
		for (int i = 0; i < saveAttendanceDetails.length; i++) {
			...
			if(uploadRecAttendanceMap !=null && uploadRecAttendanceMap.size()>0){
				...
				if(leaveRecovery !=null && leaveRecovery.length>0){
					saveAttendanceDetailsRecovery[i][count]=String.valueOf(leaveRecovery[0][3]);
				}
			}
			...
			if(leaveApprovalDaysMap !=null && leaveApprovalDaysMap.size()>0){
				...
				if(leaveApprovalDays !=null && leaveApprovalDays.length()>0){
					saveAttendanceDetailsRecovery[i][count+1]=String.valueOf(leaveApprovalDays);
				}
			}
			/**
			...
			if(uploadRecAttendanceMap !=null && uploadRecAttendanceMap.size()>0){
				...
				if(leavePolicyEmpData!=null && leavePolicyEmpData.length>0){
					for (int d = 0; d < leavePolicyEmpData.length; d++) {
						if(String.valueOf(leavePolicyEmpData[d][0]).equals(String.valueOf(saveAttendanceDetailsRecovery[i][1]))){
							if(String.valueOf(leavePolicyEmpData[d][1]).equals("P")){
								if(leaveRecovery!=null && leaveRecovery.length>0){
								saveAttendanceDetailsRecovery[i][count+1]=String.valueOf(leaveRecovery[0][2]);
								}
							}	
						}
					}								
	...
	public void getMonthAttendanceReport(MonthlyAttendanceProcessEGov bean, HttpServletResponse response
			,String[]listOfFilters) {
		try {
	 ...
	 */
	public String setFiletrs(MonthlyAttendanceProcessEGov bean, String sqlQuery) {
		try {
