...
public class NonIndustrialSalaryeGovModel extends ModelBase {  
	static org.apache.log4j.Logger logger=org.apache.log4j.Logger.getLogger(NonIndustrialSalaryeGovModel.class); 
	HashMap<String, Object[][]> deductMap = new HashMap<String, Object[][]>();
	/**
	 * GET EMP_ID THOSE FULLFILL SORTING CRITERIA
	 * @param NonIndustrialSalaryeGov object 'nonIndustrialSalary'
	...
	/**
	 * GET EMP_ID THOSE FULLFILL SORTING CRITERIA
	 * @param NonIndustrialSalaryeGov object 'nonIndustrialSalary', 'proMonth' process month
	 * @return EMPLOYEE Object 'emp_id[][]' with required info
	 */
	 ...
	 * @return ledger Code
	 */
	public Object[][] getLedgerCode(NonIndustrialSalaryeGov nonIndustrialSalary,String ledgerStatus) {
	 ...
	 */
	public Object[][] getSalary(NonIndustrialSalaryeGov nonIndustrialSalary,HttpServletRequest request,
		...
		try {
			...
			ArrayList<NonIndustrialSalaryeGov> creditNames = new ArrayList<NonIndustrialSalaryeGov>();
			ArrayList<NonIndustrialSalaryeGov> debitNames = new ArrayList<NonIndustrialSalaryeGov>();
			...
			for (int i = 0; i < credit_header.length; i++) {
				 ...
				 * LOOP IS USED
				 */
				NonIndustrialSalaryeGov creditName = new NonIndustrialSalaryeGov();
			...
			for (int i = 0; i < debit_header.length; i++) {
				 ...
				 */
				NonIndustrialSalaryeGov debitName = new NonIndustrialSalaryeGov();
	 ...
	 * @return rows Object with all details of employees
	 */
	public Object[][] recalSalary(NonIndustrialSalaryeGov nonIndustrialSalary,HttpServletRequest request,String[] recal_emp,
			String ledgerStatus,String path) {
	 ...
	 */
	public Object[][] getCredit(String emp_id, String month, String year,NonIndustrialSalaryeGov nonIndustrialSalary,String salDays,Object [][] recoveryAmount,String shiftHrs ) {
		Object[][] credit_amount = null;
	...
	public Object[][] viewSalary(NonIndustrialSalaryeGov nonIndustrialSalary,HttpServletRequest request,
			String ledgerStatus,String path) {
	...
	public Object[][] getRow(String emp_id, String emp_token, String emp_name,String location,
			...
			Object debitLength[][],NonIndustrialSalaryeGov nonIndustrialSalary,
		...
		/*
		 * INSERT PROCESS DEBIT INOT TEMP TABLE
		 */
		...
		if(debit_amount !=null && debit_amount.length>0){
			...
			for (int i = 0; i < debit_amount.length; i++) {
				for (int j = 0; j < objDebit.length; j++) {
					if(String.valueOf(objDebit[j][0]).equals(String.valueOf(debit_amount[i][0]))){
						count++;
					}
				}				
			}
			Object[][]saveObj=new Object[count][5];	
			count=0;
			for (int i = 0; i < debit_amount.length; i++) {
				for (int j = 0; j < objDebit.length; j++) {
					if(String.valueOf(objDebit[j][0]).equals(String.valueOf(debit_amount[i][0]))){
				saveObj[count][0]=emp_id;
				saveObj[count][1]=debit_amount[i][0];
				saveObj[count][2]=debit_amount[i][1];
				saveObj[count][3]=month;
				saveObj[count][4]=year;
				count++;
				}
				}
			}
	 ...
	 * @return
	 */	
	public String checkProcessStatus(NonIndustrialSalaryeGov bean) {
	 ...
	 * @param nonIndustrialSalary
	 */
	public void getFilters(NonIndustrialSalaryeGov nonIndustrialSalary){
		try {
	 ...
	 */
	public void doPaging(NonIndustrialSalaryeGov bean, int empLength, Object[][] empObj, 
	 ...
	 * @param bean
	 */
	public void filters(NonIndustrialSalaryeGov bean)
	{
		try
		{
	 ...
	 * @return boolean result
	 */
	public boolean updateLock(NonIndustrialSalaryeGov nonIndustrialSalary,String[]emp_id, Object[][] rows, Object[][] c,Object[][]d){
...
*/			try {
			...
			String dedQuery="SELECT NVL(DEBIT_CODE,0),NVL(DEBIT_AMOUNT,0),EMP_ID||'#'||DEBIT_CODE " +
				...
				if(rows !=null && rows.length>0){
					...
					for (int i = 0; i < emp_id.length; i++) {
						k=0;
						for (int j = count; j <c.length+d.length; j++) {
							calculatedDebit = Float.parseFloat(String
									.valueOf(rows[i][j]));
							key=emp_id[i]+"#"+String.valueOf(d[k][0]);
							Object [][]dedObj=deductMap.get(key);
							originaldDebit=0.0f;
							if(dedObj !=null && dedObj.length>0){
								originaldDebit = Float.parseFloat(String
										.valueOf(dedObj[0][1]));
								//System.out.println("originaldDebit :"+originaldDebit);
							}
							float debitToLock = originaldDebit-calculatedDebit;
							if (debitToLock > 0) {
								Object[][] lockData = new Object[1][4];
								lockData[0][0] = (month.equals("12")) ?"1"
										: "" + (Integer.parseInt(month) + 1);								
								String newYear = (month.equals("12")) ? ""
										+ (Integer.parseInt(year) + 1) : year;
								lockData[0][1] = emp_id[i];
								lockData[0][2] = String.valueOf(d[k][0]);
	 ...
	 * @return boolean result
	 */
	public boolean unLock(NonIndustrialSalaryeGov nonIndustrialSalary){
		boolean result = false; 		
		try {
