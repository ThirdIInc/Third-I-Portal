...
public class PFBalanceeGovReportModel extends ModelBase {
	
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(PfBalanceRegisterModel.class);
	NumberFormat formatter = new DecimalFormat("#0.00");
	public void generateReport(PFBalanceeGovReportBean  bean,HttpServletResponse response){
		String empQuery="SELECT EMP_ID,EMP_TOKEN,EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME,'' FROM HRMS_EMP_OFFC" 
			+" left JOIN HRMS_PAYBILL ON (HRMS_PAYBILL.PAYBILL_ID = HRMS_EMP_OFFC.EMP_PAYBILL)"
			+"WHERE EMP_DIV="+bean.getDivCode()+" AND EMP_CENTER="+bean.getBranchCode();
		...
		if(!bean.getPaybillId().trim().equals("")){
			empQuery+=" AND HRMS_PAYBILL.PAYBILL_ID="+bean.getPaybillId();
		} 
		 ...
		 else 
         {
			...
			noDataObj[0][0] = "No records available";
        	 System.out.println("in else part --------------------------------------->");
						TableDataSet dataName = new TableDataSet();
						dataName.setData(noDataObj);
						dataName.setCellAlignment(new int[] { 1 });
						dataName.setCellWidth(new int[] { 100 });
						dataName.setBodyFontDetails(Font.HELVETICA, 12, Font.BOLD, new Color(
								0, 0, 0));
						dataName.setBorder(false);
						rg.addTableToDoc(dataName);
	...
	public void loadEmployeeInfo(PFBalanceeGovReportBean bean,Object [][]empObj){
	...
	public HashMap getPfSubData(PFBalanceeGovReportBean bean,String frmMonth,String toMonth,String year,Object[][] empObj){
	...
	public HashMap getPfRefundData(PFBalanceeGovReportBean bean,String frmMonth,String toMonth,String year,Object[][] empObj){
		String empIds = convertEmpListToString(empObj);
		empIds = Utility.getConcatenatedIds("HRMS_SAL_DEBITS_"+year+".EMP_ID", empIds);
	...
	public HashMap getPfLoanData(PFBalanceeGovReportBean bean,String frmMonth,String toMonth,String year,Object[][] empObj){
		String empIds = convertEmpListToString(empObj);
	...
	public HashMap getPfOpeningBalanceData(PFBalanceeGovReportBean bean,Object[][] empObj){
		String frmYear="";
		String date[]=bean.getSysDate().split("-");
