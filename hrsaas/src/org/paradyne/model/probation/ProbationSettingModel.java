...
import org.paradyne.bean.probation.ProbationSetting;
...
public class ProbationSettingModel extends ModelBase {
	...
	public void setRecord(ProbationSetting probationSetting) {
		...
		String query = " SELECT DISTINCT CADRE_ID,TO_CHAR(CADRE_NAME) FROM  HRMS_CADRE  "
			...
			probationSetting.setProbationDays("0");
		} catch (Exception e) {
	...
	public void setSavedRecord(ProbationSetting probationSetting) {
		try {
		 	String query = " SELECT DISTINCT CADRE_ID,TO_CHAR(CADRE_NAME),HRMS_PROBATION_DTL.PROBATION_GRADEDAYS FROM  HRMS_CADRE "
					+ "  LEFT JOIN HRMS_PROBATION_DTL ON(HRMS_PROBATION_DTL.PROBATION_GRADE=HRMS_CADRE.CADRE_ID)  "
					+ " ORDER BY CADRE_ID ";
			Object gradeDataObj[][] = getSqlModel().getSingleResult(query);
			if (gradeDataObj != null && gradeDataObj.length > 0) {
				ArrayList list = new ArrayList();
				for (int i = 0; i < gradeDataObj.length; i++) {
					...
					bean.setGradeId(checkNull(String.valueOf(gradeDataObj[i][0])));
					bean.setGradeName(checkNull(String.valueOf(gradeDataObj[i][1])));
					bean.setProbationGradeDays(checkZero(String
	...
	public void setHdrRecord(ProbationSetting probationSetting)
	{
		try {
			String hdrQuery = " SELECT PROBATION_APPLICABLE,HRMS_EMP_TYPE.TYPE_NAME,NVL(PROBATION_DAYS,0),PROBATION_EMPTYPE "
			...
			if (objData != null && objData.length > 0) {
				...
				probationSetting.setEmpType(checkNull(String
						.valueOf(objData[0][1])));
				probationSetting.setProbationDays(checkNull(String
						.valueOf(objData[0][2])));
				probationSetting.setEmpTypeCode(checkNull(String
						.valueOf(objData[0][3])));
			}
		} catch (Exception e) {
			logger.error("Exception in setHdrRecord-------"+e);
		}
