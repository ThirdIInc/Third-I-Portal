...
public class RequestBusinessCardMisReportModel extends ModelBase {

	public String getReport(RequestBusinessCardMisReport familyreport,
			HttpServletResponse response) {
		String reportName = "Business Mis Report";
		...
		rg.addTextBold(" Business MIS Report", 0, 1, 0);
	...
	public void getFamilyMISReport(RequestBusinessCardMisReport fmr, HttpServletRequest request, HttpServletResponse response, String reportPath)
	{
		org.paradyne.lib.ireportV2.ReportDataSet rds = new org.paradyne.lib.ireportV2.ReportDataSet();
		String type= fmr.getReport();
		rds.setReportType(type);
		String fileName = "BusinessCardMISReportDetails"+ Utility.getRandomNumber(1000);
		rds.setFileName(fileName);
		rds.setReportName("Business Card MIS Report");
		...
		else
		{
			...
			request.setAttribute("action","/srd/RequestBusinessCardMisReport_input.action");
		...
		if (!fmr.getDivsion().equals("")) {
			filterClause += " AND ( OFFC2.EMP_DIV IN("
					+ fmr.getDivCode()+") OR HRMS_EMP_OFFC.EMP_DIV IN (" + fmr.getDivCode()+") )" ;
		...
		if (!(fmr.getStatus().equals("-1")))
		 {
			filterClause += " AND HRMS_BUSINESS_CARD_INFO.BUSINESS_STATUS='"+ fmr.getStatus() + "'";
		 }
	     
		//filterClause += " ORDER BY OFFC1.EMP_ID";
		...
		String query = " SELECT HRMS_EMP_OFFC.EMP_TOKEN, "
             ...
             + " HRMS_BUSINESS_CARD_INFO.BUSINESS_QTY_CARDS,TO_CHAR(HRMS_BUSINESS_CARD_INFO.BUSINESS_REQUIRE_DATE,'DD-MM-YYYY'), "
             + " DECODE(HRMS_BUSINESS_CARD_INFO.BUSINESS_STATUS,'A','APPROVED', 'P', 'Pending With Manager','D','DRAFT','B', 'Send Back', 'R', 'Rejected'), "
             + "(OFFC1.EMP_FNAME)||' '||OFFC1.EMP_MNAME||' '||(OFFC1.EMP_LNAME), TO_CHAR(BCARD_APPL_APPROVAL_DATE,'DD-MM-YYYY'), "	
             //+ "(OFFC2.EMP_FNAME)||' '||OFFC2.EMP_MNAME||' '||(OFFC2.EMP_LNAME) AS PENDING_WITH "
             + " HRMS_BUSINESCARD_APPROVER_DTL.BCARD_APPR_COMMENTS AS MANAGER_AND_ADMIN_COMMENTS, "
             + " DECODE(HRMS_BUSINESCARD_APPROVER_DTL.BCARD_APPROVER_TYPE,'A', 'Admin Approver', 'M', 'Manager Approver') "
             + " FROM HRMS_BUSINESS_CARD_INFO "
             + " LEFT JOIN HRMS_EMP_OFFC ON(HRMS_EMP_OFFC.EMP_ID=HRMS_BUSINESS_CARD_INFO.BUSINESS_EMP_ID)"	
             + " LEFT JOIN	HRMS_BUSINESCARD_APPROVER_DTL ON(HRMS_BUSINESCARD_APPROVER_DTL.BCARD_APPL_CODE = HRMS_BUSINESS_CARD_INFO.BUSINESS_APPL_CODE) "
             + " LEFT JOIN HRMS_EMP_OFFC OFFC1 ON(HRMS_BUSINESCARD_APPROVER_DTL.BCARD_APPROVER_CODE= OFFC1.EMP_ID)"
             + " LEFT JOIN HRMS_EMP_OFFC OFFC2 ON(HRMS_BUSINESS_CARD_INFO.BUSINESS_EMP_ID= OFFC2.EMP_ID) "
		     + " WHERE 1<2";
		    // + " ORDER BY OFFC1.EMP_ID";
             //+ " LEFT JOIN HRMS_EMP_OFFC OFFC2 ON(OFFC2.EMP_ID=HRMS_BUSINESS_CARD_INFO.BUSINESS_APPROVER)";
		...
		if(queryData == null || queryData.length == 0){
		   ...
		   System.out.println("In getBusinessMISReport()QueryData Length: "+queryData.length);
		   tdstable.setHeader(new String[]{"Emp ID"," Applicant Name"," Quantity Of Cards"," Require Upto Date","Status"," Approver Name", "Approval Date", "Manager/Admin Comments", "Approver Level"});	
		   tdstable.setHeaderTable(true);
		   tdstable.setHeaderBorderDetail(3);
		   //tdstable.setHeaderBorderColor(new BaseColor(255,0,0));
		   tdstable.setCellAlignment(new int[]{0,0,0,0,0,0,0,0,1});
		   tdstable.setCellWidth(new int[] {7,11,10,5,10,9,7,10,7});
