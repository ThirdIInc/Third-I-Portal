...
public class PartnerPostResumeModel extends ModelBase {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(PartnerPostResumeModel.class);
	 ...
	 * @param request
	 */
	public void setPreviousList(PartnerPostResume bean, HttpServletRequest request) {
		try {
			...
			if (bean.getFormName().equals("offer")
				...
				candCode = null;
				logger.info("candidate code null");
			}
			...
			if (candCode != null && candCode.length != 0) { // if candidate
					...
					if (bean.getFormName().equals("candSearch")
						...
						if ((status[i].equals("S") || status[i].equals("R"))
							...
							String updateQuery = "UPDATE HRMS_REC_CAND_DATABANK SET CAND_SHORT_STATUS = '"
									+ status[i] + "', CAND_REQUISITION_CODE = "+ requisitionCode
									+", CAND_POSTRESUME_EMPID ="+bean.getUserEmpId()
									+", CAND_POSTRESUME_REQUISITIONID = "+requisitionCode
									+ " WHERE CAND_CODE = " + candCode[i];

							getSqlModel().singleExecute(updateQuery);

							/*
							 * String insertQuery = "INSERT INTO
							 * REC_SHORT_LISTED_CANDIDATE (REQUISITION_CODE,
							 * CANDIDATE_CODE) " +" VALUES ("+requisitionCode+
							 * ","+candCode[i]+")";
							 * 
				...
				candidateCode = candidateCode.substring(0, candidateCode
						.length() - 1);
				logger.info("candidateCode " + candidateCode);
				...
				if (candidateDetails != null && candidateDetails.length != 0) {
					for (int i = 0; i < candidateDetails.length; i++) {
						...
						if (bean.getFormName().equals("candSearch")
							...
							// status[i].equals("S")){
							PartnerPostResume bean1 = new PartnerPostResume();
	 ...
	 * @param request
	 */
	public ArrayList<Object> getCandidateDetailsDup(PartnerPostResume bean,
	...
	public ArrayList<Object> getCandidateDetails(PartnerPostResume bean,
			HttpServletRequest request) {
		ArrayList<Object> candidateList = new ArrayList<Object>();
	 ...
	 * @return Object []
	 */
	public Object[] saveCandidateDetails(PartnerPostResume bean,
			HttpServletRequest request) {
		...
		try {
			...
			System.out.println("Requisition CODE >>>>>>>>>>>>>"
					+ requisitionCode);
			String submitResumeEmployeeID = bean.getUserEmpId();
			for (int i = 0; i < candidateCode.length; i++) {
				code = candidateCode[i];

				// if cand code is blank i.e. candidate is newly added
				if (candidateCode[i].equals("")) {
					// get candidate details in an array to insert in
					// HRMS_REC_CAND_DATABANK table
					...
					candidateData[0][12] = maritalStatus[i]; // marital
																// status
					...
					if (bean.getPartnerFlag().equals("true")) {
						...
						if (bean.isReferalFlag()) {
							...
							candidateData[0][14] = "C"; // Direct
						}
					}
					candidateData[0][15] = requisitionCode; // Requisition Code
					candidateData[0][16] = submitResumeEmployeeID; // Employee ID for Resume submitted by Consultant.
					candidateData[0][17] = requisitionCode; // Requisition Code for Resume Submitted by Consultant

					// query to insert the cand details in
					// HRMS_REC_CAND_DATABANK table
					boolean result = getSqlModel().singleExecute(getQuery(1),
							candidateData);
	 ...
	 * @return boolean
	 */
	public boolean delete(PartnerPostResume bean, HttpServletRequest request) {
		ArrayList<Object> deletedList = new ArrayList<Object>();
		String candidateNotIn = "";
		try {
			// checkboxes value to check whether that candidate is selected for
			// delete or not
			String[] checkBox = request.getParameterValues("checkBox");
			ArrayList<Object> candidateList = getCandidateDetails(bean, request);

			logger.info("checkbox length " + checkBox.length);

			if (checkBox != null && checkBox.length != 0) {
				for (int i = 0; i < checkBox.length; i++) {
					// add only those candidates who are not selected from whole
					// list
					if (checkBox[i].equals("N")) {
						logger.info("iiiiii  " + i);
						PartnerPostResume candBean = (PartnerPostResume) candidateList.get(i);
	 ...
	 * @param candCode
	 */
	public void insertInResumeBank(PartnerPostResume bean, String candCode) {
		String status = "";
	...
	public void refercandidate(PartnerPostResume postResume, HttpServletRequest request) {
		try {
	...
	public void getReport(PartnerPostResume postResume, HttpServletResponse response,
			String[] labelNames, HttpServletRequest request) {
		try {
			
			String[] candidateID = request.getParameterValues("candidateCode");
			String candidateNumber = "";
			if(candidateID != null && candidateID.length > 0) {
				for (int i = 0; i < candidateID.length; i++) {
					candidateNumber += String.valueOf(candidateID[i]+",");
				}
				candidateNumber = candidateNumber.substring(0, candidateNumber.length()-1);
			}
			...
			String query = "SELECT CAND_EXP_YEAR||' Year '||CAND_EXP_MONTH||' Month ', TO_CHAR(CAND_POSTING_DATE, 'DD-MM-YYYY'), "
					...
					+ "CAND_CODE,  CAND_FIRST_NAME||' '||CAND_MID_NAME||' '||CAND_LAST_NAME "
					+ "FROM HRMS_REC_CAND_DATABANK "
					+ "WHERE CAND_CODE IN ("+ candidateNumber + ")";
