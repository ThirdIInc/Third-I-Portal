...
import org.paradyne.bean.Recruitment.AppointmentAnalysisReport;
import org.paradyne.bean.Recruitment.OfferAnalysisReport; 
import org.paradyne.lib.ModelBase;
import org.paradyne.lib.Utility;
import org.paradyne.lib.report.ReportGenerator;

public class AppointmentAnalysisReportModel extends ModelBase{ 

	public void getReport(HttpServletRequest request,
			HttpServletResponse response,AppointmentAnalysisReport bean,String[] colnames) { 
		 ...
		 */
					...
					if(bean.getReportType().equals("X")){
						 ...
						 title [0][0] = "";
						 title [0][1] = "";
						 title [0][2] = "Appointment Letter Analysis";  
				   ...
				   int cellwidth[]=new int [Integer.parseInt(bean.getCheckedCount())];  
					...
					for (int i = 0; i < tab.length; i++) {
						...
						totalObj[i][m+1]=checkNull(""+tab[i][11]);
	...
	public void displayReq(AppointmentAnalysisReport bean)
	{ 
		 ...
		 sql +=" ORDER BY  REQS_DATE DESC";
		  ...
		  { 
			 ...
			 for (int i = 0; i < data.length; i++) {
				 AppointmentAnalysisReport bean1 = new AppointmentAnalysisReport();
	...
	public void callViewScreen(AppointmentAnalysisReport bean,HttpServletRequest request)
	{ 
		  ...
		  { 
			  bean.setDataLength(String.valueOf(data.length));
			   ...
			   for (int i=Integer.parseInt(pageIndex[0]);i<Integer.parseInt(pageIndex[1]);i++) { 
				   ...
				   bean1.setIvAppointmentDate(checkNull(String.valueOf(data[i][10])));  
				   bean1.setIvRecruiterName(checkNull(String.valueOf(data[i][11])));  
	...
	public String callSqlQuery(AppointmentAnalysisReport bean)
	{
		String query="  SELECT REQS_NAME,RANK_NAME,TO_CHAR(REQS_DATE,'DD-MM-YYYY'),SUM(NVL(VACAN_NUMBERS,0)) AS VAC_NO, " 
				+" DUE1,ISSUED1,ACCEPTED1,REJECTED1,CANCELED1, REQUISITION_CODE, TO_CHAR(APPOINT_DATE,'DD-MM-YYYY'),"
				+" REC_NAME "
				+" FROM  ( 	"
				+" SELECT REQS_NAME,RANK_NAME,SUM(ISSUED) AS ISSUED1,SUM(APPOINT_DUE) AS DUE1 ,SUM(ACCEPTED) AS ACCEPTED1, " 
				+" SUM(REJECTED) AS REJECTED1 ,SUM(CANCELED) AS CANCELED1 ,REQUISITION_CODE ,REQS_DATE,APPOINT_DATE, REC_NAME  "
				+" FROM (  "
				+" SELECT REQS_NAME,RANK_NAME, "   
				+" CASE WHEN APPOINT_STATUS ='I' THEN COUNT(APPOINT_STATUS) ELSE 0 END AS ISSUED ,  "
				+" CASE WHEN APPOINT_STATUS ='D' THEN COUNT(APPOINT_STATUS) ELSE 0 END +  "
				+" CASE WHEN APPOINT_APPR_STATUS ='P' THEN COUNT(APPOINT_STATUS) ELSE 0 END AS APPOINT_DUE , "
				+" CASE WHEN APPOINT_STATUS ='OA' THEN COUNT(APPOINT_STATUS) ELSE 0 END AS ACCEPTED, "
				+" CASE WHEN APPOINT_STATUS ='OR' THEN COUNT(APPOINT_STATUS) ELSE 0 END + "
				+" CASE WHEN APPOINT_APPR_STATUS ='R' THEN COUNT(APPOINT_STATUS) ELSE 0 END AS REJECTED, "
				+" CASE WHEN APPOINT_STATUS ='C' THEN COUNT(APPOINT_STATUS) ELSE 0 END AS CANCELED  , "
				+" HRMS_REC_REQS_HDR.REQS_CODE  AS REQUISITION_CODE ,REQS_DATE,APPOINT_DATE,"
				+" HRMS_EMP_OFFC.EMP_FNAME||HRMS_EMP_OFFC.EMP_MNAME||HRMS_EMP_OFFC.EMP_LNAME as REC_NAME   "
				+" FROM HRMS_REC_REQS_HDR   "
				+" LEFT JOIN  HRMS_REC_APPOINT ON (HRMS_REC_REQS_HDR.REQS_CODE = APPOINT_REQS_CODE ) " 
				...
				+" LEFT JOIN  HRMS_EMP_OFFC on (HRMS_REC_VACPUB_RECDTL.PUB_REC_EMPID = HRMS_EMP_OFFC.EMP_ID )"
				
				+" WHERE REQS_APPROVAL_STATUS IN ('A','Q') " 
				+" GROUP BY REQS_NAME,RANK_NAME,APPOINT_STATUS ,HRMS_REC_REQS_HDR.REQS_CODE ,REQS_DATE,APPOINT_APPR_STATUS,APPOINT_DATE, HRMS_EMP_OFFC.EMP_FNAME||HRMS_EMP_OFFC.EMP_MNAME||HRMS_EMP_OFFC.EMP_LNAME ) " 
				+" GROUP BY REQS_NAME,RANK_NAME ,REQUISITION_CODE ,REQS_DATE,APPOINT_DATE, REC_NAME  	)  "
				+" LEFT join HRMS_REC_REQS_VACDTL on(HRMS_REC_REQS_VACDTL.REQS_CODE= REQUISITION_CODE) "
				+" WHERE 1=1  ";
		...
		// ============  started  filter ===========
			...
			if(!bean.getAppointmentDate().equals("")){						
				query+= " AND APPOINT_DATE = TO_DATE('"+ bean.getAppointmentDate()+"','DD-MM-YYYY')" ; ;
			}  // end of if
			...
			// ============  start of advance =========== 
			 ...
			 query +="GROUP BY REQS_NAME,RANK_NAME ,REQUISITION_CODE ,ISSUED1, DUE1,ACCEPTED1,REJECTED1,CANCELED1,REQS_DATE,APPOINT_DATE, REC_NAME  ";
	...
	public void callBack(AppointmentAnalysisReport bean)
	{
		if(bean.getReqCodeChk().equals("on")){
	...
	public void input(AppointmentAnalysisReport bean)
	{
		    bean.setReqCodeChk("checked"); 
			bean.setPostionChk("checked");  
			bean.setReqDateChk("checked"); 
	...
	public void callPreviousRecord(AppointmentAnalysisReport bean)
	{
	  String sql =" SELECT APPOINTREP_CODE, APPOINTREP_NAME FROM HRMS_REC_APPOINTREP_FILTERS WHERE APPOINTREP_USEREMPID ="+bean.getUserEmpId()+" ORDER BY UPPER(APPOINTREP_NAME) ";
	  Object [][] data = getSqlModel().getSingleResult(sql);
		  if(data!=null && data.length>0)
	...
	public boolean callSave(AppointmentAnalysisReport bean)
	{
	...
	public boolean saveFiletr(AppointmentAnalysisReport bean)
	{   
	...
	public boolean saveSort(AppointmentAnalysisReport bean)
	{ 
		Object [][] sortObj =null;
		   if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
	...
	public boolean saveColumnDef(AppointmentAnalysisReport bean)
	{   
		Object [][] colObj = null;
		  if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
	...
	public boolean saveAdvance(AppointmentAnalysisReport bean)
	{  
		Object [][] colAdv =null;
		 if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
	...
	public boolean callUpdate(AppointmentAnalysisReport bean)
	{
		if(saveFiletr(bean) && saveSort(bean) && saveColumnDef(bean) && saveAdvance(bean))
		{
			return true;
	 ...
	 * @param bean
	 */
	public void callFilterSavedData(AppointmentAnalysisReport bean)
	{ 
		
 String sqlFilter ="   SELECT REQS_NAME , APPOINTREP_REQ_CODE,NVL(RANK_NAME,' '), APPOINTREP_POSITION_CODE, "
		   +" APPOINTREP_DATEOPTION, TO_CHAR(APPOINTREP_REQDATE_FROM,'DD-MM-YYYY'), APPOINTREP_REQSTATUS, "
		   +" APPOINTREP_REPOPTION, APPOINTREP_NAME,TO_CHAR(APPOINTREP_REQDATE_TO,'DD-MM-YYYY'),  "
		   +" APPOINTREP_RADIO_OPTION, APPOINTREP_SELECTED_REQ FROM HRMS_REC_APPOINTREP_FILTERS "		   
		   +" LEFT JOIN HRMS_REC_REQS_HDR ON (APPOINTREP_REQ_CODE = REQS_CODE) "
		   +" LEFT JOIN HRMS_RANK  ON (APPOINTREP_POSITION_CODE = RANK_ID ) "		   
		   +" WHERE APPOINTREP_CODE ="+bean.getSearchSetting(); 
	 ...
	 * @param bean
	 */
	public void callSortSavedData(AppointmentAnalysisReport bean)
	{
		String sqlSort ="  SELECT SORT_BY, SORT_BY_ORDER, SORT_THENBY, SORT_THENBY_ORDER, SORT_THENBY1,  "
				  +" SORT_THENBY1_ORDER FROM HRMS_REC_APPOINTREP_SORT WHERE APPOINTREP_CODE = "+bean.getSearchSetting();
	...
	public void callColumnSavedData(AppointmentAnalysisReport bean)
	{
	String sqlColumn ="SELECT COL_REQNAME,COL_POSITION, COL_REQDATE,COL_NUMVAC, COL_APPOINT_DUE, COL_APPOINT_ISSUE, " 
					 +" COL_APPOINT_ACCP, COL_APPOINT_REJECT, COL_APPOINT_CANCEL "
					 +" FROM HRMS_REC_APPOINTREP_COLDEF WHERE APPOINTREP_CODE ="+bean.getSearchSetting();
	...
	public void callAdvanceSavedData(AppointmentAnalysisReport bean)
	{
	  String sqlAdvance =" SELECT  APPOINTREP_NOOFVAC_OPTION, APPOINTREP_NOOFVAC, APPOINTREP_DUE_OPTION,APPOINTREP_DUE, " 
	  		+" APPOINTREP_ISSUE_OPTION, APPOINTREP_ISSUE,  APPOINTREP_ACCP_OPTION, APPOINTREP_ACCP, APPOINTREP_REJ_OPTION," 
	  		+" APPOINTREP_REJECT, APPOINTREP_CANCEL_OPTION, APPOINTREP_CANCEL  FROM HRMS_REC_APPOINTREP_ADV WHERE APPOINTREP_CODE ="+bean.getSearchSetting();
	...
	public Object [][] checkDuplicate (AppointmentAnalysisReport bean)
	{
		String sql =" SELECT APPOINTREP_NAME,APPOINTREP_CODE FROM HRMS_REC_APPOINTREP_FILTERS "
				 +" WHERE UPPER(APPOINTREP_NAME) = '"+bean.getSettingName().toUpperCase()+"' AND APPOINTREP_USEREMPID= "+bean.getUserEmpId();
