 ...
 *@date 15 April 2013
 */
public class SchInterviewAnalysisModel extends ModelBase{ 
	...
	public void getReport(HttpServletRequest request,
			HttpServletResponse response,SchInterviewAnalysis bean,String[] colnames) { 
		 ...
		 */
					...
					if(bean.getReportType().equals("X")){
						 ...
						 title [0][0] = "";
						 title [0][1] = "";
						 title [0][2] = "Schedule Interview Analysis";  
				   ...
				   int cellwidth[]=new int [Integer.parseInt(bean.getCheckedCount())];  
					...
					if(bean.getSchDateChk().equals("on")){
					...
					if(bean.getInterviewerChk().equals("on")){
					...
					//Added by prajakta B
					if(bean.getInterviewStatusChk().equals("on")){//IF CHECKED
						actualCol[k]=colnames[11];
						cellwidth[k]=15;
						alignment[k]=0;
						k++;
					} //END IF CHECKED
					//End Added by prajakta B
						...
						if(!bean.getExportAll().equals("on")){ 
						    ...
						    toIndex=reqData.length;
							 ...
							 for (int p=fromIndex;p<toIndex;p++) {   
								 ...
								 int pageReqCellWidth[]={9,10,6,75};
							  ...
							  //  rg.tableBodyNoBorder(pageReqObj, pageReqCellWidth, pageReqAlign);   
								 ...
								 Object tab[][]=callCandidateQuery(bean, request);
									...
									for (int i = 0; i < tab.length; i++) {
										...
										//Added by prajakta B
										if(bean.getSchStatusChk().equals("on")){//if interview status checked
											if(tab[i][10] !=null){//if data
											...
											m++;
										} //end if interview status checked
										//End Added by prajakta B 
										m=1;
	...
	public void displayReq(SchInterviewAnalysis bean)
	{ 
		
		String sql=" SELECT DISTINCT NVL(REQS_NAME,' '),RANK_NAME,TO_CHAR(REQS_DATE ,'DD-MM-YYYY') AS REQS_DATE,DECODE(REQS_APPROVAL_STATUS,'P','Pending', "
				...
				+" LEFT JOIN HRMS_REC_CANDEVAL ON(HRMS_REC_CANDEVAL.EVAL_REQS_CODE=HRMS_REC_REQS_HDR.REQS_CODE) "
		 ...
		 //Added by Prajakta B
		 if(!bean.getIntDateFilter().equals("1"))
		 {//if InterviewDate Filter
			   if(bean.getIntDateFilter().equals("O"))
			   {//if InterviewDate on 
				   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE = TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY')" ; 
			   }//End if InterviewDate on
			   
			   if(bean.getIntDateFilter().equals("OB"))
			   {//if InterviewDate on Before
				   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE <= TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY')" ; 
			   }//end if InterviewDate on Before
			   
			   if(bean.getIntDateFilter().equals("OA"))
			   {//if InterviewDate on After
				   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE >= TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY')" ; 
			   }//End if InterviewDate on After
			   
			   if(bean.getIntDateFilter().equals("B"))
			   {//if Interview Date Before
				   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE < TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY')" ; 
			   }//end if interview Date Before
			   
			   if(bean.getIntDateFilter().equals("A"))
			   {//if Interview Date After
				   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE > TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY')" ; 
			   }//end if InterviewDate After
			   
			   if(bean.getIntDateFilter().equals("F"))
			   {//if Interview Date Between
				   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE >= TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY') ";
				   if (!(bean.getIntToDate().equals("") || bean.getIntToDate().equals("dd-mm-yyyy"))){
					   sql +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE <= TO_DATE('"+bean.getIntToDate()+"','DD-MM-YYYY') " ; 
				   }
			   }//end If InterviewDate Between
		 }//End if InterviewDate Filter
		 //Ends Added By Prajakta B 
		  ...
		  { 
			 ...
			 for (int i = 0; i < data.length; i++) {
				 SchInterviewAnalysis bean1 = new SchInterviewAnalysis();
	...
	public void callViewScreen(SchInterviewAnalysis bean,HttpServletRequest request)
	{ 
		...
		if(bean.getInterviewerChk().equals("on")){
			bean.setInterviewerChkFlag("true");
		}  
		
		if(bean.getInterviewStatusChk().equals("on")){
			bean.setInterviewStatusChkFlag("true");
		}  
		  ...
		  { 
			  bean.setDataLength(String.valueOf(data.length));
			   ...
			   for (int i=Integer.parseInt(pageIndex[0]);i<Integer.parseInt(pageIndex[1]);i++) {
				   bean.setVReqName(checkNull(String.valueOf(data[i][0])));
				   bean.setVReqDate(checkNull(String.valueOf(data[i][1])));
				   bean.setVBranch(checkNull(String.valueOf(data[i][2])));
				   bean.setVDepartment(checkNull(String.valueOf(data[i][3])));
				   bean.setVDivision(checkNull(String.valueOf(data[i][4])));
				   bean.setVPosition(checkNull(String.valueOf(data[i][5])));
				   bean.setVHiringMgr(checkNull(String.valueOf(data[i][6])));
				   bean.setVAppStatus(checkNull(String.valueOf(data[i][7])));
				   bean.setVReqStatus(checkNull(String.valueOf(data[i][8]))); 
				   bean.setVReqCode(checkNull(String.valueOf(data[i][9]))); 
	...
	public String callReqQuery(SchInterviewAnalysis bean)
	{
String query=" SELECT DISTINCT REQS_NAME, NVL(TO_CHAR(REQS_DATE, 'DD-MM-YYYY'),' ') AS REQ_DATE ,CENTER_NAME AS BRANCH," 
		...
		// ============  started  filter ===========
			if(!bean.getReqCode().equals("")){						
			 ...
			 {//if InterviewDate Filter
				   if(bean.getIntDateFilter().equals("O"))
				   {//if InterviewDate on 
					   query +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE = TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY')" ; 
				   }//End if InterviewDate on
				   ...
				   {//if Interview Date Between
					   query +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE >= TO_DATE('"+bean.getIntFrmDate()+"','DD-MM-YYYY') ";
					   if (!(bean.getIntToDate().equals("") || bean.getIntToDate().equals("dd-mm-yyyy"))){
						   query +=" AND HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE <= TO_DATE('"+bean.getIntToDate()+"','DD-MM-YYYY') " ; 
					   }
				   }//end If InterviewDate Between
			 }//End if InterviewDate Filter
			...
			// ============  end of filter ===========  
			  ...
			  String candiReqCode = callCanidateReq(bean); 
	...
	public Object [][] callCandidateQuery(SchInterviewAnalysis bean,HttpServletRequest request)
	{
		 if(bean.getScreenFlag().equals("false")){
			 callViewScreen(bean,request);  
		 } 
		
String query="SELECT  CAND_FIRST_NAME||' '||CAND_MID_NAME||' '||CAND_LAST_NAME AS CANDI_NAME, NVL(CAND_EMAIL_ID,' ')"  
+" AS EMAIL_ID, NVL(CAND_MOB_PHONE,'') AS CONCATnUMBER, HRMS_REC_ROUND_TYPE.ROUND_TYPE AS INT_ROUND_TYPE ,"
+" nvl(TO_CHAR(HRMS_REC_SCHINT_DTL.INT_DATE,'DD-MM-YYYY'),'')||' '|| INT_TIME, NVL(TO_CHAR(HRMS_REC_CANDEVAL.EVAL_CURRINT_DATE,'DD-MM-YYYY'),'')"
+" ||' '|| HRMS_REC_CANDEVAL.EVAL_CURRINT_TIME AS INT_TIME, NVL(INT_VENUE_DET,' ') AS VENUE ,EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME AS INTERVIEWER, "
+" RECTR.EMP_FNAME||' '||RECTR.EMP_MNAME||' '||RECTR.EMP_LNAME AS RECRUITER, "
+" DECODE(INT_CONDUCT_STATUS ,'Y','Conducted','N',' Scheduled','R','Rescheduled','C','Canceled') AS SCH_STATUS "
+" ,DECODE(HRMS_REC_CANDEVAL.EVAL_INT_STATUS,'S','Selected','R','Rejected','O','On Hold') FROM HRMS_REC_SCHINT_DTL " 
+" INNER JOIN HRMS_REC_REQS_HDR ON(HRMS_REC_REQS_HDR.REQS_CODE=HRMS_REC_SCHINT_DTL.INT_REQS_CODE)  "
+" LEFT JOIN HRMS_REC_SCHINT ON(HRMS_REC_SCHINT.INT_CODE=HRMS_REC_SCHINT_DTL.INT_CODE) "
...
+" LEFT JOIN HRMS_REC_CANDEVAL ON(HRMS_REC_CANDEVAL.EVAL_REQS_CODE=HRMS_REC_SCHINT_DTL.INT_REQS_CODE "
+" AND HRMS_REC_CANDEVAL.EVAL_CAND_CODE=HRMS_REC_SCHINT_DTL.INT_CAND_CODE AND HRMS_REC_CANDEVAL.EVAL_ROUND_TYPE=HRMS_REC_SCHINT_DTL.INT_ROUND_TYPE AND HRMS_REC_CANDEVAL.EVAL_INT_DTL_CODE=HRMS_REC_SCHINT_DTL.INT_DTL_CODE)"
+" WHERE HRMS_REC_SCHINT_DTL.INT_REQS_CODE="+bean.getVReqCode();
...
//Ends Added by prajakta B
				...
				{
					query+= " AND INT_CONDUCT_STATUS ='"+bean.getSchStatusCom()+"'";
				}
				...
				if(!bean.getInterviewerCode().equals(""))
				{
					query+= " AND INT_VIEWER_EMPID ="+bean.getInterviewerCode();
				}
	   // ============  start of sorting ===========
				
				 if (bean.getSortBy().equals("C")||bean.getSortBy().equals("T")||bean.getSortBy().equals("D")||bean.getSortBy().equals("I")||bean.getSortBy().equals("N")
				   || bean.getThenBy1().equals("C")||bean.getThenBy1().equals("T")||bean.getThenBy1().equals("D")||bean.getThenBy1().equals("I")||bean.getThenBy1().equals("N") 
				    || bean.getThenBy2().equals("C")||bean.getThenBy2().equals("T")||bean.getThenBy2().equals("D")||bean.getThenBy2().equals("I")||bean.getThenBy2().equals("N")
				 )
			 	  {
				      query+=" ORDER BY ";
				  } 
			     ...
			     {
				  query+= getSortVal(bean.getSortBy()) +" "+getSortOrder(bean.getSortByOrder());
				  if ( bean.getThenBy1().equals("C")||bean.getThenBy1().equals("T")||bean.getThenBy1().equals("D")||bean.getThenBy1().equals("I")||bean.getThenBy1().equals("N") 
						    || bean.getThenBy2().equals("C")||bean.getThenBy2().equals("T")||bean.getThenBy2().equals("D")||bean.getThenBy2().equals("I")||bean.getThenBy2().equals("N"))
				   {
					  query+=" , ";
				  }
			  ...
			  {
				  query+= getSortVal(bean.getThenBy1()) +" "+getSortOrder(bean.getThenByOrder1()); 
				  if(bean.getThenBy2().equals("C")||bean.getThenBy2().equals("T")||bean.getThenBy2().equals("D")||bean.getThenBy2().equals("I")||bean.getThenBy2().equals("N"))
				  {
					  query+=" , ";
				  }
			  }
			  
			  if(!(bean.getThenBy2().equals("1")  || bean.getThenBy2().equals("R")  ||bean.getThenBy2().equals("P")))
			  {
				  query+= getSortVal(bean.getThenBy2()) +" "+getSortOrder(bean.getThenByOrder2());
			  }
			// ============  end of sorting ===========   
			  ...
			  {   bean.setNoDataCandi("false");
				  bean.setDataLength(String.valueOf(data.length));
				   ...
				   for (int i=Integer.parseInt(pageIndex[0]);i<Integer.parseInt(pageIndex[1]);i++) { 
					   ...
					   bean1.setSchDate(checkNull(String.valueOf(data[i][4])));
					   bean1.setIvInterviewDate(checkNull(String.valueOf(data[i][5]))); 
					   bean1.setIvVenue(checkNull(String.valueOf(data[i][6]))); 
					   bean1.setIvInterviewver(checkNull(String.valueOf(data[i][7]))); 
					   bean1.setIvRecruiter(checkNull(String.valueOf(data[i][8])));
					   bean1.setIvSchStatus(checkNull(String.valueOf(data[i][9])));  
					   bean1.setInterviewStatus(checkNull(String.valueOf(data[i][10])));
						...
						if(bean.getInterviewerChk().equals("on")){
							bean1.setInterviewerChkFlag("true");
						}  
						
						if(bean.getInterviewStatusChk().equals("on")){
							bean1.setInterviewStatusChkFlag("true");
						}
	...
	{  String sql="";
		...
		if(Status.equals("I"))
		{
			sql=" UPPER(EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME) ";
		}
		if(Status.equals("N")){
	...
	public void callBack(SchInterviewAnalysis bean)
	{  
		...
		if(bean.getInterviewerChk().equals("on")){
			bean.setInterviewerChk("checked");  
		}
		...
		if(bean.getInterviewStatusChk().equals("on")){//if checked
			bean.setInterviewStatusChk("checked");
		} //end if checked
	...
	public void input(SchInterviewAnalysis bean)
	{ 
		    bean.setCandiCodeChk("checked"); 
		    bean.setEmailIdChk("checked"); 
			bean.setContactNumChk("checked");  
	...
	public void callPreviousRecord(SchInterviewAnalysis bean)
	{
	  String sql =" SELECT SCHDREP_CODE,SCHDREP_NAME FROM HRMS_REC_SCHDREP_FILTERS WHERE SCHDREP_USEREMPID ="+bean.getUserEmpId()+" ORDER BY UPPER(SCHDREP_NAME) ";
	  Object [][] data = getSqlModel().getSingleResult(sql);
		  if(data!=null && data.length>0)
	...
	public boolean callSave(SchInterviewAnalysis bean)
	{
	...
	public boolean saveFiletr(SchInterviewAnalysis bean)
	{    
		Object [][] filterObj = null;
		if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
			filterObj= new Object [1][15];  
		}else{
			filterObj= new Object [1][16];   
		}
		...
		filterObj [0][2]=bean.getPositionId().equals("")?null :bean.getPositionId();
	     ...
	     filterObj [0][9]=bean.getSelectedReq();
	     filterObj [0][10]=bean.getReportType().equals("1")?"1":bean.getReportType();  
	     filterObj [0][11]=bean.getRadioStatus();
	     filterObj [0][12]=bean.getIntDateFilter().equals("1")? null: bean.getIntFrmDate().equals("")? null : bean.getIntFrmDate();
	     filterObj [0][13]=(!bean.getIntDateFilter().equals("F"))? null:bean.getIntToDate().equals("")? null : bean.getIntToDate(); 
	     if(!bean.getIntDateFilter().equals("1"))
		 {
			   if(bean.getIntDateFilter().equals("O"))
			   {
	   ...
	   if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
		   ...
		   filterObj[0][15]=bean.getSearchSetting();
		  
		   	String Sql =" UPDATE HRMS_REC_SCHDREP_FILTERS SET  SCHDREP_REQ_CODE= "+filterObj[0][0]+", SCHDREP_HIRINGMGR_CODE="+filterObj[0][1]+", "
		 			+" SCHDREP_POSITION_CODE= "+filterObj[0][2]+", SCHDREP_DATEOPTION= '"+filterObj[0][3]+"', SCHDREP_SCHD_FROMDATE= TO_DATE('"+filterObj[0][4]+"','DD-MM-YYYY'), "
		 			+" SCHDREP_REPOPTION='"+filterObj[0][5]+"', SCHDREP_NAME= '"+filterObj[0][6]+"', SCHDREP_SCHD_TODATE=TO_DATE('"+filterObj[0][7]+"','DD-MM-YYYY'), SCHDREP_USEREMPID= "+filterObj[0][8]+", " 
		 			+" SCHDREP_SELECTED_REQ= '"+filterObj[0][9]+"',SCHDREP_REP_TYPE= '"+filterObj[0][10]+"',SCHDREP_RADIO_STATUS ='"+filterObj[0][11]+"',SCHDREP_INTFROMDATE= TO_DATE('"+filterObj[0][12]+"','DD-MM-YYYY'),"
		 			+" SCHDREP_INTTODATE=TO_DATE('"+filterObj[0][13]+"','DD-MM-YYYY'),SCHDREP_INTDATEOPTION= '"+filterObj[0][14]+"' WHERE SCHDREP_CODE ="+filterObj[0][15];
	...
	public boolean saveSort(SchInterviewAnalysis bean)
	{ 
		Object [][] sortObj =null;
		   if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
	...
	public boolean saveColumnDef(SchInterviewAnalysis bean)
	{   
		Object [][] colObj = null;
		  if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
			  colObj = new Object[1][11];
		  }else{
		...
		colObj[0][5]=bean.getSchDateChk().equals("on")?"Y":"N";
		colObj[0][6]=bean.getInterviewVenueChk().equals("on")?"Y":"N";
		colObj[0][7]=bean.getInterviewerChk().equals("on")?"Y":"N"; 
		colObj[0][8]=bean.getRecruiterChk().equals("on")?"Y":"N"; 
		colObj[0][9]=bean.getSchStatusChk().equals("on")?"Y":"N";
		colObj[0][10]= bean.getInterviewStatusChk().equals("on")?"Y":"N";
		boolean flag= false; 
		 if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
			 flag = getSqlModel().singleExecute(getQuery(3),colObj); 
		 }else{
			 colObj[0][11]=bean.getSearchSetting();
	...
	public boolean saveAdvance(SchInterviewAnalysis bean)
	{  
		Object [][] colAdv =null;
		 if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
		   colAdv = new Object[1][3];
		 }else{
			 colAdv = new Object[1][4];
		 }
		  
		colAdv[0][0]=bean.getSchStatusCom();
		colAdv[0][1]=(bean.getInterviewerCode().trim().equals("")||bean.getInterviewerCode().equals("null"))?null :bean.getInterviewerCode();
		colAdv[0][2]=(bean.getCandidateCode().trim().equals("")||bean.getCandidateCode().equals("null"))?null :bean.getCandidateCode();  
		boolean flag= false;
		 if(bean.getSearchSetting().equals("B")|| (! bean.getSettingName().trim().equals(bean.getSettingNameDup().trim()))){
			 flag= getSqlModel().singleExecute(getQuery(4),colAdv); 
		 }else{
			   colAdv[0][3]=bean.getSearchSetting();
			   System.out.println("in advnce update-------");
			  for (int i = 0; i < colAdv.length; i++) {
				System.out.println("colAdv============"+i+"==="+colAdv[0][i]);
			}
	...
	public boolean callUpdate(SchInterviewAnalysis bean)
	{
		if(saveFiletr(bean) && saveSort(bean) && saveColumnDef(bean) && saveAdvance(bean))
		{
			return true;
	 ...
	 * @param bean
	 */
	public void callFilterSavedData(SchInterviewAnalysis bean)
	{
 String sqlFilter ="  SELECT REQS_NAME, SCHDREP_REQ_CODE, NVL(EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME,' ') ," +
 		" SCHDREP_HIRINGMGR_CODE,  NVL(RANK_NAME,' '),SCHDREP_POSITION_CODE, SCHDREP_DATEOPTION, " +
 		" TO_CHAR(SCHDREP_SCHD_FROMDATE,'DD-MM-YYYY'), SCHDREP_REPOPTION, SCHDREP_NAME, TO_CHAR(SCHDREP_SCHD_TODATE,'DD-MM-YYYY'), "
		+"  SCHDREP_SELECTED_REQ, SCHDREP_REP_TYPE, SCHDREP_RADIO_STATUS,TO_CHAR(SCHDREP_INTFROMDATE,'DD-MM-YYYY'),"
		+" TO_CHAR(SCHDREP_INTTODATE,'DD-MM-YYYY'),SCHDREP_INTDATEOPTION FROM HRMS_REC_SCHDREP_FILTERS "
		+" LEFT JOIN HRMS_REC_REQS_HDR ON ( SCHDREP_REQ_CODE= REQS_CODE)"
		+" LEFT JOIN HRMS_RANK  ON ( SCHDREP_POSITION_CODE= RANK_ID )  "
		+" LEFT JOIN HRMS_EMP_OFFC  ON (SCHDREP_HIRINGMGR_CODE= EMP_ID )"
		+" WHERE SCHDREP_CODE ="+bean.getSearchSetting(); 
		...
		bean.setIntFrmDate(checkNull(String.valueOf(data[0][14])));
		bean.setIntToDate(checkNull(String.valueOf(data[0][15])));
		bean.setIntDateFilter(checkNull(String.valueOf(data[0][16])));
	} 
	 ...
	 * @param bean
	 */
	public void callSortSavedData(SchInterviewAnalysis bean)
	{
		String sqlSort ="  SELECT SORT_BY, SORT_BY_ORDER,SORT_THENBY, SORT_THENBY_ORDER, SORT_THENBY1,"
					+" SORT_THENBY1_ORDER FROM HRMS_REC_SCHDREP_SORT  WHERE SCHDREP_CODE ="+bean.getSearchSetting();
	 ...
	 if(sortData!=null && sortData.length>0){
		...
		System.out.println("sortData----------value------"+sortData[0][1]);
		if(String.valueOf(sortData[0][1]).trim().equals("A")){
			bean.setSortByAsc("checked");
			System.out.println("in if--------------------"+sortData[0][1]);
		}else{ 
			bean.setSortByDsc("checked");
		}
	...
	public void callColumnSavedData(SchInterviewAnalysis bean)
	{
	String sqlColumn ="SELECT COL_CANDI_NAME, COL_EMAILID, COL_CONTACT_NO, COL_ROUND_TYPE, COL_INTERVIEW_DATE, "
			+" COL_INTERVIEW_TIME, COL_INTERVIEW_VENUE, COL_INTERVIEWER,  COL_RECRUITER, COL_SCH_STATUS,COL_INTERVIEW_STATUS "
			+" FROM HRMS_REC_SCHDREP_COLDEF WHERE SCHDREP_CODE ="+bean.getSearchSetting();
	...
	public void callAdvanceSavedData(SchInterviewAnalysis bean)
	{
	  String sqlAdvance ="  SELECT  ADV_SCH_STATUS,NVL(CAND_FIRST_NAME||' '||CAND_MID_NAME||' '||CAND_LAST_NAME,''), "
						+" ADV_CANDI_CODE ,EMP_FNAME||' '||EMP_MNAME||' '||EMP_LNAME ,ADV_INTERVIEWER FROM HRMS_REC_SCHDREP_ADV "
						+" LEFT JOIN HRMS_REC_CAND_DATABANK ON (HRMS_REC_CAND_DATABANK.CAND_CODE=ADV_CANDI_CODE) "
						+" LEFT JOIN HRMS_EMP_OFFC ON (EMP_ID=ADV_INTERVIEWER ) "
						+" WHERE SCHDREP_CODE = "+bean.getSearchSetting();
	 	 ...
	 	 if(data!=null && data.length>0)
	 	 {
	 		 ...
	 		 bean.setCandidateCode(checkNull(String.valueOf(data[0][2]).trim())); 
	 		 bean.setInterviewerName(checkNull(String.valueOf(data[0][3])));
	 		 bean.setInterviewerCode(checkNull(String.valueOf(data[0][4]).trim()));  
	 	 }
	...
	public Object [][] checkDuplicate (SchInterviewAnalysis bean)
	{
		String sql =" SELECT  SCHDREP_NAME ,SCHDREP_CODE FROM HRMS_REC_SCHDREP_FILTERS WHERE UPPER(SCHDREP_NAME) ='"+bean.getSettingName().toUpperCase()+"' AND SCHDREP_USEREMPID="+bean.getUserEmpId();
	...
	public void viewSummary(SchInterviewAnalysis bean)
	{
  String sql =" SELECT REQS_NAME, SUM(SCHEDULE), SUM(RESCHEDULE), SUM(CONDUCTED), SUM(CANCELD) FROM ( "
			 +" SELECT NVL(REQS_NAME,'') AS REQS_NAME , "
			 +" CASE WHEN INT_CONDUCT_STATUS='N' THEN COUNT(INT_CONDUCT_STATUS) ELSE 0 END AS SCHEDULE, "
			 +" CASE WHEN INT_CONDUCT_STATUS='R' THEN COUNT(INT_CONDUCT_STATUS) ELSE 0 END AS RESCHEDULE, "
			 +" CASE WHEN INT_CONDUCT_STATUS='Y' THEN COUNT(INT_CONDUCT_STATUS) ELSE 0 END AS CONDUCTED,  "
			 +" CASE WHEN INT_CONDUCT_STATUS='C' THEN COUNT(INT_CONDUCT_STATUS) ELSE 0 END AS CANCELD  "
			 +" FROM HRMS_REC_REQS_HDR  "
			 +" LEFT JOIN HRMS_REC_SCHINT_DTL ON (HRMS_REC_REQS_HDR.REQS_CODE =HRMS_REC_SCHINT_DTL.INT_REQS_CODE) " 
			 +" INNER JOIN HRMS_EMP_OFFC HIRING_MGR ON (REQS_HIRING_MANAGER = HIRING_MGR.EMP_ID) "
			 +" LEFT JOIN HRMS_REC_CANDEVAL ON(HRMS_REC_CANDEVAL.EVAL_REQS_CODE=HRMS_REC_REQS_HDR.REQS_CODE AND HRMS_REC_CANDEVAL.EVAL_CAND_CODE=HRMS_REC_SCHINT_DTL.INT_CAND_CODE AND HRMS_REC_CANDEVAL.EVAL_ROUND_TYPE=HRMS_REC_SCHINT_DTL.INT_ROUND_TYPE AND HRMS_REC_CANDEVAL.EVAL_INT_DTL_CODE=HRMS_REC_SCHINT_DTL.INT_DTL_CODE)"
  	     ...
  	     sql +=" GROUP BY  INT_CONDUCT_STATUS,REQS_NAME "
	...
	public String callCanidateReq(SchInterviewAnalysis bean)
		{
			  String candiSql =" SELECT DISTINCT HRMS_REC_SCHINT_DTL.INT_REQS_CODE   FROM HRMS_REC_SCHINT_DTL  " 
					+" INNER JOIN HRMS_REC_CAND_DATABANK ON (HRMS_REC_SCHINT_DTL.INT_CAND_CODE=HRMS_REC_CAND_DATABANK.CAND_CODE)  "
					+" WHERE 1=1 ";
			  
				if(!bean.getSchStatusCom().equals("1"))
				{
					candiSql+= " AND INT_CONDUCT_STATUS ='"+bean.getSchStatusCom()+"'";
				}
				...
				if(!bean.getInterviewerCode().equals(""))
				{
					candiSql+= " AND INT_VIEWER_EMPID ="+bean.getInterviewerCode();
				}
				String candiReqCode ="";
				if(!bean.getSchStatusCom().equals("1")||!bean.getCandidateCode().equals("")||!bean.getInterviewerCode().equals("") )
				{ 
