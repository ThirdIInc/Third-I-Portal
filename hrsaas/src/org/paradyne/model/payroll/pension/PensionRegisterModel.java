 ...
 *
 */
public class PensionRegisterModel extends ModelBase {

	NumberFormat formatter = new DecimalFormat("#0.00");
	
	static org.apache.log4j.Logger logger=org.apache.log4j.Logger.getLogger(PensionRegisterModel.class); 
	
	public void getReport(PensionRegister penReg,HttpServletResponse response){
		...
		try {
			...
			if (ledgerData != null && ledgerData.length > 0){
				...
				//logger.info("penObject length=="+penObject.length);
				int [] width=(int [])pensionVect.get(2);
				//logger.info("width length=="+width.length);
	...
	public ArrayList getSalary(PensionRegister penReg,String ledgerCode){
		...
		try{
			...
			if (penReg.getCheckRetDate().equals("Y"))
				...
				empQuery += " WHERE PENS_LEDGER_CODE IN (" + ledgerCode + ") AND PENS_ONHOLD = 'N' ";
		...
		//	empQuery += "  UPPER(HRMS_EMP_OFFC.EMP_FNAME||' '||HRMS_EMP_OFFC.EMP_MNAME||' '||HRMS_EMP_OFFC.EMP_LNAME)";
			...
			if (empData != null && empData.length > 0) {
				 for (int i = 0; i < empData.length; i++) {
					 if(String.valueOf(empData[i][1]).startsWith("0") && penReg.getReport().equals("Xls"))
				...
				for (int i = 0; i < creditHead.length; i++) {
					if(i==0){
						...
						totalCreditQuery1 += "select ' ','Total  [' || count(*)|| '] ',sum(a"+i+".PENS_AMOUNT),";
          				...
          				+" INNER JOIN HRMS_PENSION_"+penReg.getYear()+" ON HRMS_EMP_OFFC.EMP_ID = HRMS_PENSION_"+penReg.getYear()+".PENS_EMP_ID AND PENS_ONHOLD = 'N' AND PENS_LEDGER_CODE in ("+ledgerCode+") ";
				...
				for (int e = 0; e < empData.length; e++) {
					...
					repData[e][3] = String.valueOf(empData[e][3]);
	...
	public ReportGenerator printAllReport(Object [][] dataObj,String [] colsName, int [] width,int [] alignment,int recPerPage,ReportGenerator rg,
			boolean brnCheck,boolean  deptCheck,boolean desgCheck,int startSum,String reportType,String date,PensionRegister penReg){
	...
	public ReportGenerator setReport(Object[][] salData, ReportGenerator rg,
			...
			int[] cellwidth, int[] alignmnet, int startSum,String reportType,String date,PensionRegister penReg ) {
		try {
			...
			for (int i = 0; i < salData.length; i++) {
				...
				if (i < salData.length 
					...
					if (reportObj != null && reportObj.length > 0) {
						...
						rg.addTextBold(printString, 0, 0, 0,11);
						rg.addText("\n", 0, 0, 0);
						//rg.tableBody(colsName, printObj, width, alignment);	
						...
						if(check==0){
							...
							if(reportType.equalsIgnoreCase("pdf"))
								rg.tableBodyBold(colNames, reportObj, cellwidth,alignmnet,11);
							else
								rg.xlsTableBody(colNames, reportObj, cellwidth,alignmnet);
						}
						else
							rg.tableBody(reportObj, cellwidth, alignmnet,11);
												
						rg.tableBody(braTotObj, cellwidth, alignmnet,11);
			...
			grndTot [0][1]="Grand Total";
			rg.addText("", 0, 1, 1);
			rg.tableBody(grndTot, cellwidth, alignmnet,11);
