 ...
 *
 */
public class IndustrialSalaryAcquaintanceRollModel extends ModelBase {  static org.apache.log4j.Logger logger=org.apache.log4j.Logger.getLogger(ModelBase.class); 
	
	
	// GET EMP_ID THOSE FULLFILL SORTING CRITERIA
	
	public Object[][] getEmpId(IndustrialSalary nonIndustrialSalary) {
		Object emp_id[][] = null;
		try {
			/*
	...
	public void generateReport(IndustrialSalary nonIndustrialSalary ,HttpServletResponse response) throws Exception {
		...
		org.paradyne.lib.report.ReportGenerator rg = new org.paradyne.lib.report.ReportGenerator(
			...
			count = count + 1;
			colNames[count] = "OT-PAY";
			count = count + 1;
			colNames[count] = "NET-SAL";
			...
			for (int i = 0; i < emp_id.length; i++) {
				...
				data[i][column] = total_nonRecovery;
				double total_pay = 0;
				...
				data[i][column] = String.valueOf(otAmount); //OT AMOUNT
				column = column + 1;
				data[i][column] = total_salary_pay+otAmount;
			...
			//rg.xlsTableBody(finalHeader, finalData, cellWidth, alignment,);
			rg.tableBody(finalHeader, finalData, cellWidth, alignment,rg,5,2,"PAY-BILL OF INDUSTRIAL STAFF OF ND(MUMBAI) FOR THE MONTH OF "+Utility.month(Integer.parseInt(month))+" "+year+"");
