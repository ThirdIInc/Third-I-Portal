...
public class BankStatementModel extends ModelBase {  static org.apache.log4j.Logger logger=org.apache.log4j.Logger.getLogger(ModelBase.class); 
	...
	public void generateReport(BankStatement bean,HttpServletResponse response){
		...
		String paybill=bean.getPayBill();
		Object para[][]=null;
		if(!(paybill.equals(""))){
		String query ="SELECT  PAYBILL_GROUP FROM HRMS_PAYBILL WHERE PAYBILL_ID ="+paybill;
		para=getSqlModel().getSingleResult(query);	
		}
		String SQL="SELECT  HRMS_EMP_OFFC.EMP_TOKEN, HRMS_TITLE.TITLE_NAME||' '||EMP_FNAME || ' ' || EMP_MNAME || ' ' || EMP_LNAME,"
		...
		try{
			...
			if(data.length>0){
				...
				for(int i=0;i<finalData.length-1;i++){
					...
					logger.info(""+String.valueOf(finalData[i][0]));
					try {
			...
			if(bank.equals("")) {
				rg.addText("Bank : ", 0,0,0);
			
			}else {
				rg.addText("Bank : "+bank, 0,0,0);
			} 
			if(paybill.equals("")) {
				rg.addText("Paybill Group :",0,0,0);
				
			}else {
				rg.addText("Paybill Group :"+String.valueOf(para[0][0]),0,0,0);
			}
