...
import org.paradyne.model.loan.NewLoansIssuedReportModel;
	...
	public class UmcLoanIssuedReportModel extends ModelBase {
		
		static org.apache.log4j.Logger logger = org.apache.log4j.Logger
		.getLogger(UmcLoanIssuedReportModel.class);
	...
	public boolean checkDuplicate(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		try {
			String query = " SELECT * FROM HRMS_MISREPORT_HDR WHERE UPPER(REPORT_CRITERIA) LIKE '"
					+ umcLoanReportBean.getSettingName().trim().toUpperCase()
	...
	public boolean deleteSavedReport(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		try {
			String deleteDetail = "DELETE FROM HRMS_MISREPORT_DTL WHERE REPORT_ID="
					+ umcLoanReportBean.getReportId();
	...
	public boolean saveFilters(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		int count = 0;

		try {

			if (!umcLoanReportBean.getDivCode().equals("")) {
				count++;
			}// division code
			if (!umcLoanReportBean.getDivCode().equals("")) {
				count++;
			}// division name
			if (!umcLoanReportBean.getDeptCode().equals("")) {
				count++;
			}// benefit for
			if (!umcLoanReportBean.getDeptCode().equals("")) {
				count++;
			} // extra work type
			if (!umcLoanReportBean.getBranchCode().equals("")) {
				count++;
			} // benefit type
			if (!umcLoanReportBean.getBranchCode().equals("")) {
				count++;
			}// employee code
			if (!umcLoanReportBean.getDesignationCode().equals("")) {
				count++;
			}// employee name
			if (!umcLoanReportBean.getDesignationCode().equals("")) {
				count++;
			}// employee token
			if (!umcLoanReportBean.getEmpCode().equals("")) {
				count++;
			}// approver code
			if (!umcLoanReportBean.getEmpCode().equals("")) {
				count++;
			...
			if (!umcLoanReportBean.getLoanType().equals("1")) {
				count++;
			}// approver token
			
			//ADDED BY REEBA - IS REFUNDABLE
			if (!umcLoanReportBean.getRefundable().equals("1")) {
				count++;
			}
			...
			if (!umcLoanReportBean.getDivCode().equals("")) {
				...
				addObj[int_count][1] = checkNull(umcLoanReportBean.getDivCode().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getDivName().equals("")) {
				addObj[int_count][0] = "divName";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getDivName().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getDeptCode().equals("")) {
				addObj[int_count][0] = "deptCode";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getDeptCode().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getDeptName().equals("")) {
				addObj[int_count][0] = "deptName";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getDeptName().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getBranchCode().equals("")) {
				addObj[int_count][0] = "branchCode";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getBranchCode()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getBranchName().equals("")) {
				addObj[int_count][0] = "branchName";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getBranchName()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getDesignationCode().equals("")) {
				addObj[int_count][0] = "designationCode";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getDesignationCode()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getDesignationName().equals("")) {
				addObj[int_count][0] = "designationName";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getDesignationName()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getEmpCode().equals("")) {
				addObj[int_count][0] = "empCode";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getEmpCode().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getEmpName().equals("")) {
				addObj[int_count][0] = "empName";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getEmpName().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getEmpToken().equals("")) {
				addObj[int_count][0] = "empToken";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getEmpToken().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getLoanType().equals("1")) {
				addObj[int_count][0] = "loanType";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getLoanType().trim());
				int_count++;
			}
			//ADDED BY REEBA - IS REFUNDABLE
			if (!umcLoanReportBean.getRefundable().equals("1")) {
				addObj[int_count][0] = "refundable";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getRefundable().trim());
				int_count++;
			}
	...
	public boolean saveColumns(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		int count = 0;
		try {
			if (umcLoanReportBean.getEmpNameFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getDivFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getLoanAmountFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getLoanTypeFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getLoanPaymentDateFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getLoanInstallmentFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getNoOfInstallFlag().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getLoanAmountPaidChkBox().equals("true")) {
				count++;
			}
			if (umcLoanReportBean.getLoanPendingAmtChkBox().equals("true")) {
				count++;
			}
			...
			if (umcLoanReportBean.getDivFlag().equals("true")) {
				addObj[int_count][0] = "divFlag";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getLoanAmountFlag().equals("true")) {
				addObj[int_count][0] = "loanAmountFlag";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getLoanTypeFlag().equals("true")) {
				addObj[int_count][0] = "loanTypeFlag";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getLoanPaymentDateFlag().equals("true")) {
				addObj[int_count][0] = "loanPaymentDateFlag";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getLoanInstallmentFlag().equals("true")) {
				addObj[int_count][0] = "loanInstallmentFlag";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getNoOfInstallFlag().equals("true")) {
				addObj[int_count][0] = "noOfInstallFlag";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getLoanAmountPaidChkBox().equals("true")) {
				addObj[int_count][0] = "loanAmountPaidChkBox";
				addObj[int_count][1] = "Y";
				int_count++;
			}
			if (umcLoanReportBean.getLoanPendingAmtChkBox().equals("true")) {
				addObj[int_count][0] = "loanPendingAmtChkBox";
				addObj[int_count][1] = "Y";
				int_count++;
			}
	...
	public boolean saveSortOptions(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		int count = 0;

		try {
			if (!umcLoanReportBean.getSortBy().equals("1")) {
				count++;
			}
			if (!umcLoanReportBean.getHiddenSortBy().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getSortByAsc().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getSortByDsc().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getSortByOrder().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenBy1().equals("1")) {
				count++;
			}
			if (!umcLoanReportBean.getHiddenThenBy1().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenByOrder1Asc().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenByOrder1Dsc().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenByOrder1().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenBy2().equals("1")) {
				count++;
			}
			if (!umcLoanReportBean.getHiddenThenBy2().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenByOrder2Asc().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenByOrder2Dsc().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getThenByOrder2().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getHiddenColumns().equals("")) {
				count++;
			}
			...
			if (!umcLoanReportBean.getSortBy().equals("1")) {
				...
				addObj[int_count][1] = checkNull(umcLoanReportBean.getSortBy().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getHiddenSortBy().equals("")) {
				addObj[int_count][0] = "hiddenSortBy";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getHiddenSortBy()
						.trim());
				int_count++;
			}
			logger.info("getSortByAsc...." + umcLoanReportBean.getSortByAsc());
			if (!umcLoanReportBean.getSortByAsc().equals("")) {
				addObj[int_count][0] = "sortByAsc";
				addObj[int_count][1] = "Y";// checkNull(umcLoanReportBean.getSortByAsc().trim());
				int_count++;
			}
			logger.info("getSortByDsc...." + umcLoanReportBean.getSortByDsc());
			if (!umcLoanReportBean.getSortByDsc().equals("")) {
				addObj[int_count][0] = "sortByDsc";
				addObj[int_count][1] = "Y";// checkNull(umcLoanReportBean.getSortByDsc().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getSortByOrder().equals("")) {
				addObj[int_count][0] = "sortByOrder";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getSortByOrder()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenBy1().equals("1")) {
				addObj[int_count][0] = "thenBy1";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getThenBy1().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getHiddenThenBy1().equals("")) {
				addObj[int_count][0] = "hiddenThenBy1";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getHiddenThenBy1()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenByOrder1Asc().equals("")) {
				addObj[int_count][0] = "thenByOrder1Asc";
				addObj[int_count][1] = "Y";// checkNull(umcLoanReportBean.getThenByOrder1Asc().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenByOrder1Dsc().equals("")) {
				addObj[int_count][0] = "thenByOrder1Dsc";
				addObj[int_count][1] = "Y";// checkNull(umcLoanReportBean.getThenByOrder1Dsc().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenByOrder1().equals("")) {
				addObj[int_count][0] = "thenByOrder1";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getThenByOrder1()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenBy2().equals("1")) {
				addObj[int_count][0] = "thenBy2";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getThenBy2().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getHiddenThenBy2().equals("")) {
				addObj[int_count][0] = "hiddenThenBy2";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getHiddenThenBy2()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenByOrder2Asc().equals("")) {
				addObj[int_count][0] = "thenByOrder2Asc";
				addObj[int_count][1] = "Y";// checkNull(umcLoanReportBean.getThenByOrder2Asc().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenByOrder2Dsc().equals("")) {
				addObj[int_count][0] = "thenByOrder2Dsc";
				addObj[int_count][1] = "Y";// checkNull(umcLoanReportBean.getThenByOrder2Dsc().trim());
				int_count++;
			}
			if (!umcLoanReportBean.getThenByOrder2().equals("")) {
				addObj[int_count][0] = "thenByOrder2";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getThenByOrder2()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getHiddenColumns().equals("")) {
				addObj[int_count][0] = "hiddenColumns";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getHiddenColumns()
						.trim());
				int_count++;
			}
			logger.info("int_count     : " + int_count);
	...
	public boolean saveAdvancedFilters(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		int count = 0;

		try {
			if (!umcLoanReportBean.getLoanDateSelect().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getLoanFromDate().equals("")) {
				count++;
			}
			if (!umcLoanReportBean.getLoanToDate().equals("")) {
				count++;
			}
			...
			if (!umcLoanReportBean.getLoanDateSelect().equals("")) {
				...
				addObj[int_count][1] = checkNull(umcLoanReportBean.getLoanDateSelect()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getLoanFromDate().equals("")) {
				addObj[int_count][0] = "umcLoanReportBean.loanFromDate";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getLoanFromDate()
						.trim());
				int_count++;
			}
			if (!umcLoanReportBean.getLoanToDate().equals("")) {
				addObj[int_count][0] = "umcLoanReportBean.loanToDate";
				addObj[int_count][1] = checkNull(umcLoanReportBean.getLoanToDate()
						.trim());
				int_count++;
			}
	...
	public boolean saveReportCriteria(UmcLoanIssuedReport umcLoanReportBean) {
		boolean result = false;
		try {
			if (!checkDuplicate(umcLoanReportBean)) {
				Object[][] saveObj = new Object[1][3];
				saveObj[0][0] = checkNull(umcLoanReportBean.getSettingName().trim());
				saveObj[0][1] = checkNull(umcLoanReportBean.getReportTitle().trim());
				saveObj[0][2] = "Loan";
				...
				if (result) {
					...
					Object[][] codeObj = getSqlModel().getSingleResult(
							codeQuery);
					umcLoanReportBean.setReportId(String.valueOf(codeObj[0][0]));
					umcLoanReportBean.setReportTypeStr("Loan");
					if (saveFilters(umcLoanReportBean) && saveColumns(umcLoanReportBean)
							&& saveSortOptions(umcLoanReportBean)
							&& saveAdvancedFilters(umcLoanReportBean)) {
						result = true;
					} else
						result = false;
				}
	...
	public void setDetailOnScreen(UmcLoanIssuedReport umcLoanReportBean) {
	...
	if (dtlObj != null && dtlObj.length > 0) {
		try {
			...
			for (int i = 0; i < dtlObj.length; i++) {
				...
				Method modelMethod = UmcLoanIssuedReport.class
				...
				modelMethod.invoke(umcLoanReportBean, String.valueOf(dtlObj[i][2]));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private String selectQuery(UmcLoanIssuedReport bean, String[] labelNames,
			...
			if (splitComma != null) {
				...
				for (int i = 0; i < splitComma.length; i++) {
					...
					if (splitDash[1].trim().equals(labelNames[0].trim())) {
						...
						columnMap
								.put(Integer.parseInt(splitDash[0]),
										"TO_CHAR(A.LOAN_AMOUNT, '99,999,999.99') AS LOAN_AMOUNT ");
						...
						columnMap
								.put(Integer.parseInt(splitDash[0]),
										" TO_CHAR(A.LOAN_DATE,'DD-MON-YYYY') AS LOAN_PAYMENT_DATE ");
						...
						columnMap
								.put(
										Integer.parseInt(splitDash[0]),
										" EMI_AMOUNT as MONTHLY_INSTALLMENT ");
						...
						columnMap
								.put(Integer.parseInt(splitDash[0]),
										" C.LOAN_AMOUNT-PENDING_LOAN_AMT AS AMOUNT_PAID ");
	...
	private String conditionQuery(UmcLoanIssuedReport bean, String[] labelNames) {

		String query = "";

		try {
			query = " FROM HRMS_LOAN_SUPPL_APPL "
					+ " A INNER JOIN HRMS_EMP_OFFC B ON (A.EMP_ID = B.EMP_ID)"
					+ " INNER JOIN HRMS_LOAN_NEW_APPL C ON (A.EMP_ID = C.EMP_ID) ";
			...
			if (!bean.getRefundable().equals("1")) {
				query += " AND A.LOAN_IS_REFUNDABLE = '" + bean.getRefundable()+"'";
			}

			if (!bean.getLoanDateSelect().trim().equals("")) {
				if (bean.getLoanDateSelect().trim().equals("ON")) {
					query += " AND A.LOAN_DATE = TO_DATE('"
	...
	public String getSortVal(String Status, String[] labelNames) {
		...
		if (Status.equals(labelNames[0])) {
			...
			sql = " C.PENDING_LOAN_AMT ";

		} else if (Status.equals(labelNames[4])) {
			sql = " TO_CHAR(A.LOAN_DATE,'DD-MON-YYYY') ";
			...
			sql = " LOAN_AMOUNT-PENDING_LOAN_AMT ";
	...
	public String getReport(UmcLoanIssuedReport bean,
		...
		try {
			...
			if (!bean.getReportTitle().equals(""))
				...
				reportName = "Loans Issued MIS Report";
			...
			if (finalObj != null && finalObj.length > 0) {
				...
				for (int i = 0; i < finalObj.length; ++i) {
					...
					if (bean.getLoanPendingAmtChkBox().equals("true")) {
						objData1[i][++int_count] = checkNull(String
								.valueOf(finalObj[i][int_count]));
					}
					...
					if (bean.getLoanAmountPaidChkBox().equals("true")) {
						objData1[i][++int_count] = checkNull(String
								.valueOf(finalObj[i][int_count]));//  
					}
				}
	...
	public void callViewScreen(UmcLoanIssuedReport bean,
		...
		try {
			...
			if (finalObj != null && finalObj.length > 0) {
				...
				bean.setDataLength(String.valueOf(objData1.length));
				...
				if (finalObj != null && finalObj.length > 0) {
					...
					String[] pageIndex = Utility.doPaging(
							bean.getMyPage(), objData1.length, 20);
					if (pageIndex == null) {
						pageIndex[0] = "0";
						pageIndex[1] = "20";
						pageIndex[2] = "1";
