...
import org.paradyne.bean.payroll.payroll_UMC.SalaryProcessUMC;
 ...
 * *  Date   : May 11, 2010
 * @modified by : AA0554 Mangesh Jadhav
 *  Date   : June 25, 2010
 *
 */
public class SalaryProcessModelUMC extends ModelBase {
	 ...
	 * @return status
	 */	
	public String checkSalaryProcessStatus(SalaryProcessUMC bean, String[] listOfFilters) {
		String status="";
		try {
	...
	public HashMap<String, Object[][]> getEmpCreditMap(String[] listOfFilters){
		...
		try {
			String creditQuery = " SELECT HRMS_EMP_OFFC.EMP_ID,HRMS_CREDIT_HEAD.CREDIT_CODE, NVL(CREDIT_AMT,0),NVL(CREDIT_APPLICABLE_ESI,'N'),NVL(CREDIT_APPLICABLE_PTAX,'N'), "  
								+" NVL(CREDIT_TYPE,1) FROM HRMS_CREDIT_HEAD "  
	...
	public HashMap<String, Object[][]> getEmpDebitMap(String[] listOfFilters){
				...
				if(empObj != null && empObj.length > 0) {
					...
					Object [][] creditHeadObj = getCreditCodes();
						if (creditHeadObj != null && creditHeadObj.length > 0) {
							...
							empCreditConfigMap = getEmpCreditMap(listOfFilters);
							//if(empCreditConfigMap != null && empCreditConfigMap.size() > 0) {
								...
								for (int i = 0; i < empObj.length; i++) {
									...
									Object [][] tempCreditObj = makeEmpObjSameSize(empCreditConfigMap.get(String.valueOf(empObj[i][0])), creditHeadObj,String.valueOf(empObj[i][0]));
									//not used now...
								...
								creditInsertObj = createInsertObj(empMonthCreditMap, empObj, creditHeadObj);		
							//}//
						}
					//get the debitCodes from creditHead master with periodicity=monthly and payflag=yes
						...
						if (debitHeadObj != null && debitHeadObj.length > 0) {
							...
							empDebitConfigMap = getEmpDebitMap(listOfFilters);
								...
								for (int i = 0; i < empObj.length; i++) {
									...
									ArrayList dataList = getMonthlyEmpDebit(tempDebitObj, tempMonthCreditObj, empData, tempGrossCreditObj, 
																	pf_configData, pfTrust_configData, vpf_configData, esi_configData, 
																		lwfCodeList, lwfMap,incomeTax_configData,empLoanObj,empLicObj,generalData);
	...
	public boolean saveSalary(Object [][] creditObject,Object [][] debitObject,Object [][] empObj,HashMap empTotalMap,String [] dataString,String[] listOfFilters ) {
		...
		try {
			...
			String insertSalCreditQuery = "INSERT INTO HRMS_SAL_CREDITS_"+ year+" (EMP_ID , SAL_CREDIT_CODE ,SAL_AMOUNT,SAL_LEDGER_CODE) VALUES(?,?,?,'"+ledgerCode+"')";
	...
	public boolean lockSalary(String ledgerCode,SalaryProcessUMC bean) {
	...
	public boolean unlockSalary(SalaryProcessUMC bean) {
		boolean result =false;
	...
	public Object[][] createInsertObj(HashMap<String,Object[][]> creditMap,Object [][] empIdObj,Object [][] creditHead) {
	...
	public Object [][] getEmployeeDataToView(String [] dataString,HttpServletRequest request,SalaryProcessUMC bean,Object [][] attnEmpIdSaved,String []listOfFilters){
		...
		try {
			...
			ArrayList<SalaryProcessUMC> creditNames = new ArrayList<SalaryProcessUMC>();
			ArrayList<SalaryProcessUMC> debitNames = new ArrayList<SalaryProcessUMC>();
			for (int i = 0; i < creditHead.length; i++) {
				SalaryProcessUMC creditName = new SalaryProcessUMC();
			...
			for (int i = 0; i < debitHead.length; i++) {
				SalaryProcessUMC debitName = new SalaryProcessUMC();
	...
	public void getSalProcessList(SalaryProcessUMC salProcess,	HttpServletRequest request) {
			...
			String listQuery = " SELECT DECODE(LEDGER_MONTH, 1, 'January', 2, 'February', 3, 'March', 4, 'April', 5, 'May',  6, 'June', 7, 'July', 8, 'August', 9, 'September', 10, 'October', 11, 'November', 12, 'December') ATTN_MONTH, " 
						...
						if(listObj != null && listObj.length > 0) {
							for(int i=Integer.parseInt(pageIndex[0]);i<Integer.parseInt(pageIndex[1]);i++){
								 ...
								 SalaryProcessUMC  bean1 = new SalaryProcessUMC();
	 ...
	 */
	public void doPaging(SalaryProcessUMC bean, int empLength, Object[][] empObj, 
	...
	public Object[][] getLedgerCode(SalaryProcessUMC bean,String ledgerStatus) {
	...
	public Object[][] recalSalary(HttpServletRequest request,String[] recal_emp,
			String ledgerStatus,String path,SalaryProcessUMC bean,String []dataString,String[] listOfFilters ) {
		...
		try {
			...
			for (int i = 0; i < recalEmpObj.length; i++) {
				...
				try {
					...
					Object [][] tempCreditObj = makeEmpObjSameSize(empCreditConfigMap.get(String.valueOf(recalEmpObj[i][0])), creditHeadObj,String.valueOf(recalEmpObj[i][0]));
					//not used now...
					//Object [][] tempCreditObj = empCreditConfigMap.get(String.valueOf(empObj[i][0]));
					...
					Object [][] tempDebitObj = makeEmpObjSameSizeDebit( empDebitConfigMap.get(String.valueOf(recalEmpObj[i][0])), debitHeadObj,String.valueOf(recalEmpObj[i][0]));
	...
	public void downloadTemplate(SalaryProcessUMC bean, HttpServletResponse response) {
	...
	public String updCredits(SalaryProcessUMC salProcess, Object[][] empObjFrmFile,
			String path) {
		...
		try{
			...
			try {
				String insertCreditQuery = "INSERT INTO  HRMS_SAL_CREDITS_"
					+ salProcess.getYear()
					+ " (SAL_AMOUNT,EMP_ID,SAL_LEDGER_CODE,SAL_CREDIT_CODE) VALUES(?,?,"+ledgerCode+",?) ";
...
public double getPTAXAmount(Object[][] credit_amount ,String path,
			...
			String typeId, String branchId, String location, SalaryProcessUMC salProcess) {
...
public String updDebits(SalaryProcessUMC salProcess,Object[][] add,String debitCode){
	try {
		String empQuery ="select emp_id,emp_token from hrms_emp_offc ";
		ArrayList addList =new ArrayList();
		Object[][] empObj = getSqlModel().getSingleResult(empQuery);
...
public void setOtherIncomeFlags(SalaryProcessUMC bean){
...
public String pullExtraWorkBenefit(SalaryProcessUMC bean,String path,String []listOfFilters){
	String result = "0";
	String filterQuery = setEmpFiletrs(listOfFilters);
	try {
...
public String pullLeaveEncashment(SalaryProcessUMC bean,String path,String []listOfFilters){
	String result = "0";
	String filterQuery = setEmpFiletrs(listOfFilters);
	try{
...
public String pullAllowance(SalaryProcessUMC bean,String path,String []listOfFilters){
	String result = "0";
	String filterQuery = setEmpFiletrs(listOfFilters);
	try{
...
public void addRecovery(SalaryProcessUMC bean,String[] listOfFilters){
