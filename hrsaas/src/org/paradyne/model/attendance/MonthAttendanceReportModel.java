...
public class MonthAttendanceReportModel extends ModelBase {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(MonthAttendanceReportModel.class);
	 ...
	 * @param response
	 */
	private org.paradyne.lib.ireportV2.ReportGenerator annualReport(org.paradyne.lib.ireportV2.ReportGenerator rg, MonthAttendanceReport bean, HttpServletResponse response) {
		...
		try {
			...
			String filters = "Month : " + Utility.month(currMonth) + "             Year : " + currYear;

			if(!divName.equals("")) {
				
				filters += "\n\nDivision Name : "+bean.getDivName();
			}
			if(!typeName.equals("")) {
				filters += "\n\nEmployee Type : "+bean.getTypeName();
			}
			if(!payBillName.equals("")) {
				filters += "\n\nPayBill Name : "+bean.getPayBillName();
			}
			if(!brnName.equals("")) {
				filters += "\n\nBranch Name : "+bean.getBrnName();
			}
			if(!deptName.equals("")) {
				filters += "\n\nDepartment Name : "+bean.getDeptName();
			}
		...
		//	filterData.setBodyFontDetails(Font.FontFamily.HELVETICA, 6, Font.NORMAL, new BaseColor(0, 0, 0));
			...
			summaryData[0][0] = "Annual Attendance Report :" ;
		...
		//	tableheadingDateData.setBodyFontDetails(Font.FontFamily.HELVETICA, 6, Font.NORMAL, new BaseColor(0, 0, 0));
			...
			if(annualAttendance != null && annualAttendance.length != 0) {
				...
				tdstable.setData(annualAttendance);
				tdstable.setCellAlignment(cellAln);
			...
			//	rg.addTextBold("There is no data to display.", 0, 1, 0);
				...
				noDataObj[0][0] = "Annual Attendance Report : No records available";
	...
	private org.paradyne.lib.ireportV2.ReportGenerator getMonthAttendanceReport(org.paradyne.lib.ireportV2.ReportGenerator rg, MonthAttendanceReport bean, HttpServletResponse response) {
		...
		try {
			...
			try {
				...
				" ATTN_DAYS, ATTN_WOFF, ATTN_HOLIDAY, ATTN_LATE_MARKS, ATTN_HALF_DAYS, ATTN_PAID_LEVS, ATTN_UNPAID_LEVS, ATTN_RECOVERY_DAYS,ATTN_SAL_DAYS  " +
				" , NVL(DEPT_NAME,' ') " +
				...
				" ATT_LEAVE_BALANCE FROM HRMS_MONTH_ATT_DTL_" + year +
			...
			/* Setting filter details */
			String filters = "Month : " + Utility.month(currMonth) + "             Year : " + year;
		...
		//	filterData.setBodyFontDetails(Font.FontFamily.HELVETICA, 6, Font.NORMAL, new BaseColor(0, 0, 0));
			...
			Object[][] summaryData = new Object[1][1];
			summaryData[0][0] = "Month Attendance Report :" ;
			int[] cellWidthDateHeader = { 100 };
			int[] cellAlignDateHeader = { 0 };
		...
		//	tableheadingDateData.setBodyFontDetails(Font.FontFamily.HELVETICA, 6, Font.NORMAL, new BaseColor(0, 0, 0));
			...
			String[] attendanceCols = {"Employee Id", "Employee Name", "Branch Name","Department Name", "Attendance Days", "Weekly Offs", "Holidays", 
					"Late Marks", "Half Days", "Paid Leaves", "Unpaid Leaves","Recovery Days", "Salary Days"};

			String[] allCols = new String[attendanceCols.length + (leaveTypes.length * 3)];
			...
			if(attendance != null && attendance.length > 0) {
				...
				for(int i = 0; i < attendance.length; i++) {
					...
					finalObject[i][0] = checkNull(String.valueOf(attendance[i][1])); // EMPLOYEE ID
					finalObject[i][1] = checkNull(String.valueOf(attendance[i][2])); // EMPLOYEE NAME
					finalObject[i][2] = checkNull(String.valueOf(attendance[i][3])); // BRANCH NAME
					
					finalObject[i][3] = checkNull(String.valueOf(attendance[i][12]));  //Department Name
					
					finalObject[i][4] = checkNull(String.valueOf(attendance[i][4])); // ATTENDANCE DAYS
					finalObject[i][5] = checkNull(String.valueOf(attendance[i][5])); // WEEKLY OFFS
					finalObject[i][6] = checkNull(String.valueOf(attendance[i][6])); // HOLIDAYS
					finalObject[i][7] = checkNull(String.valueOf(attendance[i][7])); // LATE MARKS
					finalObject[i][8] = checkNull(String.valueOf(attendance[i][8])); // HALF DAYS
					finalObject[i][9] = checkNull(String.valueOf(attendance[i][9])); // PAID LEAVES
					finalObject[i][10] = checkNull(String.valueOf(attendance[i][10])); // UNPAID LEAVES
					finalObject[i][11] = checkNull(String.valueOf(attendance[i][11])); // RECOVERY DAYS
					finalObject[i][12] = checkNull(String.valueOf(attendance[i][12])); // SAL DAYS
			...
			if(finalObject != null && finalObject.length > 0) {
				...
				TableDataSet noData = new TableDataSet();
				Object[][] noDataObj = new Object[1][1];
				noDataObj[0][0] = "Month Attendance Report : No records available";
	...
	public void getReport(MonthAttendanceReport bean,
		...
		try {
			...
			Date date = new Date();
			String attType = bean.getAttdnType();
			String type = bean.getReportType();
			System.out.println("type==="+type);
			rds.setReportType(type);
			String fileName = "";
			if(attType.equals("M")) {
				fileName = "Monthly_Attendance_Report_"+Utility.getRandomNumber(1000);
			}else{
				fileName = "Annualy_Attendance_Report_"+Utility.getRandomNumber(1000);
			}
			
			String reportPathName=reportPath+fileName+"."+type;
			rds.setFileName(fileName);
			if(attType.equals("M")) {
				rds.setReportName("Monthly Attendance Report");
			}else{
				rds.setReportName("Annualy Attendance Report");
			}
			...
			if(reportPath.equals("")){
				...
				request.setAttribute("action", "MonthAttendanceReport_input.action");
			}
			
			if(attType.equals("M")) {
				
				rg = getMonthAttendanceReport(rg,bean, response);// Generates a report for a given month and year
			}
			if(attType.equals("A")) {
				
				rg = annualReport(rg,bean, response);// Generates a report for 12 months starting from a given month and year
			} // end of else statement
