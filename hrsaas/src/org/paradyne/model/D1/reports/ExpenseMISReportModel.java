...
public class ExpenseMISReportModel extends ModelBase {
	...
	public boolean deleteSavedReport(ExpenseMISReport misreport) {
	...
	public boolean checkDuplicate(ExpenseMISReport misreport) {
		boolean result = false;
		String query = "SELECT * FROM HRMS_MISREPORT_HDR WHERE REPORT_TYPE LIKE 'ExpenseMIS' AND UPPER(REPORT_CRITERIA) LIKE '"
	...
	public boolean saveReportCriteria(ExpenseMISReport misreport) {
		// TODO Auto-generated method stub
		boolean result = false;
	...
	public boolean saveColumns(ExpenseMISReport misreport){
		boolean result = false;
		int count = 0;
		//Name
		...
		//Status
		if(misreport.getTrackingNo().equals("true")){
			count++;
		}	
		...
		//Status
		if(misreport.getTrackingNo().equals("true")){
			addObj[int_count][0] = "misreport.trackingNoFlag";
			addObj[int_count][1] = "Y";
			int_count++;
		}
	...
	public boolean saveFilters(ExpenseMISReport misreport){
		boolean result = false;
		int count = 0;
		//Division
	...
	public boolean saveSortOptions(ExpenseMISReport misreport){
		boolean result = false;
		int count = 0;
	...
	public boolean saveAdvancedFilters(ExpenseMISReport misreport){
	...
	public void setDetailOnScreen(ExpenseMISReport misreport) {
		// TODO Auto-generated method stub
		...
		if(dtlObj!= null && dtlObj.length > 0){
			try {
				for(int i=0; i< dtlObj.length; i++){
					...
					Method modelMethod = ExpenseMISReport.class.getDeclaredMethod(
	...
	public void callViewScreen(ExpenseMISReport misreport,
	...
	public String selectQuery(ExpenseMISReport misreport, String[] labelNames,
		...
		if(splitComma!=null){
			...
			for(int i=0;i<splitComma.length;i++){
				...
				if(splitDash[1].equals(labelNames[0].trim())){				//NAME
				...
				else if(splitDash[1].equals(labelNames[7].trim())){			//TRACKING NO
					columnMap.put(Integer.parseInt(splitDash[0])," NVL(FILE_HEADER_NAME,' ')");
					labelMap.put(8, labelNames[7]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[8].trim())){			//STATUS
					columnMap.put(Integer.parseInt(splitDash[0]),
							"NVL(DECODE(STATUS,'D','Draft','F','Forwarded','P','Pending','Z','Authorized SignOff','A','Approved','R','Rejected','B','Sent Back'),' ')");
					labelMap.put(9, labelNames[8]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[9].trim())){			//MANAGER
					...
					columnMap.put(Integer.parseInt(splitDash[0])," NVL(EXPENSE_BUSINESS_PUR,' ') ");
					labelMap.put(11, labelNames[10]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[11].trim())){			//LATE HRS DEDUCTED FROM
					columnMap.put(Integer.parseInt(splitDash[0])," NVL(TOTAL_EXPENSE_AMOUNT,0)");
	...
	public String conditionQuery(ExpenseMISReport bean,
			String[] labelNames) {
		// TODO Auto-generated method stub
		String query="";
		try{
			query ="FROM HRMS_D1_EXPENSE_APPR_REQ "
				+ "   "
				...
				+ " LEFT JOIN HRMS_EMP_OFFC MNG ON (MNG.EMP_ID = HRMS_D1_EXPENSE_APPR_REQ.EXPENSE_APPR_CODE)  "
				+ " WHERE 1 = 1 ";
			//DIVISION
			...
			if(!(bean.getStatus().equals("1")) && !(bean.getStatus() == null) && !(bean.getStatus().equals("null")) ){
				query += " AND HRMS_D1_EXPENSE_APPR_REQ.STATUS = '"+bean.getStatus().trim()+"'";
			}
			//EMPLOYEE
			...
			//TRACKING NO
			if(!(bean.getTrackingNo().equals("")) && !(bean.getTrackingNo() == null) && !(bean.getTrackingNo().equals("null")) ){
				query += " AND HRMS_D1_EXPENSE_APPR_REQ.FILE_HEADER_NAME = '"+bean.getTrackingNo().trim()+"'";
			}
			...
			if (!bean.getAppliDateSelect().trim().equals("")) {
				if (bean.getAppliDateSelect().trim().equals("ON")) {
					query += " AND TO_CHAR(HRMS_D1_EXPENSE_APPR_REQ.APPLICATION_DATE,'DD-MM-YYYY') = '"
							+ bean.getAppliFromDate() + "'";
				}
				if (bean.getAppliDateSelect().trim().equals("OB")) {
					query += " AND HRMS_D1_EXPENSE_APPR_REQ.APPLICATION_DATE <= TO_DATE('"
	...
	private String getSortVal(String status, String[] labelNames) {
		...
		//APPLICATION DATE
			...
			sql = " HRMS_D1_EXPENSE_APPR_REQ.APPLICATION_DATE";
		}
		//ATTENDANCE DATE
		else if(status.equals(labelNames[7])){
			sql = " HRMS_D1_EXPENSE_APPR_REQ.FILE_HEADER_NAME";
		}
		//STATUS
		else if(status.equals(labelNames[8])){
			sql = " NVL(HRMS_D1_EXPENSE_APPR_REQ.STATUS,' ')";
		}
		//LOGIN IN TIME
	...
	public void getReport(ExpenseMISReport bean,
