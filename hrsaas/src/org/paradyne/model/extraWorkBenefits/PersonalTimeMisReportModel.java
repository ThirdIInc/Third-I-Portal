...
import org.paradyne.bean.extraWorkBenefits.PersonalTimeMisReport;
...
public class PersonalTimeMisReportModel extends ModelBase {
	...
	public boolean deleteSavedReport(PersonalTimeMisReport misreport) {
	...
	public boolean checkDuplicate(PersonalTimeMisReport misreport) {
		boolean result = false;
		String query = "SELECT * FROM HRMS_MISREPORT_HDR WHERE REPORT_TYPE LIKE 'PersonalTime' AND UPPER(REPORT_CRITERIA) LIKE '"
	...
	public boolean saveReportCriteria(PersonalTimeMisReport misreport) {
		// TODO Auto-generated method stub
		boolean result = false;
	...
	public boolean saveFilters(PersonalTimeMisReport misreport){
		boolean result = false;
		int count = 0;
		//Division
	...
	public boolean saveColumns(PersonalTimeMisReport misreport){
		boolean result = false;
		int count = 0;
		//Name
		...
		//Date
		if(misreport.getFromDateFlag().equals("true")){
			count++;
		}				
		//From Time
		if(misreport.getFromTimeFlag().equals("true")){
			count++;
		}				
		//To Time
		if(misreport.getToTimeFlag().equals("true")){
			count++;
		}
		//Total Time
		if(misreport.getTotalTimeFlag().equals("true")){
			count++;
		}	
		...
		//Date
		if(misreport.getFromDateFlag().equals("true")){
			addObj[int_count][0] = "misreport.fromDateFlag";
			addObj[int_count][1] = "Y";
			int_count++;
		}				
		//From Time
		if(misreport.getFromTimeFlag().equals("true")){
			addObj[int_count][0] = "misreport.fromTimeFlag";
			addObj[int_count][1] = "Y";
			int_count++;
		}				
		//To Time
		if(misreport.getToTimeFlag().equals("true")){
			addObj[int_count][0] = "misreport.toTimeFlag";
			addObj[int_count][1] = "Y";
			int_count++;
		}
		//Total Time
		if(misreport.getTotalTimeFlag().equals("true")){
			addObj[int_count][0] = "misreport.totalTimeFlag";
			addObj[int_count][1] = "Y";
			int_count++;
		}
	...
	public boolean saveSortOptions(PersonalTimeMisReport misreport){
		boolean result = false;
		int count = 0;
	...
	public boolean saveAdvancedFilters(PersonalTimeMisReport misreport){
		...
		//Date - Time1
		if (!misreport.getTime1DateSelect().equals("")) {
			count++;
		}
		if (!misreport.getTime1FromDate().equals("")) {
			count++;
		}
		if (!misreport.getTime1ToDate().equals("")) {
			count++;
		}	
		...
		if (!misreport.getTime1DateSelect().equals("")) {
			addObj[int_count][0] = "time1DateSelect";
			addObj[int_count][1] = checkNull(misreport.getTime1DateSelect().trim());
			int_count++;
		}
		if (!misreport.getTime1FromDate().equals("")) {
			addObj[int_count][0] = "misreport.time1FromDate";
			addObj[int_count][1] = checkNull(misreport.getTime1FromDate().trim());
			int_count++;
		}
		if (!misreport.getTime1ToDate().equals("")) {
			addObj[int_count][0] = "misreport.time1ToDate";
			addObj[int_count][1] = checkNull(misreport.getTime1ToDate().trim());
			int_count++;
		}		
		logger.info("int_count     : " + int_count);
		if(int_count==0){
	...
	public void setDetailOnScreen(PersonalTimeMisReport misreport) {
		// TODO Auto-generated method stub
		...
		if(dtlObj!= null && dtlObj.length > 0){
			try {
				for(int i=0; i< dtlObj.length; i++){
					...
					Method modelMethod = PersonalTimeMisReport.class.getDeclaredMethod(
	...
	public void callViewScreen(PersonalTimeMisReport misreport,
	...
	private String selectQuery(PersonalTimeMisReport misreport, String[] labelNames,
		...
		if(splitComma!=null){
			...
			for(int i=0;i<splitComma.length;i++){
				...
				if(splitDash[1].equals(labelNames[0].trim())){				//NAME
					...
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(PT_REG_APPLICATION_DATE,'DD-MM-YYYY')");
					labelMap.put(7, labelNames[6]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[7].trim())){			//STATUS
					columnMap.put(Integer.parseInt(splitDash[0]),
							" NVL(DECODE(PT_REG_DTL_STATUS,'P','Pending','A','Approved','R','Rejected','B','Sent Back'),' ')");
					labelMap.put(8, labelNames[7]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[8].trim())){			//FROM DATE
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(PT_REG_DTL_FROM_TIME,'DD-MM-YYYY')");
					labelMap.put(9, labelNames[8]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[9].trim())){			//FROM TIME
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(PT_REG_DTL_FROM_TIME,'HH24:MI')");
					labelMap.put(10, labelNames[9]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[10].trim())){			//TO TIME
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(PT_REG_DTL_TO_TIME,'HH24:MI')");
					labelMap.put(11, labelNames[10]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[11].trim())){		//TOTAL TIME
					columnMap.put(Integer.parseInt(splitDash[0]),
							" LPAD( TRUNC(MOD( ( TO_DATE(TO_char(PT_REG_DTL_TO_TIME, 'HH24.MI'),'HH24.MI') - TO_DATE(TO_char(PT_REG_DTL_FROM_TIME, " 
							+" 'HH24.MI'),'HH24.MI'))*24,24)) ,2,0) "
							+"  || ' : ' || "  
							+" LPAD( TRUNC(MOD( ( TO_DATE(TO_char(PT_REG_DTL_TO_TIME, 'HH24.MI'),'HH24.MI') - TO_DATE(TO_char(PT_REG_DTL_FROM_TIME, " 
							+" 'HH24.MI'),'HH24.MI'))*24*60,60)) ,2,0) as TOTALTIME ");
	...
	private String conditionQuery(PersonalTimeMisReport bean,
			String[] labelNames) {
		// TODO Auto-generated method stub
		String query="";
		try{
			query ="FROM HRMS_PT_REG_DTL "
				+ " LEFT JOIN HRMS_PT_REG_HDR ON (HRMS_PT_REG_HDR.PT_REG_ID = HRMS_PT_REG_DTL.PT_REG_ID)  "
			...
			if(!(bean.getStatus().equals("1")) && !(bean.getStatus() == null) && !(bean.getStatus().equals("null")) ){
				query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_STATUS = '"+bean.getStatus().trim()+"'";
			}
			//EMPLOYEE
			...
			if (!bean.getAppliDateSelect().trim().equals("")) {
				if (bean.getAppliDateSelect().trim().equals("ON")) {
					query += " AND HRMS_PT_REG_HDR.PT_REG_APPLICATION_DATE = TO_DATE('"
			...
			// DATE
			if (!bean.getTime1DateSelect().trim().equals("")) {
				if (bean.getTime1DateSelect().trim().equals("ON")) {
					query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME = TO_DATE('"
							+ bean.getTime1FromDate() + "','DD-MM-YYYY')";
				}
				if (bean.getTime1DateSelect().trim().equals("OB")) {
					query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME <= TO_DATE('"
							+ bean.getTime1FromDate() + "','DD-MM-YYYY')";
				}
				if (bean.getTime1DateSelect().trim().equals("OA")) {
					query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME >= TO_DATE('"
							+ bean.getTime1FromDate() + "','DD-MM-YYYY')";
				}
				if (bean.getTime1DateSelect().trim().equals("BF")) {
					query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME < TO_DATE('"
							+ bean.getTime1FromDate() + "','DD-MM-YYYY')";
				}
				if (bean.getTime1DateSelect().trim().equals("AF")) {
					query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME > TO_DATE('"
							+ bean.getTime1FromDate() + "','DD-MM-YYYY')";
				}
				if (bean.getTime1DateSelect().trim().equals("BN")) {
					query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME >= TO_DATE('"
							+ bean.getTime1FromDate() + "','DD-MM-YYYY')";
					if(!(bean.getTime1ToDate().equals("") || bean.getTime1ToDate().equals("DD-MM-YYYY"))){
						query += " AND HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME <= TO_DATE('"
							+ bean.getTime1ToDate() + "','DD-MM-YYYY')";
					}
				}
	...
	private String getSortVal(String status, String[] labelNames) {
		...
		//APPLICATION DATE
			...
			sql = " HRMS_PT_REG_HDR.PT_REG_APPLICATION_DATE";
		}
		//STATUS
		else if(status.equals(labelNames[7])){
			sql = " NVL(HRMS_PT_REG_DTL.PT_REG_DTL_STATUS,' ')";
		}
		//DATE
		else if(status.equals(labelNames[8])){
			sql = " HRMS_PT_REG_DTL.PT_REG_DTL_FROM_TIME";
		}
		//FROM TIME
		...
		//TO TIME
			...
			sql = " HRMS_PT_REG_DTL.PT_REG_DTL_TO_TIME";
		}
		//TOTAL TIME 
	...
	public void getReport(PersonalTimeMisReport bean, HttpServletResponse response, String[] labelNames, HttpServletRequest request) {
		try{
			...
			if (!bean.getReportTitle().equals("")){
				...
				reportName = "Personal_Time_Regularization_MIS_Report";
			}
			String fileName = reportName+"_"+Utility.getRandomNumber(1000);
			rds.setFileName(fileName);
			rds.setReportName("PERSONAL TIME REGULARIZATION MIS REPORT");
			...
			if (splitComma != null && splitComma.length > 0) {
				for (int i = 0; i < splitComma.length; i++) {
					...
					else if(splitDash[1].equals(labelNames[8].trim())){			//DATE
