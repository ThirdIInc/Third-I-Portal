...
public class LateReglMisReportModel extends ModelBase {
	...
	public boolean deleteSavedReport(LateReglMisReport misreport) {
	...
	public boolean checkDuplicate(LateReglMisReport misreport) {
		boolean result = false;
		String query = "SELECT * FROM HRMS_MISREPORT_HDR WHERE REPORT_TYPE LIKE 'LateRegl' AND UPPER(REPORT_CRITERIA) LIKE '"
	...
	public boolean saveReportCriteria(LateReglMisReport misreport) {
		// TODO Auto-generated method stub
		boolean result = false;
	...
	public boolean saveColumns(LateReglMisReport misreport){
		boolean result = false;
		int count = 0;
		//Name
	...
	public boolean saveFilters(LateReglMisReport misreport){
		boolean result = false;
		int count = 0;
		//Division
	...
	public boolean saveSortOptions(LateReglMisReport misreport){
		boolean result = false;
		int count = 0;
	...
	public boolean saveAdvancedFilters(LateReglMisReport misreport){
	...
	public void setDetailOnScreen(LateReglMisReport misreport) {
		// TODO Auto-generated method stub
		...
		if(dtlObj!= null && dtlObj.length > 0){
			try {
				for(int i=0; i< dtlObj.length; i++){
					...
					Method modelMethod = LateReglMisReport.class.getDeclaredMethod(
	...
	public void callViewScreen(LateReglMisReport misreport,
	...
	private String selectQuery(LateReglMisReport misreport, String[] labelNames,
		...
		if(splitComma!=null){
			...
			for(int i=0;i<splitComma.length;i++){
				...
				if(splitDash[1].equals(labelNames[0].trim())){				//NAME
					...
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(LATE_REG_APPLICATION_DATE,'DD-MM-YYYY')");
					labelMap.put(7, labelNames[6]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[7].trim())){			//ATTENDANCE DATE
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(LATE_REG_DTL_DATE,'DD-MM-YYYY')");
					labelMap.put(8, labelNames[7]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[8].trim())){			//STATUS
					columnMap.put(Integer.parseInt(splitDash[0]),
							"NVL(DECODE(LATE_REG_DTL_STATUS,'P','Pending','A','Approved','R','Rejected','B','Sent Back'),' ')");
					labelMap.put(9, labelNames[8]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[9].trim())){			//LOGIN IN TIME
					columnMap.put(Integer.parseInt(splitDash[0])," TO_CHAR(LATE_REG_DTL_INTIME,'HH24:MI')");
					labelMap.put(10, labelNames[9]);
					count++;
				}
				else if(splitDash[1].equals(labelNames[10].trim())){			//LATE HRS
					columnMap.put(Integer.parseInt(splitDash[0]),
							" LPAD( TRUNC(MOD( ( TO_DATE(TO_char(LATE_REG_DTL_INTIME, 'HH24.MI'),'HH24.MI') - TO_DATE(TO_char(LATE_REG_DTL_SFTTIME, " 
								+" 'HH24.MI'),'HH24.MI'))*24,24)) ,2,0)"
								+" || ' : ' || "  
							+" LPAD( TRUNC(MOD( ( TO_DATE(TO_char(LATE_REG_DTL_INTIME, 'HH24.MI'),'HH24.MI') - TO_DATE(TO_char(LATE_REG_DTL_SFTTIME, " 
								+" 'HH24.MI'),'HH24.MI'))*24*60,60)) ,2,0) as LATEHRS");
	...
	private String conditionQuery(LateReglMisReport bean,
			String[] labelNames) {
		// TODO Auto-generated method stub
		String query="";
		try{
			query ="FROM HRMS_LATE_REG_DTL "
				+ " LEFT JOIN HRMS_LATE_REG_HDR ON (HRMS_LATE_REG_HDR.LATE_REG_ID = HRMS_LATE_REG_DTL.LATE_REG_ID)  "
				...
				+ " LEFT JOIN HRMS_LEAVE ON (HRMS_LEAVE.LEAVE_ID = HRMS_LATE_REG_DTL.LATE_REG_DTL_LEAVE_ID) "
				+ " WHERE 1 = 1 ";
			//DIVISION
			...
			if(!(bean.getStatus().equals("1")) && !(bean.getStatus() == null) && !(bean.getStatus().equals("null")) ){
				query += " AND HRMS_LATE_REG_DTL.LATE_REG_DTL_STATUS = '"+bean.getStatus().trim()+"'";
			}
			//EMPLOYEE
			...
			if (!bean.getAppliDateSelect().trim().equals("")) {
				if (bean.getAppliDateSelect().trim().equals("ON")) {
					query += " AND HRMS_LATE_REG_HDR.LATE_REG_APPLICATION_DATE = TO_DATE('"
			...
			if (!bean.getAttDateSelect().trim().equals("")) {
				...
				if (bean.getAttDateSelect().trim().equals("ON")) {
					query += " AND HRMS_LATE_REG_DTL.LATE_REG_DTL_DATE = TO_DATE('"
	...
	private String getSortVal(String status, String[] labelNames) {
		...
		//ATTENDANCE DATE
			...
			sql = " HRMS_LATE_REG_DTL.LATE_REG_DTL_DATE";
		}
		//STATUS
		else if(status.equals(labelNames[8])){
			sql = " NVL(HRMS_LATE_REG_DTL.LATE_REG_DTL_STATUS,' ')";
		}
		//LOGIN IN TIME
		else if(status.equals(labelNames[9])){
			sql = " HRMS_LATE_REG_DTL.LATE_REG_DTL_INTIME";
		}
		//LATE HRS
	...
	/**This method generates the report as per the selected filters
	 * @param bean - LateReglMisReport
	 * @param response - response
	 * @param labelNames - labelNames
	 * @param request - request
	 */
	public void getReport(LateReglMisReport bean, HttpServletResponse response, String[] labelNames, HttpServletRequest request) {
		try{
			...
			if (!bean.getReportTitle().equals("")){
				...
				reportName = "Late_Regularization_MIS_Report";
			}
			String fileName = reportName+"_"+Utility.getRandomNumber(1000);
			rds.setFileName(fileName);
			rds.setReportName("LATE REGULARIZATION MIS REPORT");
	...
	public void getReport_old(LateReglMisReport bean, HttpServletResponse response, String[] labelNames, HttpServletRequest request) {
		try{
			String reportType = "";
			if (bean.getReportType().equals("P")) {
				reportType = "Pdf";
			...
			if (!bean.getReportTitle().equals(""))
				...
				reportName = "Late Regularization MIS Report";
