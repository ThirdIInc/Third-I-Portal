...
public class TrainingDetailsModel extends ModelBase{
	...
	public boolean checkExistingTrainingDetails(TrainingDetails bean,String apprId,String templateId,HttpServletRequest request){
		...
		if(data!=null && data.length>0){//TRAINING DETAILS FOR THE CALENDAR ALREADY EXISTS
			...
			/*for(int k=0;k<data.length;k++){
				...
				if(!data[k][6].toString().equals("null")){
					exist = true;
				}
			}*/
			...
			//STEP-1 GATHER TRAINING APPLICABILITY FLAG
			Object applData[][] = getSqlModel().getSingleResult(getQuery(8),newParam);
			if(applData!=null && applData.length>0){
				bean.setChkTrngAppl(applData[0][0].equals("Y")?"true":"false");
			}
			...
			//STEP-3 GATHER ALL TRAINING RECOMMENDED QUESTIONS 			
			Object questData[][] = getSqlModel().getSingleResult(getQuery(9),newParam);
			if(questData!=null && questData.length>0){
				
				ArrayList questList = new ArrayList<TrainingDetails>();
				
				for(int i=0;i<questData.length;i++){
					TrainingDetails bean2 = new TrainingDetails();
	 ...
	 * @param bean
	 */
	public void getConfiguredPhases(TrainingDetails bean){
	...
	public void getNewTrainingDetails(TrainingDetails bean){
	...
	public boolean remove(TrainingDetails bean,HttpServletRequest request){
		...
		if(hQuestionId!=null && hQuestionId.length>0){//IF QUESTIONS EXISTS IN THE LIST
			...
			for(int i=0;i<hQuestionId.length;i++){
				...
				if(removeQuestion[i].equals("Y") ){//IF QUESTION TO DELETE FOUND and are not in the training recommendation comments filled by appraisee
					...
					if( !hQuestSectionId[i].equals("") ){//REMOVE QUESTION FROM DATABASE
						...
						////CHECK WHETHER QUESTIONS REFERENCEs EXISTS 
						////IN TABLE FILLED BY THE APPRAISEE 
						Object param[]=new Object[2];
						param[0]= bean.getApprId();
						param[1]= hQuestionId[i];
						Object data[][]=getSqlModel().getSingleResult(getQuery(12),param);
						
						if(data!=null && data.length>0){//NO REFERENCE EXISTS
							...
							bean1.setHQuestion(hQuestion[i]);							
							list.add(bean1);
						}else{// REFERENCE EXIST THEN ADD QUESTION IN THE LIST	
