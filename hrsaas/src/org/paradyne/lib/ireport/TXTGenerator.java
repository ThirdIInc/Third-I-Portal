...
public class TXTGenerator {
	 ...
	 * @return String
	 */
	public String createDocTable(TableDataSet tds) {
		...
		if (tds.getHeaderFontColor() == null) {
			headerFontColor = new Color(0, 0, 0);
		} else {
		...
		Color dataBGcolor = null;
		Color dataFontColor = null;
		if (tds.getBodyFontColor() == null) {
			dataFontColor = new Color(0, 0, 0);
		} else {
			dataFontColor = tds.getBodyFontColor();
		}

		if (tds.getBodyBGColor() == null) {
			dataBGcolor = new Color(255, 255, 255);
		} else {
				...
				for (int j = 0; j < cellAlign.length; j++) {
					if (!(rowsAvg == null) && j == cellAlign.length - tempOne) {
						...
						if ((!(colNo == -1)) && j == colNo) {
							...
							column_fontcolor = RGBtoHex(colFontColor.color());
