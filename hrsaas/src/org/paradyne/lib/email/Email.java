 ...
 * @version $Revision: 1.1 $ $Date: 2012/09/26 14:14:12 $
 * @version $Id: Email.java,v 1.1 2012/09/26 14:14:12 cvs_mangesh_j Exp $
 */
public abstract class Email
{
    ...
    public static final String MAIL_TRANSPORT_TLS_TRUST = "mail.smtp.ssl.trust";
    ...
    protected List unsentList = new ArrayList();
    
    protected List invalidList = new ArrayList();
    ...
    protected boolean smtpWithSSL;
    
    protected boolean smtpWithTLS;
    ...
    public Session getMailSession() throws EmailException
    {
        if (this.session == null)
        {
            ...
            {
            	System.out.println("IN  MAIL_TRANSPORT_TLS");
            ...
            if (this.ssl)
            {
            	System.out.println("IS SSL....");
            	properties.setProperty(MAIL_PORT, sslSmtpPort);
                properties.setProperty(MAIL_TRANSPORT_TLS, tls ? "true" : "false");
                properties.setProperty(MAIL_TRANSPORT_TLS_TRUST, "*");
            ...
            */
            if (this.smtpWithTLS)
            {
            	System.out.println("......smtpWithTLS....");
            	properties.setProperty(MAIL_PORT, sslSmtpPort);
                properties.setProperty(MAIL_TRANSPORT_TLS,"true");
                properties.setProperty(MAIL_TRANSPORT_TLS_TRUST, "*");
             
            }
            if (this.smtpWithSSL)
            {
            	System.out.println("......smtpWithSSL....");
            	properties.setProperty(MAIL_PORT, sslSmtpPort);
                properties.setProperty(MAIL_TRANSPORT_TLS,"true");
                properties.setProperty(MAIL_TRANSPORT_TLS_TRUST, "*");
                properties.setProperty(MAIL_SMTP_SOCKET_FACTORY_PORT, sslSmtpPort);
     ...
     */
    public String sendMimeMessage() throws MessagingException
       //throws EmailException
    {
        ...
        catch (MessagingException t)
        {
        	...
        	t.printStackTrace();
        	try {
				Address[] ads = ((SendFailedException) t)
						.getInvalidAddresses();
				for (int i = 0; i < ads.length; i++) {
					// System.err.println("ads..."+String.valueOf(ads[i]));
					invalidList.add(String.valueOf(ads[i]));
				}
        	...
        	try {
				...
				for (int i = 0; i < ads.length; i++) {
					// System.err.println("ads..."+String.valueOf(ads[i]));
					unsentList.add(String.valueOf(ads[i]));
				}
				String msg = "Sending the email to the following server failed : "
						+ this.getHostName() + ":" + this.getSmtpPort();
     ...
     * @throws MessagingException 
     */
    public String send() throws EmailException, MessagingException
    {
	...
	public void setUnsentList(List unsentList) {
		this.unsentList = unsentList;
	}

	public boolean isSmtpWithSSL() {
		return smtpWithSSL;
	}

	public void setSmtpWithSSL(boolean smtpWithSSL) {
		this.smtpWithSSL = smtpWithSSL;
	}

	public boolean isSmtpWithTLS() {
		return smtpWithTLS;
	}

	public void setSmtpWithTLS(boolean smtpWithTLS) {
		this.smtpWithTLS = smtpWithTLS;
	}
