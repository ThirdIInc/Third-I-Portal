	...
	<table width="100%" border="0" align="right" cellpadding="2"
			...
			<table width="100%" border="0" align="right" cellpadding="2"
				...
				<tr>
					<td><strong class="text_head"><img
						...
						height="25" /></strong></td>
					<td width="93%" class="txt"><strong class="text_head">Birthday
					Mail </strong></td>
			...
			<table width="100%" border="0" align="center" cellpadding="0"
				...
				<tr>
					<td width="78%" colspan="3"><input type="button" class="token"
						value="    Send Birthday Mail" onclick="return calculate();" /> <input
						type="button" class="token" value="    Back"
						onclick="return backhomepage();" /></td>
					<td width="22%" align="right" colspan="1">
		...
		<tr>
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="2"
					...
					<table width="100%">
						<tr>
							...
							<td class="formtext" align="left"><s:a href="#"
								...
								click here to select <label class="set" name="department"
									id="department1"><%=label.get("department")%></label>:
						</s:a></td>
						</tr>
					</table>
					</div>
				...
				<tr>
					...
					<table width="100%">
						<tr>
							...
							<label class="set" name="division" id="division1"><%=label.get("division")%></label>
								:
						</s:a></td>
						</tr>
					</table>
				...
				<tr>
					<td width="15%" colspan="2" align="center"><Strong>
					AND/OR</strong></td>
				</tr>
				<tr>
					...
					<table width="100%">
						<tr>
							...
							<td class="formtext" align="left"><s:a href="#"
								...
								click here to select <label class="set" name="branch"
									id="branch1"><%=label.get("branch")%></label>:
						</s:a></td>
				...
				<tr>
					...
					<td width="75%" colspan="2" height="22"><s:textfield
						name="token" theme="simple" size="10" readonly="true" /><s:textfield
						size="28" theme="simple" maxlength="90" name="employee" /><img
				...
				<tr>
					...
					<img src="../pages/common/css/default/images/search2.gif"
						width="16" height="15"
						onclick="javascript:callsF9(500,325,'BirthdayMail_f9mailTemplate.action');">
		...
		<tr>
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="2"
				...
				<tr>
					...
					<td class="formth" align="center"><label class="set"
						name="list.Of.birthday.employee" id="list.Of.birthday.employee"
						ondblclick="callShowDiv(this);"><%=label.get("list.Of.birthday.employee")%></label></td>
					<td class="formth">&nbsp;</td>
				</tr>
				...
				<s:iterator value="list">
					<tr>
						<td width="10%" class="sortabletd"><%=++i%><s:hidden
							name="srNo" /></td>
						<td width="80%" class="sortabletd"><s:property
							value="empName" /><input type="hidden" name="hdeleteCode"
							id="hdeleteCode<%=i%>" value='<s:property value="hdeleteCode"/>' /></td>
						<s:hidden name="empCode" value="%{empCode}" />
						<td nowrap="nowrap" width="10%" class="sortabletd"><input
							type="checkbox" class="checkbox" id="confChk<%=i%>"
							name="confChk" value='<s:property value="confChk"/>'
...
function setEmpCode(id,i){
 ...
 if(document.getElementById('confChk'+i).checked == true)
   ...
   function calculate()
	...
	//alert('name'+name);
	//alert('chk'+document.getElementById('confChk').checked);
	if((department=='-1')&&(division=='-1')&&(branchname=='-1')&&(name=='')&&(document.getElementById('confChk').checked ==false))
	{
	alert(' Please Select atleast one criteria from Send Mail To option');
	return false;
	}
	
	//var division=document.getElementById('paraFrm_departmentCode').value
	...
	return false;
	 ...
	 var flag='<%=d %>';
	 if(flag=='0'){
	 alert( "No birthdays have been found on today's date");
	  return false;
	 }
	...
	document.getElementById('confChk'+a).checked = false;
	   ...
	   document.getElementById('paraFrm').action="BirthdayMail_sendBirthDayList.action";
	    document.getElementById('paraFrm').submit();
	    }
	    else
	    { 
	...
	document.getElementById('hdeleteCode'+a).value="";
	 ...
	 alert('Please select atleast one record form Birthday List');
	 return false;
	 }
	}
...
function previewTemplate()
{
	...
	window.open("<%=request.getContextPath() %>/AppStudio/BdayTemplate_setTemplateDataForPreview.action?tempCode="+tempCode,"","height=550,width=700,scrollbars=yes");
}
