...
<s:form action="AccountCategoryMaster" id="paraFrm" validate="true" target="main"
	theme="simple">
	...
	<s:hidden name="hiddencode"  />
		...
		<s:hidden name="accountAbbreviation"/>
	...
	<table width="100%" border="0"  class="formbg" align="right">
				...
				<s:if test="accountLength">
									<td id="ctrlShow" width="30%" align="right" class="">
										<b>Page:</b>
										<%	 totalPage = (Integer) request.getAttribute("totalPage");
											 pageNo = (Integer) request.getAttribute("pageNo");
										%>
										<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'AccountCategoryMaster_callPage.action');">
											<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
										</a>&nbsp;
										<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'AccountCategoryMaster_callPage.action');" >
											<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
										</a> 
										<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
										onkeypress="callPageText(event, '<%=totalPage%>', 'AccountCategoryMaster_callPage.action');return numbersOnly();" /> of <%=totalPage%>
										<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'AccountCategoryMaster_callPage.action')" >
											<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
										</a>&nbsp;
										<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'AccountCategoryMaster_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">					
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Account Category  </strong></td>			
						<td align="right">
								...
								<tr>
										...
										<tr class="td_bottom_border">
											<s:hidden name="myPage" id="myPage" />
											  <td class="formth" align="center" eidth="5%"><label name="serial.no"
												     id="serial.no" ondblclick="callShowDiv(this);"><%=label.get("serial.no")%></label></td>
											
											<td class="formth" align="center"><label name="account.name"
												    id="account.name" ondblclick="callShowDiv(this);"><%=label.get("account.name")%></label></td>
											
											
											<td class="formth" align="center"><label name="account.abbriviation"
												   id="account.abbriviation" ondblclick="callShowDiv(this);"><%=label.get("account.abbriviation")%></label></td>
									...
									<s:if test="accountLength">
										<td  align="center" class="formth" id="ctrlShow" width="2%" >
								...
								<s:iterator value="accountList">
									<tr  class="sortableTD"
										<%if(count%2==0){
											...
											ondblclick="javascript:callForEdit('<s:property value="accountID"/>');">
		
		
											<td width="5%" align="center" class="sortableTD">&nbsp;<%=++cn%><%++i;%><s:hidden name="srNo" /></td>
											<s:hidden value="%{accountID}" id='<%="accountID" + i%>'></s:hidden>
		                                      <script type="text/javascript">
												records[<%=i%>] = document.getElementById('accountID' + '<%=i%>').value;
											</script>
											<td width="20%" align="left" class="sortableTD">&nbsp;<s:property
												value="accountName" /> <input type="hidden"
												name="hdeleteCode" id="hdeleteCode<%=i%>" /></td>
												<td width="20%" align="left" class="sortableTD">&nbsp;<s:property
												value="accountAbbreviation" /></td>
												
											<td class="sortableTD" width="50%">&nbsp; <s:property value="isActive" /></td>
											
																					
											<td  id="ctrlShow" align="center" width="2%" class="sortableTD"><input type="checkbox"
												class="checkbox" id="confChk<%=i%>" name="confChk"
												onclick="callForDelete1('<s:property value="accountID"/>','<%=i%>')" /></td>
		...
		<tr><td width="100%"><table width="100%">
			...
			<td align="right" ><s:if test="accountLength"><b>Total No. Of Records:</b>&nbsp;<s:property value="totalRecords" /></s:if></td>
		</table></td>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'AccountCategoryMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='AccountCategoryMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'AccountCategoryMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="AccountCategoryMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="AccountCategoryMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
