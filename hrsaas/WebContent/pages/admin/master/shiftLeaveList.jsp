...
<s:form action="ShiftMaster" id="paraFrm" validate="true" theme="simple"
	target="main">
	<table width="100%">
		<tr>
			<td>
		...
		<tr>
			...
			<table width="100%" class="formbg">
				...
				<s:iterator value="leaveDataList">
					<tr>
						<input type="hidden" name="leaveAbbr" id="leaveAbbr<%=i%>"
							value='<s:property value="leaveAbbr"/>' />
						<td class="border2"><input type="hidden" name="leaveName"
							value='<s:property value="leaveName"/>' id="leaveName<%=i%>" /> <s:property
							value="leaveName" /><input type="hidden" name="leaveCode" id="leaveCode<%=i%>"
							value='<s:property value="leaveCode"/>' /></td>
...
function selectLeave(){
	...
	try{
		...
		opener.document.getElementById(leaveAbbr).value = selectedLeaveAbbr;
	}
	catch(e){
   		alert(e);
   	}
...
function setInitialValue(){
	 var count  = '<%=j%>'; 
     var oldvalue = opener.document.getElementById(leaveAbbr).value;
...
function setLeavePriority(code, buttonType){
	...
	if(buttonType == 'upWard'){
		...
		document.getElementById("paraFrm").action = "ShiftMaster_setLeavePriority.action?type=up";
		document.getElementById("paraFrm").submit();
	}
	
	if(buttonType == 'downWard'){
		document.getElementById("paraFrm").target = '';
		document.getElementById("paraFrm").action = "ShiftMaster_setLeavePriority.action?type=down";
		document.getElementById("paraFrm").submit();
	}
}
setInitialValue();
