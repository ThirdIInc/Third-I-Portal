...
<s:form action="OnsitePosting" id="paraFrm" validate="true" target="main"
	theme="simple">
	
	<s:hidden name="show" value="%{show}" />
	<s:hidden name="hiddencode"  />
		<s:hidden name="loccationId"/>
		<s:hidden name="siteLocation"/>
		<s:hidden name="hidSiteId"/>
		<s:hidden name="siteType"/>	<s:hidden name="siteAddress1 "/>
		<s:hidden name="siteCode"/>	<s:hidden name="siteAbbreviation"/>		
		<s:hidden name="siteName"/>	<s:hidden name="phone"/>	
		<s:hidden name="emailId	"/><s:hidden name="siteAddress2"/>	
		<s:hidden name=" siteAddress3"/>	
	...
	<table width="100%" border="0"  class="formbg" align="right">
							...
							<table  width="100%" border="0" align="right" cellpadding="0" cellspacing="0" class="formbg" >
								    <tr>
									    <td>
								    	 ...
								    	 <td width="93%" class="txt"><strong class="text_head">Onsite Posting </strong></td>
				...
				<s:if test="postingLength">
									<td id="ctrlShow" width="30%" align="right" class="">
										<b>Page:</b>
										<%	 totalPage = (Integer) request.getAttribute("totalPage");
											 pageNo = (Integer) request.getAttribute("pageNo");
										%>
										<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'OnsitePosting_callPage.action');">
											<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
										</a>&nbsp;
										<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'OnsitePosting_callPage.action');" >
											<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
										</a> 
										<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
										onkeypress="callPageText(event, '<%=totalPage%>', 'OnsitePosting_callPage.action');return numbersOnly();" /> of <%=totalPage%>
										<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'OnsitePosting_callPage.action')" >
											<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
										</a>&nbsp;
										<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'OnsitePosting_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">					
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Onsite Posting </strong></td>			
						<td align="right">
							...
							<table width="100%" border="0" cellpadding="2" cellspacing="2"	class="formbg">
								<tr>
										...
										<tr class="td_bottom_border">
											...
											<td class="formth" align="center"><label name="site.type"
												    id="site.type" ondblclick="callShowDiv(this);"><%=label.get("site.type")%></label></td>
											
											
											<td class="formth" align="center"><label name="site.code"
												   id="site.code" ondblclick="callShowDiv(this);"><%=label.get("site.code")%></label></td>
												
											<td class="formth" align="center"><label name="site.abbriviation"
												   id="site.abbriviation" ondblclick="callShowDiv(this);"><%=label.get("site.abbriviation")%></label></td>
												
												<td class="formth" align="center"><label name="site.name"
								...
								<s:iterator value="postingList">
									<tr  class="sortableTD"
										<%if(count%2==0){
											...
											ondblclick="javascript:callForEdit('<s:property value="onSiteID"/>');">
		
		
											<td width="10%" align="center" class="sortableTD">&nbsp;<%=++cn%><%++i;%><s:hidden name="srNo" /></td>
											<s:hidden value="%{onSiteID}" id='<%="onSiteID" + i%>'></s:hidden>
		                                      <script type="text/javascript">
												records[<%=i%>] = document.getElementById('onSiteID' + '<%=i%>').value;
											</script>
											<td width="20%" align="left" class="sortableTD">&nbsp;<s:property
												value="siteType" /> <input type="hidden"
												name="hdeleteCode" id="hdeleteCode<%=i%>" /></td>
												<td width="20%" align="left" class="sortableTD">&nbsp;<s:property
												value="siteCode" /></td>
												<td width="20%" align="left" class="sortableTD">&nbsp;<s:property
												value="siteAbbreviation" /></td>
											...
											<td  id="ctrlShow" align="center" nowrap="nowrap" class="sortableTD"><input type="checkbox"
												...
												onclick="callForDelete1('<s:property value="onSiteID"/>','<%=i%>')" /></td>
		...
		<tr><td width="100%"><table width="100%">
			...
			<td align="right" ><s:if test="postingLength"><b>Total No. Of Records:</b>&nbsp;<s:property value="totalRecords" /></s:if></td>
		</table></td>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'OnsitePosting_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='OnsitePosting_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'OnsitePosting_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="OnsitePosting_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   ...
	   document.getElementById('paraFrm').action="OnsitePosting_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
