<%@ taglib prefix="s" uri="/struts-tags" %>  
	...
	var records = new Array();
 	...
 	<s:form action="CategoryMaster"  validate="true" id="paraFrm" theme="simple" >
  	<s:hidden name="show" value="%{show}" />
	<s:hidden name="hiddencode"  />
	<s:hidden name="catgMaster.catgID"/>
	<s:hidden name="catgMaster.catgName"/>
	<s:hidden name="catgMaster.catgDesc"/>
	<s:hidden name="catgMaster.catgAge" />
	<s:hidden name="catgMaster.isActive" />
	
	<table width="100%" border="0" align="right" class="formbg">
		
		<tr>
			<td valign="bottom" class="txt">
			<table  width="100%" border="0"  cellpadding="0" align="rigth"
			...
			<td width="100%">
				...
				<s:if test="modeLength">
						<td id="ctrlShow" width="30%" align="right" class="">
							...
							%>
							<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'CategoryMaster_callPage.action');">
								<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
							</a>&nbsp;
							<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'CategoryMaster_callPage.action');" >
								<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
							</a> 
							<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'CategoryMaster_callPage.action');return numbersOnly();" /> of <%=totalPage%>
							<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'CategoryMaster_callPage.action')" >
								<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
							</a>&nbsp;
							<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'CategoryMaster_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">		
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Category
						  </strong></td>		
						<td align="right">
							...
							<tr>
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1">
									<tr>
										...
										<td class="formth" align="center"><label  class = "set" name="categoryname" id="categoryname1" ondblclick="callShowDiv(this);"><%=label.get("categoryname")%></label></td>
											<td class="formth" align="center"><label  class = "set" name="categorydescription" id="categorydescription1" ondblclick="callShowDiv(this);"><%=label.get("categorydescription")%></label></td>
									...
									<%int count=0; %>
										...
										<tr 
											<%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="catgID"/>');">
			
			
												<td width="10%" align="center" class="sortableTD"><%=++cn%><%++i;%><s:hidden name="srNo" /></td>
												<s:hidden value="%{catgID}" id='<%="catgID" + i%>'></s:hidden>
													<script type="text/javascript">
													records[<%=i%>] = document.getElementById('catgID' + '<%=i%>').value;
												</script>
												<td width="40%" align="left" class="sortableTD">&nbsp;<s:property
													value="catgName" /> <input type="hidden"
													name="hdeleteCode" id="hdeleteCode<%=i%>" /></td>
													<td width="50%" align="left" class="sortableTD">&nbsp;
													 <s:property value="catgDesc"/></td>
												...
												<td  id="ctrlShow" align="center" nowrap="nowrap" class="sortableTD"><input type="checkbox"
													...
													onclick="callForDelete1('<s:property value="catgID"/>','<%=i%>')" /></td>
											</tr>
	...
	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'CategoryMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='CategoryMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'CategoryMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="CategoryMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="CategoryMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
