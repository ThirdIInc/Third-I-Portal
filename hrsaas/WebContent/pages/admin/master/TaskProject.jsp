...
<s:hidden name="proCode"/> <s:hidden name="projName" />
...
<% int totalPage = 0; int pageNo = 0; %>
	<s:if test="modeLength">	
			<td id="ctrlShow" width="30%" align="right" class="">
				...
				%>
				<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'TaskProject_callPage.action');">
					<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
				</a>&nbsp;
				<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'TaskProject_callPage.action');" >
					<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
				</a> 
				<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
				onkeypress="callPageText(event, '<%=totalPage%>', 'TaskProject_callPage.action');return numbersOnly();" /> of <%=totalPage%>
				<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'TaskProject_callPage.action')" >
					<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
				</a>&nbsp;
				<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'TaskProject_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">		
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Task Project
						  </strong></td>		
						<td align="right">
							...
							<tr>
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1">
									<tr>
										...
										<td class="formth" align="center"><label  name="serial.no" id="serial.no" ondblclick="callShowDiv(this);"><%=label.get("serial.no")%></label></td>
										<td class="formth" align="center"><label  name="projectname" id="projectname1" ondblclick="callShowDiv(this);"><%=label.get("projectname")%></label></td>
									...
									<s:if test="modeLength">	
										...
										<s:iterator value="iteratorlist">
											...
											<tr <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="proCode" />');">
			
			
												<td width="10%" align="center" class="sortableTD"><%=++cnt%>
												<%++i;%>
												</td>
												<s:hidden value="%{proCode}" id='<%="proCode"+i %>'></s:hidden>
												<script type="text/javascript">
													records[<%=i%>] = document.getElementById('proCode' + '<%=i%>').value;
												</script>
												<td width="90%" align="left" class="sortableTD"><s:property value="projName" />&nbsp;
												...
												<td id="ctrlShow" align="center" nowrap="nowrap" class="sortableTD">&nbsp;<input type="checkbox"
													...
													onclick="callForDelete1('<s:property value="proCode" />','<%=i%>')" /></td>
											</tr>
	...
	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'TaskProject_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='TaskProject_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'TaskProject_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="TaskProject_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="TaskProject_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
