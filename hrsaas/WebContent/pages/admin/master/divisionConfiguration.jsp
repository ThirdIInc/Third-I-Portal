...
<s:form action="DivisionConfiguration"  validate="true" id="paraFrm" validate="true" theme="simple">
				...
				<table width="100%">
					<tr>
						...
						<td id="ctrlShow" width="30%" align="right" class="">
							...
							%>
							<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'DivisionConfiguration_callPage.action');">
								<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
							</a>&nbsp;
							<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'DivisionConfiguration_callPage.action');" >
								<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
							</a> 
							<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'DivisionConfiguration_callPage.action');return numbersOnly();" /> of <%=totalPage%>
							<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'DivisionConfiguration_callPage.action')" >
								<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
							</a>&nbsp;
							<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'DivisionConfiguration_callPage.action');" >
...
<script>
	...
	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'DivisionConfiguration_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='DivisionConfiguration_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'DivisionConfiguration_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	function callForEdit(id){
		callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="DivisionConfiguration_calforedit.action";
	   	document.getElementById('paraFrm').target = "_self";
	    document.getElementById("paraFrm").submit();
   }
	...
	function chkDelete()
	{
	 if(chk()){
	   ...
	   if(con){
	        document.getElementById('paraFrm').action="DivisionConfiguration_delete1.action";
	        document.getElementById('paraFrm').target = "_self";
	   		document.getElementById('paraFrm').submit();
	    }
	    else
