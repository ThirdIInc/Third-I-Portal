...
<s:hidden name="show" value="%{show}" />
	...
	<s:hidden name="casteMaster.casteName"/> 
	<s:hidden name="casteMaster.casteCatgName"/>
	 <s:hidden name="casteMaster.casteCatgCode"/>
	 
	 <s:hidden name="casteMaster.isActive" />
	...
	<table width="100%" border="0" align="right"  class="formbg">
							...
							<% int totalPage = 0; int pageNo = 0; %>
								<s:if test="modeLength">
										<td id="ctrlShow" width="30%" align="right" class="">
											...
											%>
											<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'CasteMaster_callPage.action');">
												<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
											</a>&nbsp;
											<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'CasteMaster_callPage.action');" >
												<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
											</a> 
											<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
											onkeypress="callPageText(event, '<%=totalPage%>', 'CasteMaster_callPage.action');return numbersOnly();" /> of <%=totalPage%>
											<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'CasteMaster_callPage.action')" >
												<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
											</a>&nbsp;
											<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'CasteMaster_callPage.action');" >
		...
		<tr>
			<td colspan="2" class="txt"><strong class="text_head">Caste
			  </strong></td>		
			<td align="right">
				...
				<tr>
					...
					<table width="100%" border="0" cellpadding="1" cellspacing="1">
						<tr>
							...
							<td class="formth" align="center"><label  class = "set" name="castename" id="castename1" ondblclick="callShowDiv(this);"><%=label.get("castename")%></label></td>
								<td class="formth" align="center"><label  class = "set" name="castcategory" id="castcategory1" ondblclick="callShowDiv(this);"><%=label.get("castcategory")%></label></td>
						...
						<%int count=0; %>
							...
							<tr 
								<%if(count%2==0){
									...
									ondblclick="javascript:callForEdit('<s:property value="casteCode"/>');">


									<td width="10%" align="center" class="sortableTD"><%=++cn%><%++i;%><s:hidden name="srNo" /></td>
									<s:hidden value="%{casteCode}" id='<%="casteCode" + i%>' />
										<script type="text/javascript">
										records[<%=i%>] = document.getElementById('casteCode' + '<%=i%>').value;
									</script>
					
									<td width="40%" align="left" class="sortableTD"><s:property
										value="casteName" /> <input type="hidden"
										name="hdeleteCode" id="hdeleteCode<%=i%>" /></td>
										<td width="50%" align="left" class="sortableTD"><s:property
										value="casteCatgName" />&nbsp;</td>
									...
									<td id="ctrlShow"  align="center" nowrap="nowrap" class="sortableTD"><input type="checkbox"
										...
										onclick="callForDelete1('<s:property value="casteCode"/>','<%=i%>')" /></td>
								</tr>
  	...
  	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'CasteMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='CasteMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'CasteMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="CasteMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="CasteMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
