...
<s:hidden name="titleName"  /><s:hidden name="isActive"  />
				...
				<table width="100%">
					<tr>
						...
						<td id="ctrlShow" width="30%" align="right" class="">
							...
							%>
							<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'TitleMaster_callPage.action');">
								<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
							</a>&nbsp;
							<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'TitleMaster_callPage.action');" >
								<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
							</a> 
							<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'TitleMaster_callPage.action');return numbersOnly();" /> of <%=totalPage%>
							<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'TitleMaster_callPage.action')" >
								<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
							</a>&nbsp;
							<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'TitleMaster_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">		
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Title</strong></td>		
						<td align="right">
											...
											<table width="100%" cellpadding="1" cellspacing="1" class="sortableTD">
												<tr class="td_bottom_border">
													...
													<td class="formth" align="center" width="60%">
														<label name="title" id="title1" ondblclick="callShowDiv(this);"><%=label.get("title")%></label>
													</td>
											...
											onclick="selectAllRecords(this);" />
													...
													<s:iterator value="titleList">
														<tr title="Double click for edit"
															...
															ondblclick="javascript:callForEdit('<s:property value="titleCode"/>');">			
															<td width="10%" align="center" class="sortableTD" >
																<%= ++cn %>
																<% ++i; %>
																<s:hidden name="srNo" />
															</td>
															<s:hidden value="%{titleCode}"  id='<%="titleCode" + i%>'/>
															 <script type="text/javascript">
													records[<%=i%>] = document.getElementById('titleCode' + '<%=i%>').value;
												</script>			
															<td width="90%" align="left" class="sortableTD">
																<s:property value="titleName" />
																
																<td class="sortableTD" width="50%">&nbsp; <s:property value="isActive" /></td> <!-- Added by Abhijit -->
															...
															<td align="center" nowrap="nowrap" id="ctrlShow" class="sortableTD" width="10%">
																...
																onclick="callForDelete1('<s:property value="titleCode"/>','<%=i%>')" />
															</td>
														</tr>
													</s:iterator>
													<% d = i; %>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'TitleMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='TitleMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'TitleMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="TitleMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="TitleMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
