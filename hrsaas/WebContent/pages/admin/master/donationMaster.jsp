...
<s:form action="DonationMaster" validate="true" id="paraFrm" validate="true"
	theme="simple">
	<s:hidden name="show" />
	<s:textfield name="hiddencode" />
	<s:hidden name="editFlag"/>
	<s:hidden name="cancelFlag"/>
	
	
	<s:hidden name="donationName"/> <s:hidden name="percentage"/> 
	<s:hidden name="designationStatus"/> <s:hidden name="donationCode"/>
	<table class="formbg" width="100%"  align="right">
			...
			<table width="100%" border="0" align="right" cellpadding="0"
				...
				<s:if test="modeLength">
									...
									<td id="ctrlShow" width="100%" align="right" class="">
										...
										%>
										<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'DonationMaster_callPage.action');">
											<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
										</a>&nbsp;
										<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'DonationMaster_callPage.action');" >
											<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
										</a> 
										<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
										onkeypress="callPageText(event, '<%=totalPage%>', 'DonationMaster_callPage.action');return numbersOnly();" /> of <%=totalPage%>
										<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'DonationMaster_callPage.action')" >
											<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
										</a>&nbsp;
										<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'DonationMaster_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				...
				<tr>
					<td colspan="2" class="txt"><strong class="text_head">Donation</strong></td>				
					<td align="right" >
						<input type="button" class="delete" id="ctrlShow" value=" Delete" onclick="return chkDelete();" />						
					</td>
				</tr>
				<tr>
					<td colspan="3">
						...
						<tr>
							...
							<table width="100%" border="0" >
								...
								<tr>
									...
									<td class="formth" align="center"><label class="set" name="donation.name"
										id="donation.name" ondblclick="callShowDiv(this);"><%=label.get("donation.name")%></label>
									</td>
									<td class="formth" align="center"><label class="set" name="donation.percentage"
										id="donation.percentage" ondblclick="callShowDiv(this);"><%=label.get("donation.percentage")%></label>
									</td>
                    ...
                    <s:if test="modeLength">
									...
									%> class="tableCell1"
											...
											title="Double click for edit"  ondblclick="javascript:callForEdit('<s:property  value="donationCode"/>');">
											<%=++cnt%> <%++i;%>
											</td>


											<td width="40%" align="left" class="sortableTD" title="Double click for edit"  
											ondblclick="javascript:callForEdit('<s:property  value="donationCode"/>');">
											<s:hidden value="%{donationCode}" id='<%="donationCode" + i%>'>
											</s:hidden> 
											<script type="text/javascript">
										records[<%=i%>] = document.getElementById('donationCode' + '<%=i%>').value;
									</script>
									<input type="hidden"
												name="hdeleteCode" id="hdeleteCode" />
											<!--<input type="hidden"
												name="hdeleteCode" id="hdeleteCode<%=i%>" />--> <s:property
												value="donationName" /></td>
											<td width="40%" align="left" class="sortableTD" title="Double click for edit"  
											ondblclick="javascript:callForEdit('<s:property  value="donationCode"/>');">
											<s:property value="percentage" /></td>
											...
											<td  id="ctrlShow" align="center" nowrap="nowrap" class="sortableTD"><input
												...
												name="confChk"
												onclick="callForDelete1('<s:property  value="assetCode"/>','<%=i%>')" /></td>
										</tr>
...
<script>
	...
	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'DonationMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='DonationMaster_f9Action.action';
		document.getElementById("paraFrm").submit();
	}
...
function oldRowColor(cell,val) {
	...
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="DonationMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="DonationMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
