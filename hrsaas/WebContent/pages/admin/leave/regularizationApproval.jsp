			...
			<table width="100%" border="0" align="right" cellpadding="0"
				...
				<tr>
					<td><strong class="text_head"><img
						...
						height="25" /></strong></td>
					<td width="93%" class="txt"><strong class="text_head">Regularization Approval</strong></td>
		    ...
		    function setAction(listType){
		     ...
		     if(listType=="P"){
		     	document.getElementById('paraFrm_status').value='P';
		      	document.getElementById('paraFrm').action='RegularizationApproval_input.action';
		      	document.getElementById('paraFrm').submit();
		      
		     }if(listType=="A"){
		     document.getElementById('paraFrm_status').value='A';
		      	document.getElementById('paraFrm').action='RegularizationApproval_approved.action';
		      	document.getElementById('paraFrm').submit();
		      
		     }
		     if(listType=="B"){
		     
		      	document.getElementById('paraFrm').action='RegularizationApproval_sendBack.action';
		      document.getElementById('paraFrm').submit();
		     ...
		     if(listType=="R"){
		        document.getElementById('paraFrm_status').value='R';
		      	document.getElementById('paraFrm').action='RegularizationApproval_reject.action';
		      	document.getElementById('paraFrm').submit();
					...
					<tr>
						...
						<table width="100%" class="sorttable">
							<tr>
								...
								<td class="formth"><b>Applied For</b> </td>
							...
							<s:iterator value="swipeList">
								<tr id="swipeRow<%=f1%>" style="display: none;">
									...
									<td class="sortableTD">
									<s:select disabled="true" name="applyForDecode" value="%{applyForitt}" 
									list="#{'1':'Forgot to bring access card','2':'Forgot to login/logout ','3':'Access Card Not Issued',
									'4':'Special Sanction','5':'Swipe System Not Working ','6':'Outdoor visit',
									'7':'Late Regularization','8':'Work From Home'}">
									</s:select>
									</td>
									...
									<input
										...
										value='<s:property value="viewApplication"/>' 
										onclick="viewDetails('<s:property value="swipeAppCode"/>','<s:property value="applicationType"/>','SR','<s:property value="applEmpCode"/>','<s:property value="applyForitt"/>')" /></td>
								</tr>
								<%
								i++;
								f1++;
									...
									<s:iterator value="regList">
										<tr id="lateRow<%=me%>" style="display: none;">
											...
											<td class="sortableTD" width="20%" align="center"><input
												...
												value='<s:property value="viewApplication"/>' 
												onclick="viewDetails('<s:property value="lateAppCode"/>','<s:property value="applicationType"/>','LR','<s:property value="applEmpCode"/>','LR')" /></td>
										</tr>
										<%
										me++;
										%>
									...
									<s:iterator value="list">
										<tr id="redressalRow<%=rrrr%>" style="display: none;">
											...
											<td class="sortableTD" width="20%" align="center"><input
												type="button" name="view_Details" class="token"
												value='<s:property value="viewApplication"/>' 
												onclick="viewDetails('<s:property value="redressalAppCode"/>','<s:property value="applicationType"/>','RR','<s:property value="applEmpCode"/>','RR')" /></td>
										</tr>
										<%
										b++;rrrr++;
										%>
									...
									<s:iterator value="ptList">
										<tr id="ptRow<%=pp%>" style="display: none;">
											...
											<input
												type="button" name="view_Details" class="token"
												value='<s:property value="viewApplication"/>' 
												onclick="viewDetails('<s:property value="personalTimeCode"/>','<s:property value="applicationType"/>','PT','<s:property value="applEmpCode"/>','PT')" /></td>
										</tr>
										<%
										pt++;pp++;
										%>
...
var recCount = 10;
			...
			function addapplicationFun() {	
				...
				document.getElementById('paraFrm').action = 'LeaveApplication_addNew.action';
				document.getElementById('paraFrm').submit();
			}

 		function viewDetails(appCode,status,type,empCode,swipeType){	
 			//alert("appCode "+appCode);	
 				//alert("status "+status);	
 					//alert("type "+type);	
 						//alert("empCode "+empCode);	
 							//alert("swipeType "+swipeType);	
 							 
 									     	 		  
		      	// document.getElementById('paraFrm').action='RegularizationApproval_viewApplication.action?appCode='+appCode+'&status='+status+'&type='+type+'&empCode='+empCode+'&swipeType='+swipeType;
		      	
		      	document.getElementById('paraFrm').action='RegularizationApproval_viewApplication.action?appCode='+appCode+'&status='+status+'&type='+swipeType+'&empCode='+empCode+'&swipeType='+swipeType;
