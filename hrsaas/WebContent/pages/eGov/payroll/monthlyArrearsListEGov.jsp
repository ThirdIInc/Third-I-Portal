...
<s:form action="MonthlyArrearsEGov" validate="true" id="paraFrm"
			...
			<table width="100%" align="left" cellspacing="0" cellpadding="0">
				<tr>
					...
					<s:if test="listLength">
						...
						%> <a href="#"
							onclick="callPage('1', 'F', '<%=totalPage%>', 'MonthlyArrearsEGov_input.action');">
						<img title="First Page" src="../pages/common/img/first.gif"
							width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('P', 'P', '<%=totalPage%>', 'MonthlyArrearsEGov_input.action');">
						<img title="Previous Page" src="../pages/common/img/previous.gif"
							width="10" height="10" class="iconImage" /> </a> <input type="text"
							name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
							maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'MonthlyArrearsEGov_input.action');return numbersOnly();" />
						of <%=totalPage%> <a href="#"
							onclick="callPage('N', 'N', '<%=totalPage%>', 'MonthlyArrearsEGov_input.action')">
						<img title="Next Page" src="../pages/common/img/next.gif"
							class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'MonthlyArrearsEGov_input.action');">
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="1"
						...
						<tr class="sortableTD">
							...
							<td class="formth"><label class="set"
								name="reflecting.month" id="reflecting.month1"
								ondblclick="callShowDiv(this);"><%=label.get("reflecting.month")%></label>
							</td>
							<td class="formth"><label class="set" name="reflecting.year"
								id="reflecting.year1" ondblclick="callShowDiv(this);"><%=label.get("reflecting.year")%></label>
							</td>
							...
							<td class="formth"><label class="set" name="department"
								id="department1" ondblclick="callShowDiv(this);"><%=label.get("department")%></label>
							</td>
						...
						<s:iterator value="iteratorlist">
							<tr <%if(count%2==0){
								...
								ondblclick="javascript:callForEdit('<s:property value="listArrearCode"/>','<s:property value="listReflectingMonth"/>',
																...
																'<s:property value="listPayType"/>','<s:property value="listPayTypeName"/>','<s:property value="listPayinSalFlag"/>');">
								<script type="text/javascript">
...
<script type="text/javascript">
	...
	{	
		...
		document.getElementById("paraFrm").action='MonthlyArrearsEGov_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action = 'MonthlyArrearsEGov_f9Search.action';
		document.getElementById("paraFrm").submit();
	}
	
	
	function callForEdit(arrearCode, month, monthCode, year,
	divname,divid,branch,brnId,deptName, deptId, empType, typeId, paybill, paybillId, payType, payTypeName,paySalFlag) 
	{
		try{
		//alert(paySalFlag);
		...
		document.getElementById("paraFrm").action="MonthlyArrearsEGov_callForEdit.action";
