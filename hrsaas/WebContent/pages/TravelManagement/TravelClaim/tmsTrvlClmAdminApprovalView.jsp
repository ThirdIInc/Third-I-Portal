...
<s:form action="TravelClmAdminAppvr" validate="true" id="paraFrm"
			...
			<table width="100%" align="center" class="formbg" cellpadding="0"
				...
				<tr>
					...
					<s:if test='%{clmAppFlag=="Y"}'>
						...
						Claim Admin Application</strong></td>
					</s:if>
					<s:else>
						<td width="93%" class="txt"><strong class="text_head">Travel
						Claim Admin Approval</strong></td>
					</s:else>
				...
				<table class="formbg" border = "0" width="100%">
					<tr>
						<td class="formtext" height="22"><label class="set"
							name="trvlClm.budgetExp" id="trvlClm.budgetExp"
							ondblclick="callShowDiv(this);"><%=label.get("trvlClm.budgetExp")%></label>:</td>
						<td><s:label name="budgetExpenditure" theme="simple"
							value="%{budgetExpenditure}" /></td>
						<td class="formtext"><label class="set" name="trvlClm.ActExp"
							id="trvlClm.ActExpt" ondblclick="callShowDiv(this);"><%=label.get("trvlClm.ActExp")%></label>:</td>
						<td align="right"><s:label name="actualExpenditure" theme="simple"
							value="%{actualExpenditure}" /></td> 
						<td align="right">&nbsp;<input
							name="viewExpenditure" type="button" class="token"
							value="  Details  " onclick="viewSplitExpenditure();" /></td>
						<td align="right"><input name="viewPolicy" type="button" class="token"
							value="View Application" onclick="viewApplication();" /></td>
						<td align="right">&nbsp;
						<s:if test="showBookingDetailsFlag">
						 <input name="bookingDetails" type="button"	class="token" value="Booking Details"
						...
						<tr>
							...
							<tr>
								...
								<td width="25%" nowrap="nowrap">
									<s:property value="currencyEmployeeAdvance"/><s:hidden name="currencyEmployeeAdvance"></s:hidden>
									<s:label name="clmAdvance" theme="simple" value="%{clmAdvance}" /></td>
							</tr>
							<tr>
	...
	<!-- 	<tr>
			...
			<table width="100%" class="formbg" border="0">
				<tr>
					<td width="100%" colspan="4"><b>Tour Report</b></td>
				</tr>
				<tr id="tourTable">
					<td width="100%" colspan="4">
					<table  width="100%" border="0" >
						<tr>
							<td width="22%"><label class="set" name="tourDesc"
								id="tourDesc" ondblclick="callShowDiv(this);"><%=label.get("tourDesc")%></label>
							:</td>
							<td colspan="3">
							<s:property value="tourComments" />
							</td>
						</tr>
						<tr>
							<td width="22%"><label class="set" name="serviceQual"
								id="serviceQual" ondblclick="callShowDiv(this);"><%=label.get("attachTour")%></label>
							:</td>
							<td colspan="3">
							<a href="#"  onclick="return showproofname('<s:property value="tourReportFile" />')"><font color="blue"><s:property value="tourReportFile" /></font></a>
							</td>
						</tr>
						<tr>
							<td width="22%"><label class="set" name="achieve"
								id="achieve" ondblclick="callShowDiv(this);"><%=label.get("achieve")%></label>
							:</td>
							<td colspan="3"><s:property value="achievementComments" />
							</td>
						</tr>
						<tr>
							<td width="22%"><label class="set" name="action" id="action"
								ondblclick="callShowDiv(this);"><%=label.get("action")%></label>
							:</td>
							<td colspan="3">
							<s:property value="followUpComments" />
							</td>
						</tr>
						<tr>
							<td width="22%"><label class="set" name="targetDt" id="targetDt"
								ondblclick="callShowDiv(this);"><%=label.get("targetDt")%></label>
							:</td>
							<td colspan="3">
							<s:property value="targetDate" />
							</td>
						</tr>
					</table>
					</td>
				...
				<table width="100%" align="center" class="formbg" theme="simple"
					...
					<tr>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							class="formbg">
							<tr>
								<td class="formth" width="5%" nowrap="nowrap"><strong><label
									class="set" name="trvlClm.srNo" id="trvlClm.srNo"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.srNo")%></label></strong>
								</td>
								<td class="formth" width="10%"><strong><label
									class="set" name="trvlClm.ExpDate" id="trvlClm.ExpDate"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.ExpDate")%></label></strong>
								</td>
								<td class="formth" width="20%"><strong><label
									class="set" name="trvlClm.ExpType" id="trvlClm.ExpType"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.ExpType")%></label></strong>
								</td>
								<td class="formth" width="20%"><strong><label 
									class="set" name="trvlClm.partclrs" id="trvlClm.partclrs"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.partclrs")%></label>
									</strong>
								</td>
								<td class="formth" width="10%"><strong><label
									class="set" name="trvlClm.eligibleAmt" id="trvlClm.eligibleAmt"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.eligibleAmt")%></label></strong>
								</td>
								<td class="formth" width="15%"><strong><label
									class="set" name="trvlClm.expAmt" id="trvlClm.expAmt"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.expAmt")%></label></strong>
								</td>
								<td class="formth" width="5%"><strong><label
									class="set" name="trvlClm.isPolicyViolated" id="trvlClm.isPolicyViolated"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.isPolicyViolated")%></label></strong>
								</td>
								<td class="formth" width="5%"><strong><label
								...
								<td class="formth" width="10%"><strong><label
									class="set" name="trvlClm.Proof" id="trvlClm.Proof"
									ondblclick="callShowDiv(this);"><%=label.get("trvlClm.Proof")%></label></strong>
								</td>
							</tr>
							<%!int i = 0;%>
							<%
							...
							<s:iterator value="expDtls">
								<tr>
									...
									<td class="sortableTD" nowrap="nowrap" width="10%"><s:property value="expDate" />&nbsp;</td>
									<td class="sortableTD" width="20%"><s:property value="expName" />&nbsp;</td>
									...
									-->
									<td class="sortableTD" width="20%"><s:property value="expParticlrs" />

									<input type="hidden" name="descCntAppl<%=k%>"
										id="paraFrm_descCntAppl<%=k%>"> &nbsp;</td>
									<td class="sortableTD" align="right" width="10%"><s:property
									...
									<td class="sortableTD" width="5%"><s:property
										value="isPolicyViolated" />&nbsp;</td>
										
									<td class="sortableTD" nowrap="nowrap" width="5%"><s:property
										value="expIsProof" />&nbsp;</td>
									<s:if test="(expIsProof == 'NO')">
										<td class="sortableTD" width="10%"><s:hidden name="expProofPath" />
										&nbsp;</td>
									</s:if>
									<s:else>
										<td class="sortableTD" width="10%"><s:hidden name="expProofPath" /><s:iterator
	...
	function showRecord(fileName)
{
	...
	function callFun(stat){
		...
		var confr=confirm('Do you want to '+disMsg+' the application?');
			if(confr){
				document.getElementById('paraFrm').target ="main";	 		
	  			document.getElementById('paraFrm').action = "TravelClmAdminAppvr_doFun.action";
				document.getElementById('paraFrm').submit();
				return true;		
			}else{
				return false;	
		...
		{    	
		 document.getElementById('paraFrm').action="TravelClmAdminAppvr_input.action";
  		 document.getElementById('paraFrm').submit();
        var status=document.getElementById('paraFrm_navStatus').value;				
		document.getElementById('paraFrm').action = "TravelClmAdminAppvr_getApprovedList.action";  
		document.getElementById('paraFrm').submit();  
		}
		...
		{    	
		 document.getElementById('paraFrm').action="TravelClmAdminAppvr_input.action";
  		 document.getElementById('paraFrm').submit();
        var status=document.getElementById('paraFrm_navStatus').value;				
		document.getElementById('paraFrm').action = "TravelClmAdminAppvr_getClosedList.action";  
		document.getElementById('paraFrm').submit();  
		}
	...
	{
	 	document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = "TravelClmAdminAppvr_viewAttachedProof.action?fileName="+fileName;
		document.getElementById('paraFrm').submit();
		document.getElementById('paraFrm').target = "main";
	}
	...
	function viewSplitExpenditure(){
		...
		document.getElementById("paraFrm").target="new";
	 	document.getElementById("paraFrm").action='TravelClmAdminAppvr_viewSplittedExpenditure.action'; 
	  	document.getElementById("paraFrm").submit();
	}
	function viewbookingdetailsFun(){
		try{
			var applicationId=document.getElementById('paraFrm_tmsTrvlId').value;
	...
	function viewApplication(){
		try{
			...
			win=window.open('','win','top=260,left=250,width=900,height=600,scrollbars=yes,status=no,resizable=yes');
			document.getElementById("paraFrm").target="win";
			document.getElementById("paraFrm").action="TravelApplication_viewApplication.action?applicationId="+applicationId;
			document.getElementById("paraFrm").submit();	
			document.getElementById("paraFrm").target="main";
		}catch(e){} 
	} 
