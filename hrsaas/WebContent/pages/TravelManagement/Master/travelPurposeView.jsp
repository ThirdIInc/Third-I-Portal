...
<s:form action="TravelPurpose_input" validate="true" id="paraFrm"
	...
	<table width="100%" border="0" align="right" cellpadding="1"
		...
		<tr>
			<td colspan="3"><s:if test="onLoadFlag">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<table width="100%" border="0" align="center" cellpadding="0"
							...
							<tr>
								<td width="22%" height="22" class="formtext" colspan="1" nowrap="nowrap"><label
									class="set" id="trv.name" name="trv.name"
									ondblclick="callShowDiv(this);"><%=label.get("trv.name")%></label>
								:</td>
								<td width="20%" height="22" colspan="3"><s:hidden
									name="purposeCode" /> <s:hidden name="purposeName" /> <s:property
									value="purposeName" />
							</tr>
							<tr>
								<td valign="top" height="22" class="formtext" width="22%" nowrap="nowrap"
									colspan="1"><label class="set" id="desc" name="desc"
									ondblclick="callShowDiv(this);"><%=label.get("desc")%></label>
								:</td>
								<td colspan="3" width="70%" style="overflow: hidden;">
								
								<s:textarea  readonly="true"  cssStyle="border:none;overflow: hidden;" name="Description" cols="75" rows="5"/>
							...
							<tr>
								<td height="22" class="formtext" width="22%" colspan="1" nowrap="nowrap"><label
									class="set" id="status" name="status"
									ondblclick="callShowDiv(this);"><%=label.get("status")%></label>
								:</td>
								<td colspan="3" width="20%"><s:hidden name="Status" /><s:property
									value="Status" /></td>
				...
				<s:hidden name="purposeName" />
				<s:hidden name="Status" />
				<s:hidden name="purposeCode" />
					...
					<table width="100%" border="0" cellpadding="1" cellspacing="1"
						...
						<tr class="td_bottom_border">
							<td height="15" class="formhead" nowrap="nowrap" colspan="3"><strong
								class="forminnerhead"><label class="set" id="trv.list"
								name="trv.list" ondblclick="callShowDiv(this);"><%=label.get("trv.list")%></label></strong></td>
								<td align="center"><input type="button" class="delete"  theme="simple" class="formhead"
						...
						<tr>
							...
							<td class="formth" align="center"><strong><label
								class="set" id="trv.name1" name="trv.name"
								ondblclick="callShowDiv(this);"><%=label.get("trv.name")%></label></strong></td>

							<td class="formth" align="center"><strong><label
								class="set" id="status3" name="status"
							...
							<s:iterator value="purposeList">
								...
								<tr id="tr1" <%if(count%2==0){
									...
									ondblclick="javascript:callForEdit('<s:property  value="purposeCode"/>');">
									...
									++i;
									%>
									<s:hidden value="%{purposeCode}" id='<%="purposeCode" + i%>'></s:hidden>
									</td>
									
									<script type="text/javascript">
										records[<%=i%>] = document.getElementById('purposeCode' + '<%=i%>').value;
									</script>


									<td width="40%" align="left" class="sortableTD"><s:hidden
										value="%{purposeCode}"></s:hidden> <input type="hidden"
										name="hdeleteCode" id="hdeleteCode<%=i%>" /> <s:property
										value="purposeName" />&nbsp;</td>
									...
									<td align="center" nowrap="nowrap" class="sortableTD"><input
										...
										name="confChk"
										onclick="callForDelete1('<s:property  value="purposeCode"/>','<%=i%>')" /></td>
								</tr>
...
var fieldName = ["paraFrm_purposeName"];
var lableName = ["trv.name"];
var type = ['enter'];
...
/*function updateFun()
{
	document.getElementById('paraFrm').action="TravelPurpose_update.action";
	document.getElementById('paraFrm').submit();
}*/
...
function cancelFun(){
     	document.getElementById("paraFrm").action="TravelPurpose_cancelSec.action";
	    document.getElementById("paraFrm").submit();
...
alert("callPage");
	    ...
	    document.getElementById('paraFrm').action="TravelPurpose_callPage2.action";
	   	document.getElementById('paraFrm').submit();
}*/
function callPage(id){
		if(id=='P'){
		...
		document.getElementById('myPage').value=id;
		document.getElementById('paraFrm_show').value=id;
		document.getElementById('paraFrm').action='TravelPurpose_callPageView.action';
		document.getElementById('paraFrm').submit();
...
function callForEdit(id){
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	...
	   	document.getElementById("paraFrm").action="TravelPurpose_calforedit.action";
	   	document.getElementById("paraFrm").target="main";
	    document.getElementById("paraFrm").submit();
}
...
function chkDelete(){
	 ...
	 var con=confirm('Do you really want to delete this record ? ');
	 if(con){
	   document.getElementById('paraFrm').action="TravelPurpose_delete1.action";
	    document.getElementById('paraFrm').submit();
	    }
	    else
	    {
