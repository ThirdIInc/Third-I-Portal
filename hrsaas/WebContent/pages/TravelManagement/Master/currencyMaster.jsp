...
<s:form action="Currency" id="paraFrm" validate="true" target="main"
	theme="simple">
	<s:hidden name="show" value="%{show}" />
	<s:hidden name="hiddenCode"  />
	<s:hidden name="curCode" />
	<s:hidden name="description"/>
	<s:hidden name="curStatus"/>
	<s:hidden name="curName"/>
	<s:hidden name="curAbbr"/>
...
<table width="100%" border="0"  class="formbg" align="right">
							...
							<table  width="100%" border="0" align="right" cellpadding="0" cellspacing="0" class="formbg" >
							    ...
							    <tr>
									  <td>
								    	 ...
								    	 <td width="93%" class="txt"><strong class="text_head">Currency </strong></td>
				...
				<table width="100%">
					<tr>
						...
						<td id="ctrlShow" width="30%" align="right" class="">
							...
							%>
							<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'Currency_callPage.action');">
								<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
							</a>&nbsp;
							<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'Currency_callPage.action');" >
								<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
							</a> 
							<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'Currency_callPage.action');return numbersOnly();" /> of <%=totalPage%>
							<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'Currency_callPage.action')" >
								<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
							</a>&nbsp;
							<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'Currency_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Currency </strong></td>		
						<td  align="right">
							...
							class="formbg">
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1"
									...
									<tr class="td_bottom_border">
										...
										<td class="formth" align="center"><label name="currencyname"
											id="currencyname" ondblclick="callShowDiv(this);"><%=label.get("currencyname")%></label></td>
										
										 
										<td class="formth" align="center"><label name="currencyabbr"
											id="currencyabbr" ondblclick="callShowDiv(this);"><%=label.get("currencyabbr")%></label></td>
									...
									<%int count=0; %>
										...
										<tr  class="sortableTD"
											<%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="curCode"/>');">
			
			
												<td width="10%" align="center" class="sortableTD"><%=++cn%><%++i;%><s:hidden name="srNo" /></td>
												<s:hidden value="%{curCode}" id='<%="curCode" + i%>'></s:hidden>
			                                      <script type="text/javascript">
													records[<%=i%>] = document.getElementById('curCode' + '<%=i%>').value;
												</script>
												<td width="40%" align="left" class="sortableTD"><s:property
													value="curName" /> <input type="hidden"
												...
												<td  id="ctrlShow" align="center" nowrap="nowrap" class="sortableTD"><input type="checkbox"
													...
													onclick="callForDelete1('<s:property value="curCode"/>','<%=i%>')" /></td>
		...
		<tr><td width="100%"><table width="100%">
			...
			<td width="21%" align="right"><s:if test="listLength"><b>Total No. Of Records:</b>&nbsp;<s:property value="totalRecords" /></s:if></td>
		</table></td>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'Currency_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='Currency_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'Currency_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	function newRowColor(cell) {
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddenCode').value=id;
	   	document.getElementById("paraFrm").action="Currency_callforedit.action";
	   	document.getElementById('paraFrm').target = "_self";
	    document.getElementById("paraFrm").submit();
   }
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="Currency_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
...
function chk(){
  ...
  // alert ('x');
		...
		document.getElementById("paraFrm").target = 'myWin';
		document.getElementById("paraFrm").action = 'Currency_'+action+'.action';
		document.getElementById("paraFrm").submit();
	}
