...
<s:form action="HelpDeskReqType" validate="true" id="paraFrm"
	...
	<s:hidden name="reqTypeDept" />
	<s:hidden name="reqTypeDeptId" />
							...
							<table width="100%" border="0" cellpadding="2" cellspacing="2">
								...
								<s:if test="modeLength">
									<tr>
										...
										%> <a href="#"
											onclick="callPage('1', 'F', '<%=totalPage%>', 'HelpDeskReqType_callPage.action');">
										<img title="First Page" src="../pages/common/img/first.gif"
											width="10" height="10" class="iconImage" /> </a>&nbsp; <a
											href="#"
											onclick="callPage('P', 'P', '<%=totalPage%>', 'HelpDeskReqType_callPage.action');">
										<img title="Previous Page"
											src="../pages/common/img/previous.gif" width="10" height="10"
											class="iconImage" /> </a> <input type="text" name="pageNoField"
											id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
											onkeypress="callPageText(event, '<%=totalPage%>', 'HelpDeskReqType_callPage.action');return numbersOnly();" />
										of <%=totalPage%> <a href="#"
											onclick="callPage('N', 'N', '<%=totalPage%>', 'HelpDeskReqType_callPage.action')">
										<img title="Next Page" src="../pages/common/img/next.gif"
											class="iconImage" /> </a>&nbsp; <a href="#"
											onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'HelpDeskReqType_callPage.action');">
						...
						<tr>
							...
							<table class="formbg" width="100%" cellspacing="0"
								...
								<tr>
									...
									Request Category List</strong></td>
								...
								<tr>
									...
									%>
																...
																<td width="10%" align="center" class="sortableTD"
																	...
																	ondblclick="javascript:callForEdit('<s:property  value="reqTypeCodeItr"/>');">
																<%=++cnt%> <%
 ++i;
 %>
																</td>
																<td width="30%" align="left" class="sortableTD"
																	title="Double click for edit"
																	ondblclick="javascript:callForEdit('<s:property  value="reqTypeCodeItr"/>');">
																<s:hidden value="%{reqTypeCodeItr}"
																	id='<%="reqTypeCodeItr" + i%>'>
																</s:hidden> <script type="text/javascript">
										records[<%=i%>] = document.getElementById('reqTypeCodeItr' + '<%=i%>').value;
									</script> <input type="hidden" name="hdeleteCode" id="hdeleteCode" /> <s:property
																	value="reqTypeItr" /></td>
																<td width="30%" align="left" class="sortableTD"
																	title="Double click for edit"
																	ondblclick="javascript:callForEdit('<s:property  value="reqTypeCodeItr"/>');">
																<s:property value="reqTypeDeptItr" /><input
																	type="hidden" name="reqTypeDeptIdItr"
																	id="reqTypeDeptIdItr<%=i%>" /></td>
																<td width="10%" align="center" class="sortableTD"	>
																...
																<td width="10%" id="ctrlShow" align="center"
																	...
																	name="confChk"
																	onclick="callForDelete1('<s:property  value="reqTypeCodeItr"/>','<%=i%>')" /></td>
															</tr>
...
<script>
	...
	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'HelpDeskReqType_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='HelpDeskReqType_f9Action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'HelpDeskReqType_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="HelpDeskReqType_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="HelpDeskReqType_deleteList.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
