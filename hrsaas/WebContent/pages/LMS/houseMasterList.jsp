...
<s:form action="HouseMaster" id="paraFrm" theme="simple">
	...
	<s:hidden name="houseAddress"></s:hidden>
	<s:hidden name="NoFamily"></s:hidden>
	<s:hidden name="NoIndividual"></s:hidden>
 ...
 %>
					...
					%> <a href="#"
						onclick="callPage('1', 'F', '<%=totalPage%>', 'HouseMaster_callPage.action');">
					<img title="First Page" src="../pages/common/img/first.gif"
						width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('P', 'P', '<%=totalPage%>', 'HouseMaster_callPage.action');">
					<img title="Previous Page" src="../pages/common/img/previous.gif"
						width="10" height="10" class="iconImage" /> </a> <input type="text"
						name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
						maxlength="10"
						onkeypress="callPageText(event, '<%=totalPage%>', 'HouseMaster_callPage.action');return numbersOnly();" />
					of <%=totalPage%> <a href="#"
						onclick="callPage('N', 'N', '<%=totalPage%>', 'ColonyMaster_callPage.action')">
					<img title="Next Page" src="../pages/common/img/next.gif"
						class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'HouseMaster_callPage.action');">
		...
		<tr>
			...
			<table class="formbg" width="100%" cellspacing="0" cellspacing="1"
				border="0">
				<tr>
					<td colspan="2" class="txt"><strong class="text_head">House
					Master </strong></td>
				...
				<table width="100%" border="0" cellpadding="1" cellspacing="1">
					...
					<tr>
						...
						<td class="formth" align="center"><label class="set"
							name="house.No" id="house.No" ondblclick="callShowDiv(this);"><%=label.get("house.No")%></label></td>
						<td class="formth" align="center"><label class="set"
							name="colony.Name" id="colony.Name"
							ondblclick="callShowDiv(this);"><%=label.get("colony.Name")%></label></td>
						<td class="formth" align="center"><label class="set"
							name="house.Address" id="house.Address"
							ondblclick="callShowDiv(this);"><%=label.get("house.Address")%></label></td>
					...
					<s:iterator value="HouseMasterItt">
						<tr
							ondblclick="javascript:callForEdit('<s:property  value="ittHouseId"/>');">
...
<script>
  	...
  	function addnewFun(){
  	    ...
  	    document.getElementById('paraFrm').target = "_self";
   	    document.getElementById('paraFrm').action="HouseMaster_addNew.action";
	    document.getElementById('paraFrm').submit();
  		...
  		function searchFun(){
			...
			document.getElementById("paraFrm").action ='HouseMaster_searchHouse.action';
			document.getElementById("paraFrm").submit();
  	}
	 ...
	 if(con){
		document.getElementById('paraFrm').target = "_self";
      	document.getElementById('paraFrm').action = 'HouseMaster_deleteCheck.action';
		document.getElementById('paraFrm').submit();
	}
	}
...
if(document.getElementById('confChk' + i).checked == true ) {
   ...
   document.getElementById('confChk' + i).checked =false;
	...
	function callForEdit(id){
	   document.getElementById('paraFrm_houseId').value=id;
  	   document.getElementById('paraFrm').target = "_self";
   	   document.getElementById('paraFrm').action="HouseMaster_dblClickItt.action";
	   document.getElementById('paraFrm').submit();
	   return false;
  	}
  	</script>