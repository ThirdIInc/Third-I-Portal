			...
			<table class="formbg" width="100%" cellspacing="1" cellspadding="0"
				...
				<tr>
					<td colspan="2" class="txt" width="85%"><strong
						class="text_head"> Trade Union / Work Council List </strong></td>
						...
						<table width="100%" border="0">
							...
							<tr>
								...
								<td class="formth" align="center"><label class="set"
									...
									ondblclick="callShowDiv(this);"><%=label.get("committee.Name")%></label></td>
								...
								<td class="formth" align="center"><label class="set"
									name="name.of.leader" id="name.of.leader"
									ondblclick="callShowDiv(this);"><%=label.get("name.of.leader")%></label></td>

								<td class="formth" align="center"><label class="set"
									name="bargaining.agreement" id="bargaining.agreement"
									ondblclick="callShowDiv(this);"><%=label.get("bargaining.agreement")%></label></td>

								<td class="formth" align="center"><label class="set"
									name="grievance.procedure" id="grievance.procedure"
									ondblclick="callShowDiv(this);"><%=label.get("grievance.procedure")%></label></td>
							...
							<s:if test="modeLength">
								...
								<s:iterator value="tradeUnionWorkCouncilItt">
									...
									%> class="tableCell1"
										...
										ondblclick="javascript:callForEdit('<s:property value="tradeUnionWorkCouncilID"/>');">
										<td align="center" class="sortableTD"><%=++i%></td>

										<s:hidden value="%{tradeUnionWorkCouncilID}"
											id='<%="tradeUnionWorkCouncilID" + i%>' />
										<script type="text/javascript">records[<%=i%>] = document.getElementById('tradeUnionWorkCouncilID' + '<%=i%>').value;</script>


										<td align="left" class="sortableTD">&nbsp; <s:property
											value="committeeName" /><s:hidden
											name="tradeUnionWorkCouncilID" /> <input type="hidden"
											name="hiddenCode"
											id="hiddenCode<%=i%>" /></td>
										...
										<td align="left" class="sortableTD">&nbsp; <s:property
											value="bargainingAgreement" /></td>

										<td align="left" class="sortableTD">&nbsp; <s:property
											value="procedureGrievance" /></td>

										<td align="center" nowrap="nowrap" class="sortableTD"><input
											type="checkbox" class="checkbox" id="confChk<%=i%>"
											name="confChk"
											onclick="callForDelete('<s:property value="tradeUnionWorkCouncilID"/>','<%=i%>')" />
										</td>
									</tr>
								</s:iterator>
...
function addnewFun()
{
  	    document.getElementById('paraFrm').target = "_self";
   	    document.getElementById('paraFrm').action="TradeUnionWorkCouncil_addNew.action";
...
{	 
	 ...
	 {	
	 	...
	 	if(con)
	 	{
	   		document.getElementById('paraFrm').action="TradeUnionWorkCouncil_deleteChkRecords.action";
	   		document.getElementById('paraFrm').target = "_self";
	    	document.getElementById('paraFrm').submit();
	    }
	    else
	    { 
	    	...
	    	var flag='<%=d %>';
	  		...
	  		{	
				...
				document.getElementById('tradeUnionWorkCouncilID'+a).value="";	
			}
	     return false;
	 	}
	 }
...
{	
	document.getElementById('paraFrm_tradeUnionWorkCouncilID').value=id;
	//alert('Trade Union ID = '+ document.getElementById('paraFrm_tradeUnionWorkCouncilID').value);
  	
	document.getElementById("paraFrm").action="TradeUnionWorkCouncil_calforedit.action?id="+id;
	document.getElementById('paraFrm').target = "_self";
	document.getElementById("paraFrm").submit();
}
...
function selectAllRecords(object) 
{
	if(object.checked) 
	{
		for(var i = 1; i <= records.length; i++) 
		{
				...
				document.getElementById('tradeUnionWorkCouncilID' + i).value = records[i];
		}
	}
	else 
	{
		for(var i = 1; i <= records.length; i++) 
		{
			...
			document.getElementById('tradeUnionWorkCouncilID' + i).value = "";
		}
	}
}
