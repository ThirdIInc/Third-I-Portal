			...
			<table width="100%" border="0" align="right" cellpadding="0"
				...
				<tr>
					<td><strong class="text_head"> <img
						...
						height="25" /> </strong></td>
					<td width="93%" class="txt"><strong class="text_head">Rules And Policy Master List </strong></td>
						...
						<table width="100%" border="0">
							...
							<tr>
								...
								<td class="formth" align="center"><label class="set"
									name="type.of.category" id="type.of.category"
									ondblclick="callShowDiv(this);"><%=label.get("type.of.category")%></label></td>
							...
							<s:if test="modeLength">
								...
								<s:iterator value="rulesAndPolicyList">
									...
									%> class="tableCell1"
										...
										ondblclick="javascript:callForEdit('<s:property value="rulesAndPolicyID"/>');">
										<td align="center" class="sortableTD"><%=++i%></td>

										<s:hidden value="%{rulesAndPolicyID}"
											id='<%="rulesAndPolicyID" + i%>' />
										<script type="text/javascript">records[<%=i%>] = document.getElementById('rulesAndPolicyID' + '<%=i%>').value;</script>


										<td align="left" class="sortableTD">&nbsp; <s:property
											value="categoryType" /><s:hidden
											name="rulesAndPolicyID" />  <input type="hidden"
											name="hiddenCode"
											id="hiddenCode<%=i%>" /></td>
										...
										<td align="center" nowrap="nowrap" class="sortableTD"><input
											...
											name="confChk"
											onclick="callForDelete('<s:property value="rulesAndPolicyID"/>','<%=i%>')" />
										</td>
									</tr>
								</s:iterator>
...
function addnewFun()
{
  	    document.getElementById('paraFrm').target = "_self";
   	    document.getElementById('paraFrm').action="RulesAndPolicyMaster_addNew.action";
...
function searchFun()
{
	...
	document.getElementById("paraFrm").action ='RulesAndPolicyMaster_f9searchCustomer.action';
	document.getElementById("paraFrm").submit();
}
...
{	 
	 ...
	 {	
	 	...
	 	if(con)
	 	{
	   		document.getElementById('paraFrm').action="RulesAndPolicyMaster_deleteChkRecords.action";
	   		document.getElementById('paraFrm').target = "_self";
	    	document.getElementById('paraFrm').submit();
	    }
	    else
	    { 
	    	...
	    	var flag='<%=d %>';
	  		...
	  		{	
				...
				document.getElementById('rulesAndPolicyID'+a).value="";	
			}
	     return false;
	 	}
	 }
...
{	
	document.getElementById('paraFrm_rulesAndPolicyID').value=id;
	//alert('Rules & Policy ID = '+ document.getElementById('paraFrm_rulesAndPolicyID').value);
  	
	document.getElementById("paraFrm").action="RulesAndPolicyMaster_calforedit.action?id="+id;
	document.getElementById('paraFrm').target = "_self";
	document.getElementById("paraFrm").submit();
}
...
function selectAllRecords(object) 
{
	if(object.checked) 
	{
		for(var i = 1; i <= records.length; i++) 
		{
				...
				document.getElementById('rulesAndPolicyID' + i).value = records[i];
		}
	}
	else 
	{
		for(var i = 1; i <= records.length; i++) 
		{
			...
			document.getElementById('rulesAndPolicyID' + i).value = "";
		}
	}
}
