...
<s:form action="WareHouseMaster" id="paraFrm" validate="true"
	target="main" theme="simple">

	<s:hidden name="show" value="%{show}" />
	<s:hidden name="hiddencode" />
	<s:hidden name="wareHousecode" />
	<s:hidden name="wareHouseName"/>
	<s:hidden name="respName"/>
	<s:hidden name="respToken"/>
			...
			<table width="100%" border="0" align="right" cellpadding="0"
				...
				<tr>
					<td><strong class="text_head"><img
						...
						height="25" /></strong></td>
					<td width="93%" class="txt"><strong class="text_head">Warehouse </strong></td>
			...
			<td colspan="3" width="100%">
				<table width="100%" align="left" cellspacing="0" cellpadding="0">
					...
					<s:if test="recordsLength">
							...
							%> <a href="#"
								onclick="callPage('1', 'F', '<%=totalPage%>', 'WareHouseMaster_input.action');">
							<img title="First Page" src="../pages/common/img/first.gif"
								width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
								onclick="callPage('P', 'P', '<%=totalPage%>', 'WareHouseMaster_input.action');">
							<img title="Previous Page" src="../pages/common/img/previous.gif"
								width="10" height="10" class="iconImage" /> </a> <input type="text"
								name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
								maxlength="10"
								onkeypress="callPageText(event, '<%=totalPage%>', 'WareHouseMaster_input.action');return numbersOnly();" />
							of <%=totalPage%> <a href="#"
								onclick="callPage('N', 'N', '<%=totalPage%>', 'WareHouseMaster_input.action')">
							<img title="Next Page" src="../pages/common/img/next.gif"
								class="iconImage" /> </a>&nbsp; <a href="#"
								onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'WareHouseMaster_input.action');">
		...
		<tr>
			<td colspan="3" width="100%">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formbg">
					<tr>
						<td class="txt" colspan="2"><strong class="text_head">Warehouse </strong></td>
							...
							<tr>
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1">
									<tr class="td_bottom_border">
										...
										<label  class = "set" name="warehouse.srno" id="warehouse.srno" ondblclick="callShowDiv(this);"><%=label.get("warehouse.srno")%></label>
										</td>
										<td class="formth" align="center">
										<label  class = "set" name="warehouse.name" id="warehouse.name" ondblclick="callShowDiv(this);"><%=label.get("warehouse.name")%></label>
										</td>
			
										<td class="formth" align="center">
										<label  class = "set" name="warehouse.resperson" id="warehouse.resperson" ondblclick="callShowDiv(this);"><%=label.get("warehouse.resperson")%></label>
									...
									<s:if test="recordsLength">
										...
										<s:iterator value="recordList">
											<tr class="sortableTD" <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="wareHousecode" />');">
												...
												<s:hidden value="%{wareHousecode}" id='<%="wareHousecode" + i%>'></s:hidden>
												</td>
													<td width="30%" class="sortableTD" align="left"><s:property
													value="respName" /></td>									
												<script type="text/javascript">												
													records[<%=i-1%>] = document.getElementById('wareHousecode' + '<%=i%>').value;
												</script>
											...
											<td width="40%" align="left" class="sortableTD"><s:property
												...
												<td align="center" nowrap="nowrap" class="sortableTD" id="ctrlShow"  ><input type="checkbox"
													...
													onclick="callForDelete1('<s:property value="wareHousecode" />','<%=i%>')" />
													</td>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'WareHouseMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='WareHouseMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'WareHouseMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	...
	  	document.getElementById('paraFrm_wareHousecode').value=id;
	   	document.getElementById("paraFrm").action="WareHouseMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="WareHouseMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
