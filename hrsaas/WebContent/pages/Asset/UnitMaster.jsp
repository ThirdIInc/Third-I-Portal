...
<s:form action="UnitMaster" id="paraFrm" validate="true"
			...
			<td colspan="3" width="100%">
				<table width="100%" align="left" cellspacing="0" cellpadding="0">
					...
					<s:if test="recordsLength">
							...
							%> <a href="#"
								onclick="callPage('1', 'F', '<%=totalPage%>', 'UnitMaster_callPage.action');">
							<img title="First Page" src="../pages/common/img/first.gif"
								width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
								onclick="callPage('P', 'P', '<%=totalPage%>', 'UnitMaster_callPage.action');">
							<img title="Previous Page" src="../pages/common/img/previous.gif"
								width="10" height="10" class="iconImage" /> </a> <input type="text"
								name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
								maxlength="10"
								onkeypress="callPageText(event, '<%=totalPage%>', 'UnitMaster_callPage.action');return numbersOnly();" />
							of <%=totalPage%> <a href="#"
								onclick="callPage('N', 'N', '<%=totalPage%>', 'UnitMaster_callPage.action')">
							<img title="Next Page" src="../pages/common/img/next.gif"
								class="iconImage" /> </a>&nbsp; <a href="#"
								onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'UnitMaster_callPage.action');">
		...
		<tr>
			<td colspan="3" width="100%">
				<table width="100%" border="0" cellpadding="0" cellspacing="1" class="formbg">
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Unit Master</strong></td>
						<td align="right" width="10%"><input theme="simple"
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1">
									<tr class="td_bottom_border">
										...
										<td class="formth" align="center">
											<label  class = "set" name="unit.srno" id="unit.srno" ondblclick="callShowDiv(this);"><%=label.get("unit.srno")%></label>
							
																</td>
										<td class="formth" align="center">
										<label  class = "set" name="unit.name" id="unit.name1" ondblclick="callShowDiv(this);"><%=label.get("unit.name")%></label>					
								</td>
									...
									<s:if test="recordsLength">
										...
										<s:iterator value="recordList">
											<tr class="sortableTD" <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="unitCode" />');">
												...
												<s:hidden value="%{unitCode}" id='<%="unitCode" + i%>'></s:hidden>
												</td>																	
												<script type="text/javascript">												
													records[<%=i-1%>] = document.getElementById('unitCode' + '<%=i%>').value;
												</script>
												...
												<td align="center" nowrap="nowrap" class="sortableTD" id="ctrlShow"  ><input type="checkbox"
													...
													onclick="callForDelete1('<s:property value="unitCode" />','<%=i%>')" />
													</td>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'UnitMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='UnitMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'UnitMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="UnitMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="UnitMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
