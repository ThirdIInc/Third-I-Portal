...
<s:form action="VenueMaster" id="paraFrm" validate="true" target="main"
	...
	<s:hidden name="venueMaster.venueName" />
	<s:hidden name="branchName" />
	<s:hidden name="resPersonName" />
	<s:hidden name="branchCode" />
	<s:hidden name="resPersonCode" />
	<s:hidden name="resPersonToken" />
	...
	<table width="100%" border="0" class="formbg" align="right">
				...
				<tr>
					...
					<s:if test="listLength">
						<td id="ctrlShow" width="30%" align="right"><b>Page:</b> <%
							...
							%> <a href="#"
							onclick="callPage('1', 'F', '<%=totalPage%>', 'VenueMaster_callPage.action');">
						<img title="First Page" src="../pages/common/img/first.gif"
							width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('P', 'P', '<%=totalPage%>', 'VenueMaster_callPage.action');">
						<img title="Previous Page" src="../pages/common/img/previous.gif"
							width="10" height="10" class="iconImage" /> </a> <input type="text"
							name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
							maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'VenueMaster_callPage.action');return numbersOnly();" />
						of <%=totalPage%> <a href="#"
							onclick="callPage('N', 'N', '<%=totalPage%>', 'VenueMaster_callPage.action')">
						<img title="Next Page" src="../pages/common/img/next.gif"
							class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'VenueMaster_callPage.action');">
		...
		<tr>
			<td colspan="3">
			<table class="formbg" width="100%" cellspacing="0" cellpadding="0"
				border="0">
				<tr>
					<td colspan="2" class="txt"><strong class="text_head">Conference
					Room</strong></td>
					<td align="right"><input type="button" id="ctrlShow"
						class="delete" value="  Delete" onclick="return chkDelete();" />
					</td>
				</tr>
				<tr>
					<td colspan="3">
							...
							<table width="100%" border="0" cellpadding="1" cellspacing="1">
								<tr class="td_bottom_border">
									...
									<td class="formth" align="center"><label class="set"
										name="conference.srno" id="conference.srno"
										ondblclick="callShowDiv(this);"><%=label.get("conference.srno")%></label>

									</td>
									<td class="formth" align="center"><label class="set"
										name="conference.name" id="conference.name1"
									...
									<td class="formth" align="center"><label class="set"
										name="respPerson" id="conference.respPerson1"
								...
								<s:if test="listLength">
									...
									<s:iterator value="recordList">
										<tr class="sortableTD" <%if(count%2==0){
											...
											ondblclick="javascript:callForEdit('<s:property value="venueCode" />');">
											...
											<td class="sortableTD" width="30%" nowrap="nowrap"
												...
												id="hdeleteCode<%=i%>" /> <s:property value="venueName" />
											<s:hidden value="%{venueCode}" id='<%="venueCode" + i%>'></s:hidden>
											</td>
											<td class="sortableTD" width="20%" nowrap="nowrap"
												align="left"><s:property value="branchName" /></td>
											<td class="sortableTD" width="30%" nowrap="nowrap"
												align="left"><s:property value="resPersonName" />
											</td>
											<script type="text/javascript">												
													records[<%=i-1%>] = document.getElementById('venueCode' + '<%=i%>').value;
												</script>
											<td align="center" nowrap="nowrap" class="sortableTD"
												id="ctrlShow"><input type="checkbox" class="checkbox"
												id="confChk<%=i%>" name="confChk"
												onclick="callForDelete1('<s:property value="venueCode" />','<%=i%>')" />
											</td>
										</tr>
									</s:iterator>
									<%
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'VenueMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='VenueMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'VenueMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="VenueMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="VenueMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
