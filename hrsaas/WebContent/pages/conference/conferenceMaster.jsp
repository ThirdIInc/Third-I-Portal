...
<s:form action="ConferenceMaster" id="paraFrm" validate="true"
	target="main" theme="simple">

	<s:hidden name="show" value="%{show}" />
	<s:hidden name="hiddencode" />
	<s:hidden name="confMaster.accessCode" />
	<s:hidden name="confMaster.accessoryName"/>
	<s:hidden name="myPage" id="myPage" />
	<s:hidden name="confMaster.resPersonCode"/>
	<s:hidden name="confMaster.resPersonToken"/>
	<s:hidden name="confMaster.resPersonName"/>
	<table width="100%" border="0" class="formbg" align="right">
		...
		<s:if test="listLength">
				...
				%> <a href="#"
					onclick="callPage('1', 'F', '<%=totalPage%>', 'ConferenceMaster_callPage.action');">
				<img title="First Page" src="../pages/common/img/first.gif"
					width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
					onclick="callPage('P', 'P', '<%=totalPage%>', 'ConferenceMaster_callPage.action');">
				<img title="Previous Page" src="../pages/common/img/previous.gif"
					width="10" height="10" class="iconImage" /> </a> <input type="text"
					name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
					maxlength="10"
					onkeypress="callPageText(event, '<%=totalPage%>', 'ConferenceMaster_callPage.action');return numbersOnly();" />
				of <%=totalPage%> <a href="#"
					onclick="callPage('N', 'N', '<%=totalPage%>', 'ConferenceMaster_callPage.action')">
				<img title="Next Page" src="../pages/common/img/next.gif"
					class="iconImage" /> </a>&nbsp; <a href="#"
					onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'ConferenceMaster_callPage.action');">
		...
		<tr>
			<td colspan="3">
				<table class="formbg" width="100%" cellspacing="0" cellpadding="0" border="0">
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Conference
						Accessories</strong></td>
							...
							<tr>
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1">
									<tr class="td_bottom_border">
										...
										<label class="set"
											name="conf.srno" id="conf.srno" ondblclick="callShowDiv(this);"><%=label.get("conf.srno")%></label>
																</td>
										<td class="formth" align="center"><label class="set"
											name="conf.accessname" id="conf.accessname1"
									...
									<s:if test="listLength">
										...
										<s:iterator value="recordList">
											<tr class="sortableTD" <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="accessCode" />');">
												...
												<td class="sortableTD" width="45%" nowrap="nowrap" align="left">
												<input	type="hidden" name="hdeleteCode" id="hdeleteCode<%=i%>" />
													<s:property value="accessoryName" />
												<s:hidden value="%{accessCode}" id='<%="accessCode" + i%>'></s:hidden>
												</td>
																						
												<script type="text/javascript">												
													records[<%=i-1%>] = document.getElementById('accessCode' + '<%=i%>').value;
												</script>
												<td align="center" nowrap="nowrap" class="sortableTD" id="ctrlShow"  ><input type="checkbox"
													class="checkbox" id="confChk<%=i%>" name="confChk"
													onclick="callForDelete1('<s:property value="accessCode" />','<%=i%>')" />
													</td>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'ConferenceMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='ConferenceMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'ConferenceMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="ConferenceMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="ConferenceMaster_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
