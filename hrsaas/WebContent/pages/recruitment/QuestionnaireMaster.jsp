...
<table width="100%" border="0" align="right" class="formbg">
		...
		<tr>
			...
			<strong class="text_head"><img
				...
				height="25" /></strong></td>
			<td width="93%" class="txt"><strong class="text_head">Questionnaire
			  </strong></td>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" >
				...
				<s:if test="modeLength">
						<td id="ctrlShow" align="right" class="">
							...
							%>
							<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'QuestionnaireMaster_callPage.action');">
								<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
							</a>&nbsp;
							<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'QuestionnaireMaster_callPage.action');" >
								<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
							</a> 
							<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'QuestionnaireMaster_callPage.action');return numbersOnly();" /> of <%=totalPage%>
							<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'QuestionnaireMaster_callPage.action')" >
								<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
							</a>&nbsp;
							<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'QuestionnaireMaster_callPage.action');" >
	...
	<tr>
		<td colspan="3" width="100%">
				<table width="100%" border="0" cellpadding="1" cellspacing="0" class="formbg">
					<tr>
						<td class="txt" colspan="2"><strong class="text_head">Questionnaire
						  </strong></td>
						<td align="right">
							...
							<tr>
								...
								<table width="100%" border="0" cellpadding="1" cellspacing="1" class="sortableTD">
									<tr>
										...
										<td class="formth" align="center"><label  class = "set" name="questionname" id="questionname1" ondblclick="callShowDiv(this);"><%=label.get("questionname")%></label></td>
										<td class="formth" align="center"><label  class = "set" name="questiontype" id="questiontype1" ondblclick="callShowDiv(this);"><%=label.get("questiontype")%></label></td>
									...
									<s:if test="modeLength">	
										...
										<s:iterator value="questionlist">
									    	...
									    	title="Double click for edit"
												...
												ondblclick="javascript:callForEdit('<s:property value="quesCode"/>');">
												
												<td width="10%" align="center" class="sortableTD"><%=++cn%><%++i;%></td>
												<s:hidden value="%{quesCode}"  id='<%="quesCode"+i %>' ></s:hidden>
												<script type="text/javascript">
													records[<%=i%>] = document.getElementById('quesCode' + '<%=i%>').value;
												</script>
												...
												<td id="ctrlShow" align="center" nowrap="nowrap" class="sortableTD"><input type="checkbox"
													...
													onclick="callForDelete1('<s:property value="quesCode"/>','<%=i%>');" /></td>
											</tr>
										</s:iterator>
										<%
										d = i;
...
<script>
	...
	function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'QuestionnaireMaster_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='QuestionnaireMaster_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'QuestionnaireMaster_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddenCode').value=id;
	  		document.getElementById('paraFrm').target = "_self";
	   	document.getElementById("paraFrm").action="QuestionnaireMaster_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="QuestionnaireMaster_deletequestion.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
