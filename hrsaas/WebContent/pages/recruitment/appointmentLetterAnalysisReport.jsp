...
<s:form action="AppointLetterAnalysis" method="post" id="paraFrm" theme="simple">  
	           ...
	           <table width="100%" border="0" cellpadding="0" cellspacing="0" >
					<tr>
					   <td width="100%" colspan="3">
					        ...
					        <s:submit cssClass="reset" action="AppointLetterAnalysis_reset" theme="simple" value="    Reset"  /> 
		         ...
		         <table   border="0" cellpadding="0" cellspacing="0" class="formbg" width="100%">
			       ...
			       <table id="filterDisp" width="100%" border="0" cellpadding="2" cellspacing="2" class="formbg"  > 
							...
							<tr>	
						  	...
						  	<tr>
						  		<td >  </td>
						  		<td width="20%" colspan="1" class="formtext"><label  class = "set"  id="appointmentDateLabel"  name="appointmentDateLabel" ondblclick="callShowDiv(this);"><%=label.get("appointmentDateLabel")%></label> : </td>					
								<td width="80%" colspan="2" >
									<s:textfield name="appointmentDate"  size="9" maxlength="10"  onkeypress="return numbersWithHiphen();" onfocus="clearText('paraFrm_appointmentDate','dd-mm-yyyy')" onblur="setText('paraFrm_appointmentDate','dd-mm-yyyy')"  />
									<img src="../pages/images/recruitment/Date.gif"  class="iconImage" height="18" align="absmiddle" width="18" onclick="javascript:NewCal('paraFrm_appointmentDate','DDMMYYYY');">
								</td>
						  	</tr>    
						  	<tr>	 
						  	 <td >  </td>
					   ...
					   <table id="sort"  width="100%" border="0" cellpadding="2" cellspacing="2" class="formbg"> 
							...
							<tr>								
								...
								<td width="83%" colspan="1">  <s:select headerKey="1" headerValue="--Select--" name="sortBy" list="#{'R':'Requisition code','D':'Requisition date', 'V':'Number of Vacanies','U':'Appointment Due','I':'Appointment Issued','A':'Appointment Accepted','J':'Appointment Rejected','C':'Appointment Canceled'}"  />  </td>							
						 	</tr>  
						 	 <tr>								
						 	...
						 	<tr>								
								...
								<td width="83%" colspan="1">  <s:select headerKey="1" headerValue="--Select--" name="thenBy1"  list="#{'R':'Requisition code','D':'Requisition date', 'V':'Number of Vacanies','U':'Appointment Due','I':'Appointment Issued','A':'Appointment Accepted','J':'Appointment Rejected','C':'Appointment Canceled'}"    />    </td>							
						 	</tr>  
						 	<tr>								
						 	 ...
						 	 <tr>								
								...
								<td width="83%" colspan="1">   <s:select headerKey="1" headerValue="--Select--" name="thenBy2" list="#{'R':'Requisition code','D':'Requisition date', 'V':'Number of Vacanies','U':'Appointment Due','I':'Appointment Issued','A':'Appointment Accepted','J':'Appointment Rejected','C':'Appointment Canceled'}"   />  </td>							
						 	  </tr>  
						  	 <tr>								
					...
					<table id="column" width="100%" border="0" cellpadding="2" cellspacing="2" class="formbg"> 
							...
							<tr>								
						        ...
						        <td width="28%" colspan="1" class="formtext"><label  class = "set"  id="reqs.code011"  name="reqs.code" ondblclick="callShowDiv(this);"><%=label.get("reqs.code")%></label> </td>
						  	...
						  	<tr>	
						        ...
						        <td width="28%" colspan="1" class="formtext"><label  class = "set"  id="position1001"  name="position" ondblclick="callShowDiv(this);"><%=label.get("position")%></label> </td>
...
{ 
  ...
  { 
   ...
   var fieldName =["paraFrm_frmDate","paraFrm_toDate","paraFrm_appointmentDate"];
...
if(settingVal=="B")
{
 document.getElementById('paraFrm').action="AppointLetterAnalysis_reset.action";
}else{
 document.getElementById('paraFrm').action="AppointLetterAnalysis_viewSavedRecord.action";
 }
...
{    
	    ...
	    if(reqCode=="")
	       {
		      ...
		      { 
		        ...
		        if(appointmentDate=="dd-mm-yyyy")
		         { 
		          callTab('filtOpt','filter');
		         alert("Please enter/select the "+document.getElementById('appointmentDateLabel').innerHTML.toLowerCase()); 
		          clearText("paraFrm_appointmentDate","dd-mm-yyyy");
			     document.getElementById('paraFrm_appointmentDate').focus();
			     return false; 	 
		        }   
		       if(!validateDate('paraFrm_frmDate', 'reqs.date'))
		        {  return false;
		        }
		       if(!validateDate('paraFrm_appointmentDate', 'appointmentDateLabel'))
		        {  return false;
		        }
		        if(dateFilter=="F")
		         {
    ...
    // for sort the filter   
   	  ...
   	  if(sortBy!="1" &&thenBy1!="1"){
   	    ...
   	    callTab('sortOpt','sort');
   	     ...
   	     document.getElementById('paraFrm_thenBy1').selected='selected';
   	  ...
   	  if(sortBy!="1" &&thenBy2!="1"){
   	    if(sortBy==thenBy2){ 
   	     callTab('sortOpt','sort');
   	     alert("Duplicate sorting option."); 
   	     document.getElementById('paraFrm_thenBy2').selected='selected';
   ...
   document.getElementById('paraFrm').action="AppointLetterAnalysis_save.action";
   document.getElementById('paraFrm').submit();
...
function callF9ReqCode(status) {
   ...
   document.getElementById('paraFrm_position').value="";  
      if(status=='selF9'){
   callsF9(500,325,'AppointLetterAnalysis_f9actionReqName.action'); 
   }
}
...
{ 
 ...
 var dateFilter= document.getElementById('paraFrm_dateFilter').value;
   ...
   if(status=='selF9'){
   callsF9(500,325,'AppointLetterAnalysis_f9Position.action');
   }
} 
...
var fieldName =["paraFrm_frmDate","paraFrm_toDate","paraFrm_appointmentDate"];
 ...
 function callSelectReq(status)
 {
  ...
  if(status=='edit'){
   ...
   document.getElementById('paraFrm').action="AppointLetterAnalysis_displayReq.action";
   document.getElementById('paraFrm').submit();
