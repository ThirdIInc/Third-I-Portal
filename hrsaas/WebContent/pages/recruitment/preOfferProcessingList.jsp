	        	...
	        	<td colspan="3" width="100%">
	        		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="formbg">
	        			 <tr>
				          ...
				          <td width="93%" class="txt"><strong class="text_head">Pre Offer Processing List </strong></td>
...
<label  class = "set" name="serial.no" id="serial.no" ondblclick="callShowDiv(this);">
						<%=label.get("serial.no")%></label></b>
							...
							<!--  Candidate Name-->
			<label  class = "set" name="cand.name" id="LcanName" ondblclick="callShowDiv(this);">
						<%=label.get("cand.name")%></label></b>
							
							</td>
							<td width="20%" align="center" class="formth">
							<!--  Requisition Code--><b>
						<label  class = "set" name="reqs.code" id="Lreqcode" ondblclick="callShowDiv(this);">
						<%=label.get("reqs.code")%></label></b>
							
							</td>
							<td width="20%" align="center" class="formth">
							
							<!--  Position--><b>
					<label  class = "set" name="position" id="Lposition" ondblclick="callShowDiv(this);">
						<%=label.get("position")%></label></b>
									
							</td>
							<td width="10%" align="center" class="formth">
							
							<!--  Offer Status-->
			<b><label  class = "set" name="Lofferstatus" id="Lofferstatus" ondblclick="callShowDiv(this);">
						<%=label.get("Lofferstatus")%></label></b>
				
							</td>
							
							<td width="10%" align="center" class="formth"><b>
							
							<!--  View CV-->
		<label  class = "set" name="view.cv" id="Lviewcv" ondblclick="callShowDiv(this);">
						<%=label.get("view.cv")%></label></b>
								...
								<!--  Check List Type-->
				<b><label  class = "set" name="Lchecklisttype" id="Lchecklisttype" ondblclick="callShowDiv(this);">
						<%=label.get("Lchecklisttype")%></label></b>
...
<label  class = "set" name="cand.name" id="LcanName" ondblclick="callShowDiv(this);"><%=label.get("cand.name")%></label></b>
							
							</td>
							<td width="20%" align="center" class="formth"><b>
							
<label  class = "set" name="reqs.code" id="Lreqcode" ondblclick="callShowDiv(this);"><%=label.get("reqs.code")%></label></b>
							</td>
							<td width="20%" align="center" class="formth"><b>
							
	<label  class = "set" name="position" id="Lposition" ondblclick="callShowDiv(this);"><%=label.get("position")%></label></b>
							</td>
							<td width="20%" align="center" class="formth"><b>
							
		<label  class = "set" name="Lofferstatus" id="Lofferstatus" ondblclick="callShowDiv(this);">
						<%=label.get("Lofferstatus")%></label></b>
							</td>
...
<label  class = "set" name="view.cv" id="Lviewcv" ondblclick="callShowDiv(this);">
						...
						<s:iterator value="BgpendingChkList">
							...
							<tr <%if(count%2==0){%> class="tableCell1" <%}else{%>
								...
								<td class="sortabletd" width="5%" nowrap="nowrap" align="center"><%=++cn%>&nbsp;&nbsp;
								<% if (status != "C") {%>
								...
								<td class="sortabletd" width="25%"><s:hidden name="Lreqcode"
									id="<%="Lreqcode"+k%>" /> <s:hidden name="Loffercode"
									id="<%="Loffercode"+k%>" /> <s:hidden name="Lcancode"
									id="<%="Lcancode"+k%>" /> <s:property value="Lcandidate" /></td>
								<td class="sortabletd"  width="20%" nowrap="nowrap">
								<s:property value="LreqName" />
									 </td>
					...
					</table><s:hidden name="bgpendinglistLength" />
					</td>
				</tr>
		...
		<tr>
			...
			<s:if test="conduct">
				...
				<s:submit cssClass="token" value="Conduct" action="PreOfferProcessing_conducted"
...
function cancelFun()
{
		...
		document.getElementById('paraFrm').action="PreOfferProcessing_input.action";
...
function callFun(status){
	   ...
	   document.getElementById("paraFrm").action='PreOfferProcessing_callstatus.action?status='+status;
	   document.getElementById("paraFrm").submit();
	}
	...
	function showRecord(fileName)
	{
		...
		document.getElementById('paraFrm').action = "PreOfferProcessing_viewCV.action?fileName="+fileName;
		document.getElementById('paraFrm').submit();
		document.getElementById('paraFrm').target="main";
	}
	...
	function showRequisition(reqCode,status){
		document.getElementById("paraFrm").action='EmployeeRequi_viewReqDetails.action?reqCode='+reqCode+'&menuCode=355&formAction=PreOfferProcessing_callstatus.action&statusKey='+status;
	    document.getElementById("paraFrm").submit();
	}
...
function callPage(id,pageImg){  
	 ...
	 if(pageNo!="0"  & pageNo<totalPage){	
       ...
       if(pageImg=="N")
       {
	    ...
	    document.getElementById('paraFrm').action="PreOfferProcessing_callPage.action";
		document.getElementById('paraFrm').submit(); 
	}	
