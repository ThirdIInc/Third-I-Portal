		...
		<tr>
			...
			<table width="100%" cellspacing="2" cellspacing="2" border="0">
				...
				<s:if test="modeLength">
					...
					%> <a href="#"
						onclick="callPage('1', 'F', '<%=totalPage%>', 'SalaryPlan_callPage.action');">
					<img title="First Page" src="../pages/common/img/first.gif"
						width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('P', 'P', '<%=totalPage%>', 'SalaryPlan_callPage.action');">
					<img title="Previous Page" src="../pages/common/img/previous.gif"
						width="10" height="10" class="iconImage" /> </a> <input type="text"
						name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
						maxlength="10"
						onkeypress="callPageText(event, '<%=totalPage%>', 'SalaryPlan_callPage.action');return numbersOnly();" />
					of <%=totalPage%> <a href="#"
						onclick="callPage('N', 'N', '<%=totalPage%>', 'SalaryPlan_callPage.action')">
					<img title="Next Page" src="../pages/common/img/next.gif"
						class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'SalaryPlan_callPage.action');">
							...
							<table width="100%" border="0" cellpadding="2" cellspacing="2">
								<tr>
									...
									<td class="formth" align="center" width="90%" colspan="1"><label
										class="set" name="zip_no" id="zip_no"
										ondblclick="callShowDiv(this);"><%=label.get("zip_no")%></label></td>
								...
								<s:if test="modeLength">
									...
									<s:iterator value="salaryPlanList">
										<tr <%if(count%2==0){
												%> class="tableCell1"
											...
											ondblclick="javascript:callForEdit('<s:property value="salCode"/>');">
											...
											<s:hidden value="%{salCode}"
												id='<%="salCode" + i%>'></s:hidden>
											<script type="text/javascript">
													records[<%=i%>] = document.getElementById('salCode' + '<%=i%>').value;
												</script>
											<td width="90%" align="left" class="sortableTD" colspan="1"><s:property
												value="zipCode" /> <input type="hidden"
											...
											<td colspan="1" width="5%" id="ctrlShow" align="center"
												...
												name="confChk"
												onclick="callForDelete1('<s:property value="salCode"/>','<%=i%>')" /></td>
										</tr>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'SalaryPlan_addNew.action';
...
function searchFun() {
		...
		document.getElementById("paraFrm").action ='SalaryPlan_f9action.action';
		document.getElementById("paraFrm").submit();
	}
...
function callForEdit(id){
	callButton('NA', 'Y', 2);
	  	...
	  	document.getElementById('paraFrm_salCode').value=id;
	   	document.getElementById("paraFrm").action="SalaryPlan_editSalaryplan.action";
	   	document.getElementById('paraFrm').target = "_self";
	    document.getElementById("paraFrm").submit();
   }
...
function chk(){
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="SalaryPlan_deleteChkRecords.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
