			...
			<table width="100%" border="0" align="right" class="formbg">
				<tr>
					...
					<td colspan="1" width="80%" class="txt"><strong
						class="text_head"><b>Referral Source</b></strong></td>
		...
		<tr>
			...
			<table width="100%" cellspacing="2" cellspacing="2" border="0">
				...
				<s:if test="referralListLength">
					...
					%> <a href="#"
							onclick="callPage('1', 'F', '<%=totalPage%>', 'ReferralSource_callPage.action');">
						<img title="First Page" src="../pages/common/img/first.gif"
							width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('P', 'P', '<%=totalPage%>', 'ReferralSource_callPage.action');">
						<img title="Previous Page" src="../pages/common/img/previous.gif"
							width="10" height="10" class="iconImage" /> </a> <input type="text"
							name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
							maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'ReferralSource_callPage.action');return numbersOnly();" />
						of <%=totalPage%> <a href="#"
							onclick="callPage('N', 'N', '<%=totalPage%>', 'ReferralSource_callPage.action')">
						<img title="Next Page" src="../pages/common/img/next.gif"
							class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'ReferralSource_callPage.action');">
			...
			<table class="formbg" width="100%" cellspacing="2" cellspacing="2"
				...
				<tr>
					<td colspan="1" class="txt" width="5%"><strong
						class="text_head"><b> Referral Source</b> </strong></td>
					<td colspan="2" align="right" width="95%"><input type="button"
							...
							<table width="100%" border="0" cellpadding="2" cellspacing="2">
								<tr>
									...
									<td class="formth" align="center" width="90%" colspan="1"><label
										class="set" name="ref_source" id="ref_source"
										ondblclick="callShowDiv(this);"><%=label.get("ref_source")%></label></td>
									...
									<s:if test="referralListLength">
										...
										<s:iterator value="referralList">
											<tr <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="refCode"/>');">
												...
												<s:hidden value="%{refCode}" id='<%="refCode" + i%>'></s:hidden>
												<script type="text/javascript">
													records[<%=i%>] = document.getElementById('refCode' + '<%=i%>').value;
												</script>
												<td colspan="1" width="90%" align="left" class="sortableTD"><s:property
													value="refSource" /> <input type="hidden"
												...
												<td colspan="1" id="ctrlShow" align="center" nowrap="nowrap"
													...
													onclick="callForDelete1('<s:property value="refCode"/>','<%=i%>')" /></td>
											</tr>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'ReferralSource_addNew.action';
...
function searchFun() {
		...
		document.getElementById("paraFrm").action ='ReferralSource_f9referralAction.action';
		document.getElementById("paraFrm").submit();
	}
...
function callForEdit(id){
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_refCode').value=id;
	   	document.getElementById("paraFrm").action="ReferralSource_editBusinessData.action";
	   	document.getElementById('paraFrm').target = "_self";
	    document.getElementById("paraFrm").submit();
   }
...
function chk(){
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="ReferralSource_deleteChkRecords.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
