...
<s:form action="InformationSystemChangeMisReport" validate="true"
		...
		<tr>
			...
			<table width="100%" class="formbg">
				<tr>
					...
					System Change MIS Report</strong></td>
				...
				<tr>
					...
					<td width="35%">
						...
						<img src="../pages/images/search2.gif" class="iconImage" height="18" align="absmiddle" width="18"
							onclick="javascript:callsF9(500,325,'InformationSystemChangeMisReport_f9TicketNumber.action');">
					</td>
					<td width="20%"><label class="set" id="statusLabel"
						name="statusLabel"><%=label.get("statusLabel")%></label>:</td>
					<td colspan="4">
						<s:select headerKey="-1" headerValue="------------Select--------------" name="applicationStatus" 
							list="#{'D':'Draft', 'P':'Pending','F':'Forwarded', 'A':'Approved', 'R':'Rejected', 'B':'Send Back', 'C':'Closed', 'Z':'Group Forwarded', 'Y':'Activity Closed', 'Q':'Reopen', 'X':'Feedback Given'}">
						</s:select>
					</td>
				</tr>
			</table>
...
function callReportCommon(type, reportName){
	...
	if(reportName == 'MIS') {
		document.getElementById('paraFrm').action = 'InformationSystemChangeMisReport_getInfoChangeReport.action';
	} else {
		document.getElementById('paraFrm').action = 'InformationSystemChangeMisReport_getInfoChangeActivityLogsReport.action';
	}
...
function callResetFun() {
	document.getElementById('paraFrm').target = 'main';
	document.getElementById('paraFrm').action='InformationSystemChangeMisReport_reset.action';
	document.getElementById('paraFrm').submit();
}
</script>