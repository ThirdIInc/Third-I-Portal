...
<s:form action="BRDType" validate="true" id="paraFrm" validate="true" theme="simple">
			...
			<table width="100%" border="0" align="right" class="formbg">
				<tr>
					...
					<td  width="80%" class="txt"><strong
						class="text_head">BRD Type </strong></td>
		...
		<tr>
			...
			<table width="100%" cellspacing="2" cellspacing="2" border="0">
				...
				<s:if test="modeLength">
					...
					%> <a href="#"
						onclick="callPage('1', 'F', '<%=totalPage%>', 'BRDType_callPage.action');">
					<img title="First Page" src="../pages/common/img/first.gif"
						width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('P', 'P', '<%=totalPage%>', 'BRDType_callPage.action');">
					<img title="Previous Page" src="../pages/common/img/previous.gif"
						width="10" height="10" class="iconImage" /> </a> <input type="text"
						name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
						maxlength="10"
						onkeypress="callPageText(event, '<%=totalPage%>', 'BRDType_callPage.action');return numbersOnly();" />
					of <%=totalPage%> <a href="#"
						onclick="callPage('N', 'N', '<%=totalPage%>', 'BRDType_callPage.action')">
					<img title="Next Page" src="../pages/common/img/next.gif"
						class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'BRDType_callPage.action');">
			...
			<table class="formbg" width="100%" cellspacing="2" cellspacing="2"
				...
				<tr>
					...
					<table width="100%" border="0" class="formbg" cellspacing="2"
						...
						<tr>
							<td class="formtext">
							<table width="100%" border="0" cellpadding="2" cellspacing="2" id="typeTableId">
									<tr>
										...
										<td class="formth" align="center" width="85%" ><label
											class="set" name="typeLabel" id="typeLabel"
										...
										<s:if test="modeLength">
											<td align="right" class="formth" id="ctrlShow"
												...
												onclick="selectAllRecords('typeTableId');" />
											</td>
										</s:if>
										<s:else>
								...
								<s:iterator value="typeList">
									<tr <%if(count%2==0){
										...
										<td width="10%" id="ctrlShow" align="center" nowrap="nowrap"
											...
											onclick="callForDelete('<s:property value="typeCode"/>','<%=i%>')" />
										</td>
									</tr>
								</s:iterator>
									<% d = i; %>
...
function addnewFun() {
	...
	document.getElementById('paraFrm').action = 'BRDType_addNew.action';
...
function searchFun() {
	...
	document.getElementById("paraFrm").action ='BRDType_f9action.action';
	document.getElementById("paraFrm").submit();
}
...
function callForEdit(id){
	callButton('NA', 'Y', 2);
  	document.getElementById('paraFrm_typeCode').value=id;
   	document.getElementById("paraFrm").action="BRDType_editData.action";
   	document.getElementById('paraFrm').target = "_self";
    document.getElementById("paraFrm").submit();
}
	...
	try {
		...
		if (selectAllCheckBox) {
			for(var i =1; i<=iteration; i++) {
				...
				document.getElementById('hdeleteCode' + i).value = document.getElementById('typeCode' + i).value;
			} 
		} else {
			for(var i =1; i<=iteration; i++) {
...
function chkDelete() {
	 if(chk()){
		 ...
		 if(con){
		   	document.getElementById('paraFrm').action="BRDType_deleteChkRecords.action";
		   	document.getElementById('paraFrm').target = "_self";
		   	document.getElementById('paraFrm').submit();
		 } else { 
