	...
	<table width="100%" border="0" align="right" class="formbg">
		...
		<tr>
			...
			<table width="100%">
				...
				<s:if test="modeLength">
					...
					%> <a href="#"
						onclick="callPage('1', 'F', '<%=totalPage%>', 'Region_callPage.action');">
					<img title="First Page" src="../pages/common/img/first.gif"
						width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('P', 'P', '<%=totalPage%>', 'Region_callPage.action');">
					<img title="Previous Page" src="../pages/common/img/previous.gif"
						width="10" height="10" class="iconImage" /> </a> <input type="text"
						name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
						maxlength="10"
						onkeypress="callPageText(event, '<%=totalPage%>', 'Region_callPage.action');return numbersOnly();" />
					of <%=totalPage%> <a href="#"
						onclick="callPage('N', 'N', '<%=totalPage%>', 'Region_callPage.action')">
					<img title="Next Page" src="../pages/common/img/next.gif"
						class="iconImage" /> </a>&nbsp; <a href="#"
						onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'Region_callPage.action');">
		...
		<tr>
			...
			<table class="formbg" width="100%" cellspacing="0" cellspacing="1"
				border="0">
				<tr>
					<td colspan="2" class="txt"><strong class="text_head">Region
					Master</strong></td>
							...
							<table width="100%" border="0">
								<tr>
									...
									<td class="formth" align="center" width="90%" colspan="1"><label
										class="set" name="region_name" id="region_name"
										ondblclick="callShowDiv(this);"><%=label.get("region_name")%></label></td>
									...
									<s:if test="modeLength">
										...
										<s:iterator value="regionList">
											<tr <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="regionCode"/>');">
												...
												<s:hidden value="%{regionCode}" id='<%="regionCode" + i%>'></s:hidden>
												<script type="text/javascript">
													records[<%=i%>] = document.getElementById('regionCode' + '<%=i%>').value;
												</script>
												<td colspan="1" width="90%" align="left" class="sortableTD"><s:property
													value="regionName" /> <input type="hidden"
													name="hdeleteCode" id="hdeleteCode<%=i%>" /></td>
												<td colspan="1" width="5%" id="ctrlShow" align="center" nowrap="nowrap"
													class="sortableTD"><input type="checkbox"
													class="checkbox" id="confChk<%=i%>" name="confChk"
													onclick="callForDelete1('<s:property value="regionCode"/>','<%=i%>')" /></td>
											</tr>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'Region_addNew.action';
...
function searchFun() {
		...
		document.getElementById("paraFrm").action ='Region_f9action.action';
		document.getElementById("paraFrm").submit();
	}
...
function callForEdit(id){
	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_regionCode').value=id;
	   	document.getElementById("paraFrm").action="Region_calforedit.action";
	   	document.getElementById('paraFrm').target = "_self";
	    document.getElementById("paraFrm").submit();
   }
...
function chk(){
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="Region_deleteChkRecords.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
