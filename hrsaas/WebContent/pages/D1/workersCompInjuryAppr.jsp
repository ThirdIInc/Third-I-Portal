...
<s:form action="WorkersCompInjuryAppr" name="EmpTypeMaster" id="paraFrm" validate="true" target="main" theme="simple">
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="formbg">
	...
	<tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tr>
				...
				<s:hidden name="flagResubmit"/>
				<s:hidden name="reportingOfficer"/><jsp:include page="/pages/ApplicationStudio/navigationPanel.jsp" />
				<s:if test="%{flag ==''&& flagResubmit ==''}">
				
				 <input type="submit" value="Approve" class="save" onclick="return approveFun();">
				 <input type="submit" value="Reject" class="reject" onclick="rejectFun();">
				  <input type="submit" value="SendBack" class="save" onclick="return sendBackFun();">
				</s:if>
				<s:elseif test="flagResubmit =='true'">
				 <input type="submit" value="Approve" class="save" onclick="return approveFun();">
				</s:elseif>
 ...
 <td colspan="4">
   <table width="100%" cellspacing ="2" cellpadding="2" class="formbg" border="0">		
		...
		<s:if test="%{flag ==''&& flagResubmit ==''}">	
			<tr>
				<td width="25%" colspan="2"><B>Comments By Approver</B>
				</td>
				<td width="70%" colspan="2">
			<s:textarea  name="comments" cols="80" rows="2.5"  id="comments"  onkeypress="return imposeMaxLength(event, this, 900);"/>
...
<s:hidden name="socialInsNo" />
    ...
    <tr>
			...
			<td width="20%" colspan="1"><s:textfield name="companyWork" readonly="true" cssStyle="background-color: #F2F2F2;"  cssStyle="border:none;"/></td>
				...
				<%=label.get("dep")%></label> :</td>
					<td width="20%"><s:textfield name="department" size="30" readonly="true" cssStyle="background-color: #F2F2F2;"  cssStyle="border:none;"/> </td>
    ...
    <tr>
			...
			<s:textfield name="executive" readonly="true" cssStyle="background-color: #F2F2F2;"  cssStyle="border:none;"/>
			</td>
...
<tr>
  ...
  <td width="12%" colspan="1"><s:textfield name="doi" onkeypress="return numbersWithHiphen();"  cssStyle="border:none;" readonly="true"/></td>
	...
	<tr>
			...
			<td colspan="3">
			<s:textfield name="hrsWorkedDate" readonly="true" size="6" onkeypress="return numbersWithColon();" cssStyle="border:none;" maxlength="5" />(HH24:MM)
			</td>
...
<tr>
	...
	<td colspan="1"><s:select list="#{'Y':'Yes','N':'No'}" disabled="true"  name="injuredReturnWork" onchange="callReturnWork()"   /></td>	
	
	 <td id="W1"><label class="datereturn" name="datereturn" id="ssn" ondblclick="callShowDiv(this);">
				<%=label.get("datereturn")%></label></td>
    <td width="12%" colspan="1" id="W2"><s:textfield name="dor"  onkeypress="return numbersWithHiphen();"  cssStyle="border:none;" readonly="true"/>
		</td>
</tr>
 ...
 <tr>
    ...
    <s:textfield  maxlength="5" name="tor"   onkeypress="return numbersWithColon();" size="6"  cssStyle="border:none;" readonly="true"/>(HH24:MM)</td>
</tr>
   ...
   <table width="100%" cellspacing ="1" cellpadding="1" class="formbg" border="0">
	  ...
	  <tr>
			...
			<strong><label class="set" name="completed.by" id="completed.by" ondblclick="callShowDiv(this);">
			 ...
			 :</td>
			<td width="20%" colspan="1"><s:property value="completedBy"/></td>
...
function draftFun() {
	...
	function sendBackFun() {
		var vv=confirm("Do you really want to send back this application?");
	if(vv){
		document.getElementById('paraFrm').target = "_self";
		document.getElementById('paraFrm').action = 'WorkersCompInjuryAppr_sendBack.action';
		document.getElementById('paraFrm').submit();
		}else{
		return false;
		}
	...
	if(vv){
		document.getElementById('paraFrm').target = "_self";
		document.getElementById('paraFrm').action = 'WorkersCompInjuryAppr_approve.action';
		document.getElementById('paraFrm').submit();
		}else{
		return false;
		}
	...
	if(vv){
		document.getElementById('paraFrm').target = "_self";
		document.getElementById('paraFrm').action = 'WorkersCompInjuryAppr_reject.action';
		document.getElementById('paraFrm').submit();
		}else{
		return false;
		}
