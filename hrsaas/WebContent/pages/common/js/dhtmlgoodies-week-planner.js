...
var externalSourceFile_items = 'week_schedule_getItems.jsp';	// File called by ajax when changes are loaded from the server(by Ajax).
var externalSourceFile_save ='WeekPlanner_save.action'; //'week_schedule_save.php';	// File called by ajax when changes are made to an element
...
var externalSourceFile_delete = 'WeekPlanner_delete.action';//'/pages/common/week_schedule_delete.jsp';
var popupWindowUrl = 'WeekPlanner_editEvent.action';	// Called when double clicking on an event. use false if this option is disabled.
...
var initMinutes = 0;	// Used to auto set start time. Example: 15 = auto set start time to 0,15,30 or 45. It all depends on where the mouse is located ondragstart
var snapToMinutes = 1;	// Snap to minutes, example: 5 = allow minute 0,5,10,15,20,25,30,35,40,45,50,55
...
var aa=document.getElementsByName('hidden').value;
...
{	
	...
	//if(inputDiv.id.indexOf('new_')>=0)return;description
	var ddd= appointmentProperties[inputDiv.id]['description'];
      //alert('----description----123--id-'+ddd);
	var ta = document.getElementById(inputDiv.id).getElementsByTagName('TEXTAREA')[0].value;
	//alert('desc-----'+ta);
	var editEvent = window.open(popupWindowUrl + '?id=' + inputDiv.id+ '&desc=' + ddd,'editEvent','width=500,height=500,status=no');
	editEvent.focus();
}
	...
	weekSchedule_ajaxObjects[ajaxIndex] = false;
		...
		itemsToBeCreated[no] = new Array();
			...
			if(key=='eventStartDate' || key=='eventEndDate'){
					...
					//alert('---------value-------------'+value);//value = value+' UTC+530 2008';
					//alert('---------ddddd-------------'+d);
				value = d;
							}		
//alert('----------------------'+value);
...
//alert(' length-------'+ajaxIndex);
	weekSchedule_ajaxObjects[ajaxIndex] = new sack();
	weekSchedule_ajaxObjects[ajaxIndex].requestFile = '../pages/common/week_schedule_getItems.jsp'  + '?year=' + dateStartOfWeek.getFullYear() + '&month=' + (dateStartOfWeek.getMonth()/1+1) + '&day=' + dateStartOfWeek.getDate();	// Specifying which file to get
	...
	weekSchedule_ajaxObjects[ajaxIndex].runAJAX();		// Execute AJAX function	externalSourceFile_items
...
function initNewAppointment(e,inputObj)
{
	...
	//alert('inputObj--------'+inputObj.getElementById('abcd').value);
...
function timerNewAppointment()
{
	...
	//  currentAppointmentDiv.id = 'new_' + startIdOfNewItems;
		currentAppointmentDiv.id =  startIdOfNewItems;
...
{//alert('under save function-------------------');
	...
	+ '&eventStartDate=' + appointmentProperties[inputId]['eventStartDate']
	+ '&eventEndDate=' + appointmentProperties[inputId]['eventEndDate'];
	
	if(appointmentProperties[inputId]['id'].indexOf('')>=0){
...
{//alert('deleteEventFromView-------================'+weekSchedule_ajaxObjects[index].response);
	if('OK'=='OK'){
...
function schedulerKeyboardEvent(e){
	...
	if(e.keyCode==46 && activeEventObj){
		if(confirm(txt_deleteEvent)){
		 ...
		 deleteEventFromView(ajaxIndex); 	// Specify function that will be executed after file has been found
			weekSchedule_ajaxObjects[ajaxIndex].runAJAX();		// Execute AJAX function	
		}		
	}
...
{//alert('22');
	...
	//document.documentElement.ondblclick = schedulerwindow;
...
function schedulerwindow()
{
		...
		document.getElementById('paraFrm').action = "WeekPlanner_editEvent.action";
