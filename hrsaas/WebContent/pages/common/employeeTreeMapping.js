...
function createTree(arrName, startNode, openNode) {
	...
	if (nodes.length > 0) {
		...
		if(checkLength==''){
				document.write(" <font color='red'>No employee is pending for mapping.</font>\ ");
		}
	}
}
...
function moveToGroup(length){
		...
		win=window.open('','win','top=240,left=130,width=400,height=250,scrollbars=no,status=no,resizable=no');
...
function removeFromGroup(length){
		...
		var conf=confirm("Do you really want to remove these employees ?");
  			if(conf) {
		document.getElementById("paraFrm").action="SectionMapping_removeFromGroup.action";
		document.getElementById("paraFrm").submit();
		}
	}
