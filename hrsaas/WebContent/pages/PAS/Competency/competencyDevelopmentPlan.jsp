...
<s:hidden name="compDevName"  /><s:hidden name="isActive"  />
			...
			<td valign="bottom" class="txt">
				<table width="100%" align="right" class="formbg">
					<tr>
						...
						<td width="93%" class="txt"><strong class="text_head">Competency Development Plan Master</strong></td>
				...
				<table width="100%">
					<tr>
						...
						<td id="ctrlShow" width="30%" align="right" class="">
							...
							%>
							<a href="#" onclick="callPage('1', 'F', '<%=totalPage%>', 'CompetencyDevelopmentPlan_callPage.action');">
								<img title="First Page" src="../pages/common/img/first.gif" width="10" height="10" class="iconImage"/>
							</a>&nbsp;
							<a href="#" onclick="callPage('P', 'P', '<%=totalPage%>', 'CompetencyDevelopmentPlan_callPage.action');" >
								<img title="Previous Page" src="../pages/common/img/previous.gif" width="10" height="10" class="iconImage"/>
							</a> 
							<input type="text" name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'CompetencyDevelopmentPlan_callPage.action');return numbersOnly();" /> of <%=totalPage%>
							<a href="#" onclick="callPage('N', 'N', '<%=totalPage%>', 'CompetencyDevelopmentPlan_callPage.action')" >
								<img title="Next Page" src="../pages/common/img/next.gif" class="iconImage" />
							</a>&nbsp;
							<a href="#" onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'CompetencyDevelopmentPlan_callPage.action');" >
		...
		<tr>
			<td colspan="3" width="100%">
				<table class="formbg" width="100%" cellspacing="0" cellspacing="1" border="0">		
					<tr>
						<td colspan="2" class="txt"><strong class="text_head">Competency Development Plan</strong></td>		
						<td align="right">
											...
											onclick="selectAllRecords(this);" />
													...
													<s:iterator value="titleList">
														<tr title="Double click for edit"
															...
															ondblclick="javascript:callForEdit('<s:property value="compDevCode"/>');">			
															<td width="10%" align="center" class="sortableTD" >
																<%= ++cn %>
																<% ++i; %>
																<s:hidden name="srNo" />
															</td>
															<s:hidden value="%{compDevCode}"  id='<%="compDevCode" + i%>'/>
															 <script type="text/javascript">
													records[<%=i%>] = document.getElementById('compDevCode' + '<%=i%>').value;
												</script>			
															<td width="90%" align="left" class="sortableTD">
																<s:property value="compDevName" />
															...
															<td align="center" nowrap="nowrap" id="ctrlShow" class="sortableTD" width="10%">
																...
																onclick="callForDelete1('<s:property value="compDevCode"/>','<%=i%>')" />
															</td>
														</tr>
													</s:iterator>
													<% d = i; %>
...
function addnewFun() {
		...
		document.getElementById('paraFrm').action = 'CompetencyDevelopmentPlan_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='CompetencyDevelopmentPlan_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'CompetencyDevelopmentPlan_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="CompetencyDevelopmentPlan_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="CompetencyDevelopmentPlan_delete1.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
