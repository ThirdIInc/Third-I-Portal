...
<s:form action="AppraisalMisReportKF" validate="true" id="paraFrm"
			...
			<table width="100%" border="0" align="right" cellpadding="0"
				...
				<tr>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td><input type="button" class="token" value=" Submit"
								onclick="return getURL();" /> <input type="button"
								class="token" onclick="resetScreen();" value="Reset" /></td>
							<td>
							...
							<table width="98%" border="0" align="center" cellpadding="0"
								...
								<tr>
									<td colspan="1" width="20%"><label class="set"
										id="appraisal.code" name="appraisal.code"
										ondblclick="callShowDiv(this);"><%=label.get("appraisal.code")%></label>
									<font color="red">*</font>:</td>
									<td colspan="1" width="30%"><s:textfield name="apprCode"
										theme="simple" readonly="true" size="25" /> <img
										src="../pages/images/recruitment/search2.gif"
										class="iconImage" height="18" align="absmiddle" width="18"
										onclick="javascript:callsF9(500,325,'AppraisalMisReportKF_f9AppCal.action');"></td>
									<td colspan="1" width="20%"><s:hidden name="frmDate" /><s:hidden
										name="toDate" /><s:hidden name="apprPeriod" /></td>
								...
								<tr>
									...
									<td colspan="3" width="80%"><s:hidden name="empId" /> <s:textfield
										...
										theme="simple" readonly="true" size="50" name="empName" /><img
										src="../pages/images/recruitment/search2.gif"
										class="iconImage" height="18" align="absmiddle" width="18"
										onclick="callCheck('employee');"></td>
								</tr>
								
								<tr>
									<td colspan="1" width="20%"><label class="set"
										id="reportType" name="reportType" ondblclick="callShowDiv(this);"><%=label.get("reportType")%></label>
									<font color="red">*</font>	:</td>
									<td colspan="3" width="80%"><s:select theme="simple"
							name="reportType" headerKey="Txt" headerValue="Text"
							list="#{'Pdf':'Pdf'}" /></td>
								</tr>
		...
		<tr>
			...
			<table width="100%" class="formbg">
				<s:if test="recordFlag">
					...
					%>
					<tr>
						<td nowrap="nowrap"><a href="#" onclick="callFun('<%=i%>');">Click
						here to Download Appraisal Report for <%=(i * pagecount) + 1%> to <%=(i * pagecount) + pagecount%>
						</a></td>
					</tr>
					<%
							}
					...
					<tr>
						<td nowrap="nowrap"><a href="#" onclick="callFun('<%=i%>');">Click
						here to Download Appraisal Report for <%=(i * pagecount) + 1%> to <%=total%>
						</a></td>
					</tr>
					<%
								}
...
function getURL(){
		...
		document.getElementById('paraFrm').action="AppraisalMisReportKF_getURL.action";	
		document.getElementById('paraFrm').submit();
}
function callFun(rowstart)
 {
 	
   	  document.getElementById("paraFrm").action='AppraisalMisReportKF_getReport.action?rangeValue='+eval(rowstart);
 ...
 function callCheck(check){
		...
		if(check =='div')
			javascript:callsF9(500,325,'AppraisalMisReportKF_f9Division.action');
		if(check =='brn'){
			if(document.getElementById("paraFrm_divisionId").value==""){
				alert("Please select "+document.getElementById('division').innerHTML.toLowerCase());
			...
			javascript:callsF9(500,325,'AppraisalMisReportKF_f9Branch.action');
		}
		...
		if(check =='dept'){
			...
			javascript:callsF9(500,325,'AppraisalMisReportKF_f9Dept.action');
		}
		...
		if(check =='empType'){
			...
			javascript:callsF9(500,325,'AppraisalMisReportKF_f9Emptype.action');
		}
		if(check =='employee'){
			if(document.getElementById("paraFrm_divisionId").value==""){
				alert("Please select "+document.getElementById('division').innerHTML.toLowerCase());
			...
			javascript:callsF9(500,325,'AppraisalMisReportKF_f9Employee.action');	
		}
 }
 function resetScreen(){
 		document.getElementById('paraFrm').action="AppraisalMisReportKF_reset.action";
 		document.getElementById('paraFrm').target="main";	
 		document.getElementById("paraFrm").submit();
 }
</script>
