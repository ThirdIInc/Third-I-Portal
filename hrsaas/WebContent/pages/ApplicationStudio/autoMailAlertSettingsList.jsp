...
<s:form action="AutoMailSetting" id="paraFrm" validate="true"
	target="main" theme="simple">
	<s:hidden name="show" value="%{show}" />
	<s:hidden name="hiddencode" />
	<s:hidden name="autoMailSettings.tempCode" />
	<s:hidden name="autoCode" />
	<s:hidden name="autoMailName" />
	<s:hidden name="isEnable" />
	...
	<table width="100%" border="0" class="formbg" align="right">
		...
		<tr>
			...
			<table width="100%" border="0" align="right" cellpadding="0"
				...
				<tr>
					...
					Auto Mail Alert Settings </strong></td>
			...
			<table width="100%">
				<tr>
					...
					<s:if test="modeLength">
						...
						%> <a href="#"
							onclick="callPage('1', 'F', '<%=totalPage%>', 'AutoMailSetting_callPage.action');">
						<img title="First Page" src="../pages/common/img/first.gif"
							width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('P', 'P', '<%=totalPage%>', 'AutoMailSetting_callPage.action');">
						<img title="Previous Page" src="../pages/common/img/previous.gif"
							width="10" height="10" class="iconImage" /> </a> <input type="text"
							name="pageNoField" id="pageNoField" size="3" value="<%=pageNo%>"
							maxlength="10"
							onkeypress="callPageText(event, '<%=totalPage%>', 'AutoMailSetting_callPage.action');return numbersOnly();" />
						of <%=totalPage%> <a href="#"
							onclick="callPage('N', 'N', '<%=totalPage%>', 'AutoMailSetting_callPage.action')">
						<img title="Next Page" src="../pages/common/img/next.gif"
							class="iconImage" /> </a>&nbsp; <a href="#"
							onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'AutoMailSetting_callPage.action');">
		...
		<tr>
			...
			<table class="formbg" width="100%" cellspacing="0" cellspacing="1"
				border="0">
				<tr>
					<td colspan="2" class="txt"><strong class="text_head">Auto
					Mail Alert Settings</strong></td>
						...
						<tr>
							...
							<table width="100%" border="0" cellpadding="1" cellspacing="1"
								...
								<tr class="td_bottom_border">
									...
									<td class="formth" align="center"><label name="tempName"
										id="tempName" ondblclick=""><%=label.get("templateName")%></label></td>
									<td class="formth" align="center"><label name="autoFlag"
										id="autoFlag" ondblclick=""><%=label.get("isEnable")%></label></td>
									<s:if test="modeLength">
									...
									<s:if test="modeLength">
										...
										<s:iterator value="randItt">
											<tr class="sortableTD" <%if(count%2==0){
												...
												ondblclick="javascript:callForEdit('<s:property value="tempCode"/>');">

												<td width="10%" align="center" class="sortableTD"><%=++cn%>
												<%
												++i;
												%><s:hidden name="srNo" /></td>
												<s:hidden value="%{tempCode}" id='<%="tempCode" + i%>'></s:hidden>
												<script type="text/javascript">
													records[<%=i%>] = document.getElementById('tempCode' + '<%=i%>').value;
												</script>
												<td width="40%" align="left" class="sortableTD"><s:property
													value="tempName" /> <input type="hidden"
												...
												<td id="ctrlShow" align="center" nowrap="nowrap"
													...
													onclick="callForDelete1('<s:property value="tempCode"/>','<%=i%>')" /></td>
...
<script>
function addnewFun() {
		document.getElementById('paraFrm').action = 'AutoMailSetting_addNew.action';
	...
	function searchFun() {
		...
		document.getElementById("paraFrm").action ='AutoMailSetting_f9action.action';
		document.getElementById("paraFrm").submit();
	}
	
	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'AutoMailSetting_report.action';
		document.getElementById('paraFrm').submit();
	}
...
function oldRowColor(cell,val) {
	...
	//	document.getElementById(cell).backgroundColor="#FFA31D";
  	...
  	callButton('NA', 'Y', 2);
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="AutoMailSetting_calforedit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="AutoMailSetting_delete.action";
	   document.getElementById('paraFrm').target = "_self";
	    document.getElementById('paraFrm').submit();
	    }
	    else
...
function chk(){
  ...
  // alert ('x');
		...
		document.getElementById("paraFrm").target = 'myWin';
		document.getElementById("paraFrm").action = 'AutoMailSetting_'+action+'.action';
		document.getElementById("paraFrm").submit();
	}
