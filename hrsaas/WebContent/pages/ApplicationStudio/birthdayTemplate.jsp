...
<%@ taglib uri="http://java.fckeditor.net" prefix="FCK" %>
	...
	%>
<s:form action="BdayTemplate" method="post" name="LetterTemplate"
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<s:hidden name="tempContent" value="%{tempContent}" /> <s:hidden
						...
						cssClass="save" action="BdayTemplate_save" theme="simple"
						value=" Save" onclick="return callSave(); " />
						<s:submit
						cssClass="reset" action="BdayTemplate_reset" theme="simple"
						value=" Reset" />
						</td>
					<td width="22%">
		...
		<tr>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				class="formbg">
				<tr>
					<td width="20%" colspan="1" height="22"><label name="tempName" id="tempName" ondblclick="callShowDiv(this);"><%= label.get("tempName")%></label> <font
						color="red">*</font>:</td>
		...
		<td valign="middle" colspan="1" width="100%">
			<%!int i = 0;%>
			<!-- 
			<input type="button"  value="First Name"  onclick="editArea('<#FIRST_NAME#>');" class="token"/>
			<input type="button"  value="Last Name"  onclick="editArea('<#LAST_NAME#>');" class="token"/>
			<input type="button"  value="Location Name"  onclick="editArea('<#LOCATION_NAME#>');" class="token"/>
			 -->
			<input type="button"  value="Company Name"  onclick="editArea('<#COMPANY_NAME#>');" class="token"/>
			<input type="button"  value="Birth Date"  onclick="editArea('<#BIRTH_DATE#>');" class="token"/>
			<input type="button"  value="Employee List"  onclick="editArea('<#EMP_LIST#>');" class="token"/>
...
<script>
	...
	{
		///setMessage();
		callsF9(500,325,'BdayTemplate_f9action.action');
	}	
</script>