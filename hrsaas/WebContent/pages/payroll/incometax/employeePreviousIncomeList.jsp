...
<s:form action="EmpPrevIncome" id="paraFrm" validate="true"
	theme="simple">
	<s:hidden name="show" />
	<s:hidden name="hiddencode" />
	
	<s:hidden name="prevIncCode" /><s:hidden name="fromYear" /><s:hidden name="toYear" />
	<s:hidden name="empToken" /><s:hidden name="empID" /><s:hidden name="empName" />
	<s:hidden name="panNo" />
		...
		<tr>
				...
				<tr>
					...
					<table align="left" width="100%" cellpadding="0" cellspacing="0">
						<tr>
							...
							%> <a href="#"
									onclick="callPage('1', 'F', '<%=totalPage%>', 'EmpPrevIncome_input.action');">
								<img title="First Page" src="../pages/common/img/first.gif"
									width="10" height="10" class="iconImage" /> </a>&nbsp; <a href="#"
									onclick="callPage('P', 'P', '<%=totalPage%>', 'EmpPrevIncome_input.action');">
								<img title="Previous Page"
									src="../pages/common/img/previous.gif" width="10" height="10"
									class="iconImage" /> </a> <input type="text" name="pageNoField"
									id="pageNoField" size="3" value="<%=pageNo%>" maxlength="10"
									onkeypress="callPageText(event, '<%=totalPage%>', 'EmpPrevIncome_input.action');return numbersOnly();" />
								of <%=totalPage%> <a href="#"
									onclick="callPage('N', 'N', '<%=totalPage%>', 'EmpPrevIncome_input.action')">
								<img title="Next Page" src="../pages/common/img/next.gif"
									class="iconImage" /> </a>&nbsp; <a href="#"
									onclick="callPage('<%=totalPage%>', 'L', '<%=totalPage%>', 'EmpPrevIncome_input.action');">
			...
			%>
							<table width="100%" border="0" cellpadding="1" cellspacing="0"
								...
								<tr>
									<td class="txt"><strong class="text_head">Employee
					Previous Income</strong></td>
								...
								<tr>
									...
									<table width="100%" border="0" cellpadding="1" cellspacing="1">
										<tr>
											...
											<td class="formth" align="center"><label class="set"
												name="finYrFrm" id="finYrFrm1"
												ondblclick="callShowDiv(this);"><%=label.get("finYrFrm")%></label>
											</td>
											<td class="formth" align="center"><label class="set"
												name="finYrTo" id="finYrTo1"
												ondblclick="callShowDiv(this);"><%=label.get("finYrTo")%></label>
											</td>
							...
							int i = 0;
								%>
									...
									%> class="tableCell1"
													...
													ondblclick="javascript:callForEdit('<s:property value="prevIncCodeItr" />');">


													<td width="10%" align="center" class="sortableTD"><%=++cnt%>
													<%++i;%>
													</td>
													<s:hidden value="%{prevIncCodeItr}"
														id='<%="prevIncCodeItr"+i%>' />
													<script type="text/javascript">
											records[<%=i%>] = document.getElementById('prevIncCodeItr'+'<%=i%>').value;
										</script>
													<td width="30%" align="left" class="sortableTD"><s:property
														value="employeeNameItr" /></td>
													<td width="20%" align="left" class="sortableTD"><s:property
														value="fromYearItr" /></td>
													<td width="20%" align="left" class="sortableTD"><s:property
														value="toYearItr" /><input type="hidden"
													...
													<td align="center" nowrap="nowrap" class="sortableTD"
														...
														onclick="callForDelete('<s:property value="prevIncCodeItr" />','<%=i%>')" /></td>
												</tr>
...
<script>
	...
	{	
		...
		document.getElementById("paraFrm").action='EmpPrevIncome_addNew.action';
	...
	function searchFun() {
			...
			document.getElementById("paraFrm").action = 'EmpPrevIncome_f9Search.action';
			document.getElementById("paraFrm").submit();
	}

	function reportFun() {
		document.getElementById('paraFrm').target = "_blank";
		document.getElementById('paraFrm').action = 'EmpPrevIncome_report.action';
		document.getElementById('paraFrm').submit();
	}
...
<script>
	...
	function callForEdit(id){
	  	document.getElementById('paraFrm_hiddencode').value=id;
	   	document.getElementById("paraFrm").action="EmpPrevIncome_callEdit.action";
	...
	function chkDelete()
	{
	 ...
	 if(con){
	   document.getElementById('paraFrm').action="EmpPrevIncome_deleteList.action";
	    document.getElementById('paraFrm').submit();
	    }
	        else
	    {     
