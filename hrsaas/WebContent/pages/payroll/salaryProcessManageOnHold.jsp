	...
	<table width="100%" class="formbg"  >
			...
			<td width="100%">
				<table width="100%" class="formbg">
					<tr>
						...
						<td width="92%" class="txt"><strong class="text_head">Salary Process Manage OnHold View</strong></td>
				...
				<table width="100%">
					<tr>
						<td>
							...
							<!--<input type="button" class="save" theme="simple" onclick="return removeOnHold();" value=" Remove OnHold" />
		...
		<tr>		
			<td width="100%">
				<table width="100%" class="formbg" >
					<tr>
						<td width="100%"><b><label id="manage.current.onhold" name="manage.current.onhold" ondblclick="callShowDiv(this);"><%=label.get("manage.current.onhold")%></label></b>
							<table width="100%" align="center" border="0" >
								<tr>
									<td width="20%" align="left" ><label id="select.employee1" name="select.employee" ondblclick="callShowDiv(this);"><%=label.get("select.employee")%></label>  :<font color="red">*</font></td>
									<td width="30%" ><s:textfield name="editEmpName" size="45" readonly="true"  cssStyle="background-color: #F2F2F2; "/>
									<s:hidden name="editEmpCode" /><s:hidden name="editEmpToken" />
									
									</td>
									<td width="3%">
									<img src="../pages/images/recruitment/search2.gif" class="iconImage" height="18" align="middle" width="18" title="Select the employee" onclick="callsF9(500,325,'SalaryProcess_f9onholdEmployee.action');">
									</td>
									<td width="34%" align="left" >
									<s:if test="ledgerStatus=='SAL_FINAL'||ledgerStatus == 'ATTN_UNLOCK'||ledgerStatus == 'ATTN_READY'">
							</s:if>
							<s:else>									
						<s:submit action="SalaryProcess_addonHoldEmpRecord"  value="Add OnHold" cssClass="token" title="Add OnHold"  onclick="return callShowRecord();"/>
							</s:else>		
									</td>
								</tr>
					...
					<tr>
						<td width="100%"><b></b>
							<table width="100%" align="center" border="0" bordercolor="red" id="onHoldTable">
								...
								<td class="formth" width="5%"> <input type="checkbox" title="Select All" name="onholdCheckBox" id="onholdCheckBoxAll" onclick="selectAllRemoveOnHold()"/></td>
								</tr>
								<%int j=1; %>
								...
								<td class="sortableTD"> <input type="checkbox" title="Select the employee" name="onholdCheckBox" id="onholdCheckBox<%=j%>" onclick="callRemoveOnHold('<%=j%>')"/>
								<input type="hidden" name="onholdCheckBoxH" id="onholdCheckBoxH<%=j%>"/></td>
								</tr>
								<%j++ ;%>
								</s:iterator>
...
function removeOnHold(){
	...
	document.getElementById('paraFrm').action="SalaryProcess_removeOnHoldEmp.action";
...
function callSumCredit(id){
			...
			con=confirm('Do you really want to add onhold the record?');
		 	if(con){
			return true;
		 	}
		 	else{
