...
<s:form action="CustomerReportLogin" validate="true" id="paraFrm"
	...
	<s:hidden name="checkUploadProfile" />
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr><s:hidden name="customerCode" />
				<s:if test="customerCode!=''">
				<td align="right"><a 
													href="#?" class="link"
													onclick="javascript:callForBackToList();">Back To List
												</a></td>
												</s:if>
				</tr>
				<tr height="28">
				...
				<tr>
					...
					<tr>
						...
						<s:hidden name="loginMultipleName" />
					...
					<tr>
						...
						<s:iterator value="reportList">
							...
							<td  valign="top">
								...
								<a  title="Home" onClick="callJobNameDetailsWindow('CustomerAccountInfo_generateReport.action?reportCode=<s:property value='listReportCode'/>&accountvalue=<s:property value='listAccountCode'/>&listGroupNameValue=<s:property value='listGroupName'/>&listAccountIDValue=<s:property value='listAccountID'/>&listhCustomerCode=<s:property value='hCustomerCode'/>&&app_random=<%=java.lang.Math.random()%>&publishDatevalue=<s:property value='hPublishDate'/>',event)"
						href="#" class="servicelink" ><s:property value="listReportName" /></a>
							...
							<s:property value="listLastRun" />
								...
								onClick="callJobNameDetailsWindow('CustomerAccountInfo_generateReport.action?reportCode=<s:property value='listReportCode'/>&accountvalue=<s:property value='listAccountCode'/>&listGroupNameValue=<s:property value='listGroupName'/>&listAccountIDValue=<s:property value='listAccountID'/>&listhCustomerCode=<s:property value='hCustomerCode'/>&&app_random=<%=java.lang.Math.random()%>&publishDatevalue=<s:property value='hPublishDate'/>',event)"
								class="contlink"
								title="Generate"
								target="main">
...
<script>
	 ...
	 function callJobNameDetailsWindow(action,e) {
		...
		callPageDisplay(action,'300','100','false',e,160);	 
	}


	
	function callForBackToList()
		{
			try{
			var loginName=(document.getElementById('paraFrm_loginMultipleName').value).replace(/,$/,'');
			//alert(loginName);
					document.getElementById('paraFrm').action = '<%=request.getContextPath()%>/cr/CustomerAccountInfo_getMultipleAccountList.action?loginName='+loginName;
					document.getElementById('paraFrm').submit();
			}
			catch(e)
			{
